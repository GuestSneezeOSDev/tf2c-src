...
#if defined( _WINDOWS ) || defined( _PS3 )
// This strptime implementation is taken from the Google Site Map Generator project:
		...
		switch ( *format )
		{
			...
			case 'A':
			{
				...
				for ( int i = 0; i < 7; ++i )
				{
					...
					if ( !V_strnicmp( kWeekAbbr[i], s, len ) )
					{
						tm->tm_wday = i;
						break;
					}
					...
					if ( !V_strnicmp( kWeekFull[i], s, len ) )
					{
						tm->tm_wday = i;
						break;
					}
			...
			case 'h':
			{
				...
				for ( int i = 0; i < 12; ++i )
				{
					...
					if ( !V_strnicmp( kMonthAbbr[i], s, len ) )
					{
						tm->tm_mon = i;
						break;
					}
					...
					if ( !V_strnicmp( kMonthFull[i], s, len ) )
					{
						tm->tm_mon = i;
						break;
					}
...
char CRTime::sm_rgchLocalTimeCur[16] = { '\0' };
char CRTime::sm_rgchLocalDateCur[16] = { '\0' };
...
//-----------------------------------------------------------------------------
int CRTime::CSecsPassed( void ) const
{
...
//-----------------------------------------------------------------------------
void CRTime::UpdateRealTime( void )
{
	...
	if ( sm_nTimeCur < nTimePrev )
	{
		...
		Warning( "System time went backward by %d seconds\n", nTimePrev - sm_nTimeCur );
	}
	...
	{
#ifdef _WINDOWS
		// Get the local time, generate time and date strings and cache the strings, as we will need these
		// frequently for logs.
...
void CRTime::SetSystemClock( RTime32 rTime32Current )
{
	...
	// If it fails, don't set system clock to garbage.
...
//-----------------------------------------------------------------------------
int CRTime::GetMonth( void ) const
{
	time_t timeCur = m_nStartTime;
	struct tm tmStruct;
	struct tm *ptmCur = m_bGMT ? Plat_gmtime( &timeCur, &tmStruct ) : Plat_localtime( &timeCur, &tmStruct );
...
//-----------------------------------------------------------------------------
int CRTime::GetDayOfYear( void ) const
{
	time_t timeCur = m_nStartTime;
	struct tm tmStruct;
	struct tm *ptmCur = m_bGMT ? Plat_gmtime( &timeCur, &tmStruct ) : Plat_localtime( &timeCur, &tmStruct );
...
//-----------------------------------------------------------------------------
int CRTime::GetDayOfMonth( void ) const
{
	time_t timeCur = m_nStartTime;
	struct tm tmStruct;
	struct tm *ptmCur = m_bGMT ? Plat_gmtime( &timeCur, &tmStruct ) : Plat_localtime( &timeCur, &tmStruct );
...
//-----------------------------------------------------------------------------
int CRTime::GetDayOfWeek( void ) const
{
	time_t timeCur = m_nStartTime;
	struct tm tmStruct;
	struct tm *ptmCur = m_bGMT ? Plat_gmtime( &timeCur, &tmStruct ) : Plat_localtime( &timeCur, &tmStruct );
...
//-----------------------------------------------------------------------------
int CRTime::GetMinute( void ) const
{
	time_t timeCur = m_nStartTime;
	struct tm tmStruct;
	struct tm *ptmCur = m_bGMT ? Plat_gmtime( &timeCur, &tmStruct ) : Plat_localtime( &timeCur, &tmStruct );
...
//-----------------------------------------------------------------------------
int CRTime::GetSecond( void ) const
{
	time_t timeCur = m_nStartTime;
	struct tm tmStruct;
	struct tm *ptmCur = m_bGMT ? Plat_gmtime( &timeCur, &tmStruct ) : Plat_localtime( &timeCur, &tmStruct );
...
//-----------------------------------------------------------------------------
int CRTime::GetISOWeekOfYear( void ) const
{
...
RTime32 CRTime::RTime32FromFmtString( const char *pchFmt, const char* pchValue )
{
	...
	const char *pchYYYY = Q_strstr( pchFmt, "YYYY" ), 
		*pchYY = Q_strstr( pchFmt, "YY" ), 
		*pchMM = Q_strstr( pchFmt, "MM" ), 
		*pchMnt = Q_strstr( pchFmt, "Mnt" ), 
		*pchDD = Q_strstr( pchFmt, "DD" ), 
		*pchThh = Q_strstr( pchFmt, "hh" ), 
		*pchTmm = Q_strstr( pchFmt, "mm" ), 
		*pchTss = Q_strstr( pchFmt, "ss" ), 
		*pchTzone = Q_strstr( pchFmt, "Z0000" );
	if ( pchYYYY )
	{
	...
	if ( pchMnt )
	{
		static const char *rgszMonthNames[] =
		{
			"Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"
		};
...
{
	// Get the first day of start month, add X months to that - guaranteed to be correct month, then get the Nth day of that month.
	return CRTime::RTime32NthDayOfMonth( CRTime::RTime32DateAdd( CRTime( rtime32StartDate ).GetFirstDayOfMonth(), nMonthsToAdd, k_ETimeUnitMonth ), nNthDayOfMonth );
}
