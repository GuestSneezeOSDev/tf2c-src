...
public:
	...
	void SetToCurrentTime( void ) { m_nStartTime = sm_nTimeCur; }
	...
	int	CSecsPassed( void ) const;

	// Time accessors.
	RTime32 GetRTime32( void ) const { return m_nStartTime; }
	void SetRTime32( RTime32 rTime32 ) { m_nStartTime = rTime32; }

	// Access our cached current time value.
	static void UpdateRealTime( void );
	...
	static char *PchTimeCur( void ) { return sm_rgchLocalTimeCur; }
	// Return a representation of the current system date.
	static char *PchDateCur( void ) { return sm_rgchLocalDateCur; }
	...
	int GetMonth( void ) const; // Returns 0..11.
	...
	int GetISOWeekOfYear( void ) const;

	// Handy references to nearby time boundaries, all at 00:00:00.
	static RTime32 RTime32BeginningOfDay( const RTime32 );
	static RTime32 RTime32BeginningOfNextDay( const RTime32 );
	static RTime32 RTime32FirstDayOfMonth( const RTime32 );
	static RTime32 RTime32LastDayOfMonth( const RTime32 );
	static RTime32 RTime32FirstDayOfNextMonth( const RTime32 );
	static RTime32 RTime32LastDayOfNextMonth( const RTime32 );
	...
	// If the month only has K days, K < N, returns Kth day, at 00:00:00.
	static RTime32 RTime32NthDayOfMonth( const RTime32, int nDay );
	...
	RTime32 GetBeginningOfDay( void ) const { return RTime32BeginningOfDay( m_nStartTime ); }		
	RTime32 GetBeginningOfNextDay( void ) const { return RTime32BeginningOfNextDay( m_nStartTime ); }	
	RTime32 GetFirstDayOfMonth( void ) const { return RTime32FirstDayOfMonth( m_nStartTime ); }	
	RTime32 GetLastDayOfMonth( void ) const { return RTime32LastDayOfMonth( m_nStartTime ); }
	RTime32 GetFirstDayOfNextMonth( void ) const { return RTime32FirstDayOfNextMonth( m_nStartTime ); }	
	RTime32 GetLastDayOfNextMonth( void ) const { return RTime32LastDayOfNextMonth( m_nStartTime ); }
	...
	bool BIsGMT( void ) const { return m_bGMT; }
