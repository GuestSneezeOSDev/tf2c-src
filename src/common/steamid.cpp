...
//-----------------------------------------------------------------------------
// Purpose: Converts a string integer to uint64.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Constructor
// Input  : pchSteamID -		Text representation of a SteamID.
//-----------------------------------------------------------------------------
...
void CSteamID::SetFromString( const char *pchSteamID, EUniverse eDefaultUniverse )
{
	...
	else
	{
        ...
        if ( unVal64 > UINT64_MAX )
        {
            ...
            // Catch cases where we're allowing sloppy input that we
            // might not want to allow.
            AssertMsg1( this->operator==( StrictID ), "SteamID does not pass strict parsing: '%s'", pchSteamIDString );
            return;
        }
        else
        {
...
//-----------------------------------------------------------------------------
// Purpose: SetFromString allows many partially-correct strings, constraining how
...
bool CSteamID::SetFromStringStrict( const char *pchSteamID, EUniverse eDefaultUniverse )
{
    ...
    char chPrefix = *pchSteamID;
	EAccountType eAccountType = k_EAccountTypeIndividual;
	uint nAccountID = 0, nInstance = 1, unMaxVal = 2;
	bool bValid = true;
    switch ( chPrefix )
    {
		case 'A':
...
//-----------------------------------------------------------------------------
// Purpose: Initializes a SteamID from a Steam2 ID string.
...
bool CSteamID::SetFromSteam2String( const char *pchSteam2ID, EUniverse eUniverse )
{
	...
	// Customer support is fond of entering SteamIDs in the following form: 'STEAM_n:x:y'.
	...
	// Now convert to SteamID from the Steam2 ID structure.
...
//-----------------------------------------------------------------------------
// Purpose: Renders the SteamID to a buffer. NOTE: For convenience of calling
//			code, this code returns a pointer to a static buffer and is NOT thread-safe.
// Output:  Buffer with rendered SteamID.
//-----------------------------------------------------------------------------
const char *CSteamID::Render() const
{
	...
	if ( m_steamid.m_comp.m_EAccountType == k_EAccountTypeAnonGameServer )
	{
	...
	else if ( m_steamid.m_comp.m_EAccountType == k_EAccountTypeGameServer )
	{
		V_snprintf( pchBuf, k_cBufLen, "[G:%u:%u]", m_steamid.m_comp.m_EUniverse, m_steamid.m_comp.m_unAccountID );
	}
	else if ( m_steamid.m_comp.m_EAccountType == k_EAccountTypeMultiseat )
	{
	...
	else if ( m_steamid.m_comp.m_EAccountType == k_EAccountTypePending )
	{
		V_snprintf( pchBuf, k_cBufLen, "[P:%u:%u]", m_steamid.m_comp.m_EUniverse, m_steamid.m_comp.m_unAccountID );
	} 
	else if ( m_steamid.m_comp.m_EAccountType == k_EAccountTypeContentServer )
	{
		V_snprintf( pchBuf, k_cBufLen, "[C:%u:%u]", m_steamid.m_comp.m_EUniverse, m_steamid.m_comp.m_unAccountID );
	}
	else if ( m_steamid.m_comp.m_EAccountType == k_EAccountTypeClan )
	{
		// 'g' for "group".
		V_snprintf( pchBuf, k_cBufLen, "[g:%u:%u]", m_steamid.m_comp.m_EUniverse, m_steamid.m_comp.m_unAccountID );
	}
	else if ( m_steamid.m_comp.m_EAccountType == k_EAccountTypeChat )
	{
	...
	else if ( m_steamid.m_comp.m_EAccountType == k_EAccountTypeInvalid )
	{
		V_snprintf( pchBuf, k_cBufLen, "[I:%u:%u]", m_steamid.m_comp.m_EUniverse, m_steamid.m_comp.m_unAccountID );
	}
	else if ( m_steamid.m_comp.m_EAccountType == k_EAccountTypeIndividual )
	{
	...
	else if ( m_steamid.m_comp.m_EAccountType == k_EAccountTypeAnonUser )
	{
...
//-----------------------------------------------------------------------------
// Purpose: Renders the passed-in SteamID to a buffer. NOTE: for convenience of calling
//			code, this code returns a pointer to a static buffer and is NOT thread-safe.
// Input:	64-bit representation of SteamID to render.
// Output:  Buffer with rendered SteamID.
//-----------------------------------------------------------------------------
...
bool CSteamID::BValidExternalSteamID() const
{
	...
	if ( m_steamid.m_comp.m_EAccountType != k_EAccountTypeAnonGameServer &&
		m_steamid.m_comp.m_EAccountType != k_EAccountTypeContentServer &&
...
//-----------------------------------------------------------------------------
// Purpose: Asserts SteamID type before conversion.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose:	CGameID "hidden" functions move these somewhere else maybe.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Renders this GameID to string.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Static method to render a uint64 representation of a GameID to a string.
//-----------------------------------------------------------------------------
