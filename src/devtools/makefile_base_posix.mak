...
ifeq ($(CLANG_BUILD),1)
	CXXFLAGS = $(BASE_CFLAGS) -std=c++20 -Wno-c++11-narrowing -Wno-dangling-else $(ENV_CXXFLAGS)
else
	CXXFLAGS = $(BASE_CFLAGS) -std=gnu++20 -fpermissive $(ENV_CXXFLAGS)
...
LDFLAGS = $(CFLAGS) $(GCC_ExtraLinkerFlags) $(OptimizerLevel) $(HARDEN_LFLAGS) -pthread -lstdc++fs
...
CCACHE := $(SRCROOT)/devtools/bin/linux/ccache_4.8.2
#-d /tmp
...
ifeq ($(origin AR), default)
	...
	CC := $(CCACHE) $(STEAM_RUNTIME_PATH)/bin/$(CC)
#endif
#ifeq ($(origin CXX), default)
	CXX := $(CCACHE) $(STEAM_RUNTIME_PATH)/bin/$(CXX)
#endif
