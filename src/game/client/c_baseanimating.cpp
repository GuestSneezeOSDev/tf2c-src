...
// Yes yes, I am stealing code from cathook for now, shut up
class Materials
{
public:
...
void initBullshit()
{
    if (!init_mat)
    {
        ...
        }
        {
            ...
            kv->SetInt("$additive", true);
            kv->SetInt("$pearlescent", true);
...
bool DrawChamsIfNeeded(ClientModelRenderInfo_t* pInfo, DrawModelState_t* pState, matrix3x4_t* pBoneToWorldArray, float* rgba)
{
    initBullshit();
    
    int                 localPlayerIdx      = GetLocalPlayerIndex();
    IClientNetworkable* localCliNetworkable = cl_entitylist->GetClientEntity(localPlayerIdx);
    C_BasePlayer*       localBasePlayer     = static_cast<C_BasePlayer*>(localCliNetworkable);
    C_TFPlayer*         localTFPlayer       = ToTFPlayer(localBasePlayer);
    auto                localPlayerTFClass  = localTFPlayer->GetPlayerClass();
    if (!localPlayerTFClass->IsClass(TF_CLASS_MEDIC))
    {
        return false;
    }

    // Get our mat
    IMaterial* mat = mats.mat_dme_lit;
    // Duh.
    ...
    // 
    if (!pState || !pState->m_pStudioHdr || !pState->m_pStudioHdr->pszName())
    {
        return false;
    }

    // We're doing fast string compares here, if you don't understand what's going on
    // than you should google pointer math
    constexpr const size_t playerSearchStrLen = 7; // strlen("player/");
    const char* modelName = pState->m_pStudioHdr->pszName();

    // not a player model
    if (V_strncmp(modelName, "player/", playerSearchStrLen) != 0)
    {
        return false;
    }

    // strip off the "player/" part
    const char* modelNameNoPlayer = &modelName[playerSearchStrLen];

    // Not a class model. theres gotta be a better way to do this
    if
    (
               strcmp(modelNameNoPlayer, "scout.mdl")       != 0
            && strcmp(modelNameNoPlayer, "soldier.mdl")     != 0
            && strcmp(modelNameNoPlayer, "pyro.mdl")        != 0
            && strcmp(modelNameNoPlayer, "demo.mdl")        != 0
            && strcmp(modelNameNoPlayer, "heavy.mdl")       != 0
            && strcmp(modelNameNoPlayer, "engineer.mdl")    != 0
            && strcmp(modelNameNoPlayer, "medic.mdl")       != 0
            && strcmp(modelNameNoPlayer, "sniper.mdl")      != 0 
            // ^^^ something is wrong with this stupid model and i don't know why but it displays our overlay material wrongly
            && strcmp(modelNameNoPlayer, "spy.mdl")         != 0
    )
    {
        return false;
    }
    
    // get the renderable's ent index from one of our vars
    int renderableEntIdx = pInfo->entity_index;
    // this is to convert it to a CBasePlayer
    IClientNetworkable* renderableCliNetworkable = cl_entitylist->GetClientEntity(renderableEntIdx);
    // This should never happen
    if (!renderableCliNetworkable)
    {
        return false;
    }
    // Can't see them, they're out of pvs
    if (renderableCliNetworkable->IsDormant())
    {
        return false;
    }

    // This is insane but we know it's a baseplayer if they're using a player model
    // At least hopefully. This could possibly crash if you set something
    // that isn't a C_BasePlayer to a player model
    // TODO FIX THAT!!
    C_BasePlayer* renderableBasePlayer = static_cast<C_BasePlayer*>(renderableCliNetworkable);
    if (!renderableBasePlayer)
    {
        return false;
    }

    // Todo: make this account for spies
    int myTeam      = GetLocalPlayerTeam();
    int renderTeam  = renderableBasePlayer->GetTeamNumber();

    // Don't display non teammates
    if (myTeam != renderTeam)
    {
        return false;
    }

    bool isVisible = localBasePlayer->IsAbleToSee(renderableBasePlayer, C_BaseCombatCharacter::FieldOfViewCheckType::USE_FOV);
    ...
    int health      = renderableBasePlayer->GetHealth();
    int maxHealth   = renderableBasePlayer->GetMaxHealth();

    // If this is ever black that's not good
    float color[3]              = { 0.0, 0.0, 0.0 };

    float dyingColor[3]         = { 255.0f / 255, 99.0f / 255, 71.0f / 255 };
    float belowHalfColor[3]     = { 255.0f / 255, 161.0f / 255, 71.0f / 255 };
    float needsHealingColor[3]  = { 162.0f / 255, 148.0f / 255, 71.0f / 255 };
    float okayColor[3]          = { 67.0f / 255, 161.0f / 255, 71.0f / 255 };
    float overhealedColor[3]    = { 1.0f, 1.0f, 1.0f };

    // ratio of how healthy the player is, if it's > 1.0 than theyre overhealed
    // Todo: check for 0s
    float healthRatio = static_cast<float>(health) / maxHealth;

    // lambda to lerp colors into each other
    auto lerpTheseColors = [&](auto cStart, auto cEnd, float flPercent, auto outLerped)
        {
            outLerped[0] = (float)((float)(cStart[0]) + (float)(cEnd[0] - cStart[0]) * flPercent);
            outLerped[1] = (float)((float)(cStart[1]) + (float)(cEnd[1] - cStart[1]) * flPercent);
            outLerped[2] = (float)((float)(cStart[2]) + (float)(cEnd[2] - cStart[2]) * flPercent);
        };

    // Stupid math that i hate involving proportional ratios blah blah blah
    // dying (0-25)
    if (healthRatio < 0.25)
    {
        lerpTheseColors(dyingColor, belowHalfColor, healthRatio * 4.0, color);
    }
    // 25-50
    else if (healthRatio < 0.50)
    {
        lerpTheseColors(belowHalfColor, needsHealingColor, ( healthRatio - 0.25 ) * 4.0, color);
    }
    // 50-101
    else if (healthRatio < 1.01)
    {
        lerpTheseColors(needsHealingColor, okayColor, (healthRatio - 0.50) * 4.0, color);
    }
    // 101-150
    else if (healthRatio < 1.51)
    {
        lerpTheseColors(okayColor, overhealedColor, (healthRatio - 1.0) * 2.0, color);
    }
    else
    {
        AssertMsg(false, "FAILED lerping colors in C_BaseAnimating!!!");
    }

    // store our old color mod in our variable arg
    pState->m_pRenderable->GetColorModulation(rgba);
    // ^
    rgba[3] = render->GetBlend();
    
    
    // Setup material for modulating
    render->SetColorModulation(color);
    // Alpha
    float blend = 0.4f;
    // Set it
    render->SetBlend(blend);
    mat->AlphaModulate(blend);
...
ConVar tf2c_use_beta_completely_unfinished_medic_wallhacks
(
    "tf2c_use_beta_completely_unfinished_medic_wallhacks", "0", FCVAR_REPLICATED
);
//-----------------------------------------------------------------------------
// 
//-----------------------------------------------------------------------------
void C_BaseAnimating::DoInternalDrawModel( ClientModelRenderInfo_t *pInfo, DrawModelState_t *pState, matrix3x4_t *pBoneToWorldArray )
{
	if (pState)
	{
        if (tf2c_use_beta_completely_unfinished_medic_wallhacks.GetBool())
        {

            float originalColor[4] = {};
            bool needToResetOverride = DrawChamsIfNeeded(pInfo, pState, pBoneToWorldArray, originalColor);
            modelrender->DrawModelExecute(*pState, *pInfo, pBoneToWorldArray);

            if (needToResetOverride)
            {
                // Reset it!
                modelrender->ForcedMaterialOverride(nullptr);
                // idiotic type punning, i should be killed for this one
                render->SetColorModulation(static_cast<float*>(originalColor));
                render->SetBlend(originalColor[3]);
...
void C_BaseAnimating::DoAnimationEvents( CStudioHdr *pStudioHdr )
{
	...
	if ( nSeqNum >= nStudioNumSeq )
	{
		...
		DevMsg( 2, "%s[%d]: Playing sequence %d but there's only %d in total?\n", GetDebugName(), entindex(), nSeqNum, nStudioNumSeq );
		return;
	}
