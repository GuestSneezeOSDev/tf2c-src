...
void CBaseLesson::Init()
{
	...
	m_bCanOpenWhenDead			= true; // TODO: DONT DO THIS!!!! EXPOSE TO ENV_INSTRUCTOR_HINT INSTEAD PLS - AZZY 
...
//-----------------------------------------------------------------------------
// Update the current distance if needed based on m_fUpdateDistanceTime and
// then return m_fCurrentDistance.
//-----------------------------------------------------------------------------
float CIconLesson::GetCurrentDistance()
{
...
bool CIconLesson::ShouldDisplay()
{
	VPROF_BUDGET( "CIconLesson::ShouldDisplay", "GameInstructor" );

	if ( m_iHintEntSpawnFlags || m_iHintEntTeam )
	{
		C_TFPlayer *pPlayer = static_cast<C_TFPlayer*>(GetGameInstructor().GetLocalPlayer());
		if ( pPlayer )
		{
	...
	if ( m_fRange > 0.0f && GetCurrentDistance() > m_fRange)
	{
...
LessonVariable LessonVariableFromString( const char *pchName, bool bWarnOnInvalidNames )
{
	...
	{
	//	AssertMsg( 0, "Invalid scripted lesson variable - %s!", pchName );
