...
static ConVar	cl_clean_textures_on_death( "cl_clean_textures_on_death", "0", FCVAR_NONE,  "If enabled, attempts to purge unused textures every time a freeze cam is shown" );
...
void C_BasePlayer::OnDataChanged( DataUpdateType_t updateType )
{
	...
	if ( IsLocalPlayer() )
	{
		...
		if ( m_Local.m_szScriptOverlayMaterial[0] )
		{
			IMaterial *pMaterial = materials->FindMaterial( m_Local.m_szScriptOverlayMaterial, TEXTURE_GROUP_OTHER, false );
...
/*static*/ bool C_BasePlayer::ShouldDrawLocalPlayer()
{
	...
	if ( GetLocalPlayer() && GetLocalPlayer()->ShouldForceDrawLocalPlayer() )
		return true;
