...
public:
	...
	virtual bool			ShouldAnnounceAchievement( void ){ return m_flNextAchievementAnnounceTime < gpGlobals->curtime; }
	void					SetNextAchievementAnnounceTime( float flTime ){ m_flNextAchievementAnnounceTime = flTime; }

#if defined ( USES_ECON_ITEMS ) || defined ( TF_CLASSIC_CLIENT )
	// Wearables
	virtual void			UpdateWearables();
	C_EconWearable			*GetWearable( int i ) const { return m_hMyWearables[i]; }
	...
	bool					ShouldForceDrawLocalPlayer( void ) const { return m_Local.m_bForceLocalPlayerDraw; }
