...
IMaterial *CreateTempMaterialForPlayerLogo( int iPlayerIndex, player_info_t *info, char *texname, int nchars )
{
	...
	std::string strTexNameAsSTDString = fulltexname;
	if (!UTIL_IsVTFValid(strTexNameAsSTDString))
	{
		int localidx = engine->GetLocalPlayer();
		if (localidx == iPlayerIndex)
		{
			INetChannel* nc = static_cast<INetChannel*>(engine->GetNetChannelInfo());
			if (nc)
			{
