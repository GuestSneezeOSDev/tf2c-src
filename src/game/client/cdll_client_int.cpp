...
int CHLClient::Init( CreateInterfaceFn appSystemFactory, CreateInterfaceFn physicsFactory, CGlobalVarsBase *pGlobals )
{
	...
	if ( !CommandLine()->CheckParm( "-noscripting" ) )
	{
		...
		static CDllDemandLoader s_VScript( CFmtStr( "%s/tf2classic/bin/vscript%s", szCwd, DLL_EXT_STRING ) );
	#else
	...
	//
	// In CEngineAPI::ModInit, the engine code INTENTIONALLY only enables FCVAR_SERVER_CAN_EXECUTE and
	// FCVAR_CLIENTCMD_CAN_EXECUTE restrictions for Valve games, and leaves them disabled for third-party mods:
	//
	...
	//
	// This is pants-on-head crazy, because it means that any non-Valve Source Engine-based game/mod will, by default,
	// NOT disallow the server from forcing ANY client to execute literally ANY client-side console command. This is
	// horrendous for security. It means that anyone with the capability of writing a basic SourceMod plugin can just
	// make clients do whatever they want (as if the player had typed the command into their own console). So, for
	// example, an abusive server could easily just decide to send `quit` to a client, and the client would simply go
	// ahead and do exactly the same thing as if the player had manually typed `quit` into their console, skipping any
	// checks about whether the `quit` console command is one that really ought to be allowed to be remotely triggered.
	//
	// Go read the comments above the declarations of IVEngineServer::ClientCommand (public/eiface.h) and
	// IVEngineClient::ClientCmd (public/cdll_int.h) for more info; but basically, under normal circumstances (i.e. when
	// the restrictions are actually ENABLED), IVEngineServer::ClientCommand is unable to force clients to execute any
	// commands other than those which were explicitly declared with the FCVAR_SERVER_CAN_EXECUTE flag; and
	// IVEngineClient::ClientCmd is unable to force itself to execute any commands other than those which were
	// explicitly declared with the FCVAR_CLIENTCMD_CAN_EXECUTE flag. When the restrictions are off, literally any
	// command can be executed via IVEngineServer::ClientCommand or IVEngineClient::ClientCmd, regardless of whether it
	// was declared with the flags indicating that this sort of execution should be allowed. (This is very very bad.)
	//
	// So what we do here is we override the idiocy done by CEngineAPI::ModInit by using the publicly-exposed API
	// functions (which Valve thankfully put in the client engine interface) to explicitly set both restrictions ON, as
	// if we were a Valve game. (Or, you know, as if we wanted to be secure and disallow client abuse by servers.)
...
void CHLClient::PostInit()
{
    ...
    if (!moddir)
    {
        Error(  "Mod directory was null, somehow.\n"
                "Did you install the game properly?"  );
    }

    if
    (
        strcmp(moddir, "tf2classic") != 0
#ifdef TF2C_BETA
        &&
        strcmp(moddir, "tf2classic-beta") != 0
#endif
    )
    {
#ifdef TF2C_BETA
        Error(  "Your game folder must be named \"tf2classic\" or \"tf2classic-beta\", not \"%s\".\n"
                "Did you install the game properly?", moddir  );
#else
        Error(  "Your game folder must be named \"tf2classic\", not \"%s\".\n"
                "Did you install the game properly?", moddir  );
#endif
    }
    auto civString      = g_pVGuiLocalize->Find("#TF_Civilian");
    auto outdatedString = g_pVGuiLocalize->Find("#TF_GameOutdated");

    if ( !civString || !outdatedString )
    {
        Error(  "Failed localization self-test.\n"
                "Did you install the game properly?"  );
    }
...
{
	// BONK: Cheats (at least NaCL) are using g_pClientMode to do stupid bullshit!
	// Change this byte signature to fuck over the lazy!
	// `volatile` is needed or the compiler will optimise this away...
	// -sappho
	
	// IClientMode* volatile realclientmode = g_pClientMode;
	// realclientmode->ProcessInput( bActive );
	
	// UNDONE: i think this might be causing sporadic crashes with g_pClientMode dereferences??
