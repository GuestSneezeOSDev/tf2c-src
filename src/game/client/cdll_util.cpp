...
dworldlight_t* UTIL_GetWorldLights(int* argNumOfWorldLights)
{
	*argNumOfWorldLights = -1;
	// Signature for sub_100FB4E0:
	// 56 57 68 ? ? ? ? FF 15 ? ? ? ? 8B 35 ? ? ? ? 68 ? ? ? ? 
	// unique string
	// "Level unlit, setting 'mat_fullbright 1'\n"
	constexpr const char* pattern = \
		"\x56\x57\x68\x2A\x2A\x2A\x2A\xFF\x15\x2A\x2A\x2A\x2A\x8B\x35\x2A\x2A\x2A\x2A\x68\x2A\x2A\x2A\x2A";
	// dword is offset 0xC5 into the mov + 0x1 for the single eax byte
	static uintptr_t host_state_worldmodel = memy::FindPattern(engine_bin, pattern, 24, 0xC5 + 0x1);

	if (!host_state_worldmodel)
	{
		Warning("[WORLDLIGHTS HACK] host_state_worldmodel == 0, sigscan failed, not grabbing worldlights\n");
		return nullptr;
	}
	// FindPattern gives us a ptr to a ptr 
	uintptr_t real_hswmdl = **(uintptr_t**)host_state_worldmodel;

	// no worldmodel
	if (!real_hswmdl)
	{
		Warning("[WORLDLIGHTS HACK] real_hswmdl ( host_state.worldmodel ) == 0, not grabbing worldlights\n");
		return nullptr;
	}
	/*
	* 
	* 
	Original ASM:
	if ( !*(_DWORD *)(*((_DWORD *)dword_512E69B0 + 0xE) + 0x9C) )
	{
	  ConDMsg("Level unlit, setting 'mat_fullbright 1'\n");
	  sub_50EF8810(&unk_5125DF88, 1);
	}


	sub_xxxxxx+C5   008 A1 B0 69 2E 51         mov     eax, dword_512E69B0 <----- this dword is the value of real_hswmdl 
	sub_xxxxxx+CA   008 8B 40 38               mov     eax, [eax+38h]
	sub_xxxxxx+CD   008 83 B8 9C 00 00 00 00   cmp     dword ptr [eax+9Ch], 0 ; Compare Two Operands
	sub_xxxxxx+D4   008 75 1A                  jnz     short loc_50DAB5D0 ; Jump if Not Zero (ZF=0)
	*/

    constexpr const int pSharedOffs             = 0x38;
    constexpr const int worldLightNumOffs       = 0x9C / 4;
    constexpr const int worldLightArrayOffs     = worldLightNumOffs + 1;


    uintptr_t* pShared              = *( (uintptr_t**)( real_hswmdl + pSharedOffs ) );
    int numOfWorldLights            = *(        (int*)( pShared + worldLightNumOffs ) );
    dworldlight_t* worldLightArray  = *( (dworldlight_t**)( pShared + worldLightArrayOffs ) );


	if (numOfWorldLights <= 0 || !worldLightArray)
	{
		Warning("[WORLDLIGHTS HACK] numOfWorldLights = %i <= 0 || worldLightArray = %p\n", numOfWorldLights, worldLightArray);
		return nullptr;
	}

    for (int i = 0; i < numOfWorldLights; ++i)
    {
        dworldlight_t* light = &worldLightArray[i];
        if (!light->origin.IsValid())
        {
            Warning("[WORLDLIGHTS HACK] %p : isValid() = %i\n", light, light->origin.IsValid());
        }
    }

	*argNumOfWorldLights = numOfWorldLights;
	return worldLightArray;
}

const char* SchemeFactory(std::string_view defaultScheme, std::string_view schemePath, bool appendResExtension = false)
{
    std::string schemaStr;
    if (appendResExtension)
    {
        schemaStr = fmt::format(FMT_STRING("{:s}{:s}.res"), schemePath, defaultScheme);
    }
    else
    {
        schemaStr = fmt::format(FMT_STRING("{:s}{:s}"), schemePath, defaultScheme);
    }

    const std::size_t schemaSize    = schemaStr.size();
    const char*       schemaCStr    = schemaStr.c_str();
    const char*       outptr        = new char[schemaSize+1]{};
    memcpy( (void*)outptr, schemaCStr, schemaSize );
    return outptr;
}

const char* ClientSchemesArray[SCHEME_COUNT] = {};
#include <misc_helpers.h>
void SetupTF2CScheme()
{
	std::string strDefaultScheme;
    if ( checkWine() )
    {
		strDefaultScheme = "linux";
    }
    
    // get user defined custom schema if they want one, otherwise fall back to above
	if( CommandLine()->FindParm("-defaultscheme") )
		strDefaultScheme = CommandLine()->ParmValue( "-defaultscheme", "" ); // no value means we want the default. make it empty

    // prepend underscore to our scheme value
    // only if it's something rather than an empty string
    if ( !strDefaultScheme.empty() )
	{
		strDefaultScheme = fmt::format( FMT_STRING("_{:s}"), strDefaultScheme );
	}

	ClientSchemesArray[SCHEME_CLIENT_PATHSTRING]        = SchemeFactory(strDefaultScheme, "resource/ClientScheme",      true);
	ClientSchemesArray[SCHEME_CLIENT_PATHSTRINGTF2C]    = SchemeFactory(strDefaultScheme, "resource/ClientScheme_tf2c", true);
	ClientSchemesArray[SCHEME_CHAT_PATHSTRING]          = SchemeFactory(strDefaultScheme, "resource/ChatScheme",        true);
	ClientSchemesArray[SCHEME_SOURCE_PATHSTRING]        = SchemeFactory(strDefaultScheme, "resource/SourceScheme",      true);

    ClientSchemesArray[SCHEME_CLIENT_STRING]            = SchemeFactory(strDefaultScheme, "ClientScheme");
    ClientSchemesArray[SCHEME_CLIENT_STRINGTF2C]        = SchemeFactory(strDefaultScheme, "ClientScheme_tf2c");
	ClientSchemesArray[SCHEME_CHAT_STRING]              = SchemeFactory(strDefaultScheme, "ChatScheme");
	ClientSchemesArray[SCHEME_SOURCE_STRING]            = SchemeFactory(strDefaultScheme, "SourceScheme");
}

void FreeTF2CScheme()
{
    for (const char* str : ClientSchemesArray)
    {
        delete [] str;
    }
}
...
bool IsWine()
{
	// i'll just keep this here for generic reasons
	static bool bIsWine = checkWine() && !CommandLine()->CheckParm("-disablewinefixes");
	return bIsWine;
}
