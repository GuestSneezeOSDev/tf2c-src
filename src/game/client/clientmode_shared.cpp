...
#define ACHIEVEMENT_ANNOUNCEMENT_MIN_TIME 3.0f
...
ConVar cl_show_num_particle_systems( "cl_show_num_particle_systems", "0", FCVAR_CHEAT | FCVAR_CLIENTDLL, "Display the number of active particle systems." );
...
void ClientModeShared::ReloadScheme( bool flushLowLevel )
{
	...
	m_pViewport->ReloadScheme( ClientSchemesArray[SCHEME_CLIENT_PATHSTRING] );
	ClearKeyValuesCache();
}
...
int	ClientModeShared::KeyInput( int down, ButtonCode_t keynum, const char *pszCurrentBinding )
{
	if ( engine->Con_IsVisible() )
		return 1;
	/*
	Speculative fix for crashes in this func and its children
	if ( !pszCurrentBinding || !pszCurrentBinding[0] )
	{
		return 1;
	}
	*/
...
void ClientModeShared::FireGameEvent( IGameEvent *event )
{
	...
	else if (Q_strcmp("achievement_earned", eventname) == 0)
	{
		...
		// this message is never for a local player
		// But I am going to leave local player checks so we can revert easily anytime
		if (!IsInCommentaryMode() && !pPlayer->IsLocalPlayer())
		{
			...
			if (pAchievement)
			{
				...
				if (g_PR && (!bStreamerMode || pPlayer->IsLocalPlayer()))
#else
				if (g_PR)
#endif
				{
	...
	else if (Q_strcmp("achievement_earned_local", eventname) == 0)
	{
		int iAchievement = event->GetInt("achievement");
		...
		if (!IsInCommentaryMode())
		{
			...
			if (g_PR)
			{
				...
				if (pchLocalizedAchievement)
				{
					...
					hudChat->ChatPrintf(pPlayer->entindex(), CHAT_FILTER_ACHIEVEMENT, "%s", szLocalized);
				}
			}
		}
	}
			...
			if ( pszLocString )
			{
				...
				{	
					if ( pItemRarity )
					{
						...
						vgui::HScheme scheme = vgui::scheme()->GetScheme( ClientSchemesArray[SCHEME_CLIENT_STRING] );
