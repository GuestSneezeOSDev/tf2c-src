...
void C_WaterExplosionEffect::Create( const Vector &position, float force, float scale, int flags )
{
	...
	// Make sure there's room for the effect i.e. not an underwater tunnel
	vecTrace.Init( 0, 0, 128 );
	UTIL_TraceLine( m_vecWaterSurface, m_vecWaterSurface + vecTrace, MASK_OPAQUE, NULL, COLLISION_GROUP_NONE, &tr );
	if ( tr.DidHit() )
		return;
