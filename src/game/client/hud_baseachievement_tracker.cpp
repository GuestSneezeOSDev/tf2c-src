...
{
	return ( hud_achievement_tracker.GetBool() && pAchievement && pAchievement->ShouldShowOnHUD() && !pAchievement->IsAchieved() && !m_bNoAchievements );
}
...
void CHudBaseAchievementTracker::UpdateAchievementItems()
{
	...
	m_bNoAchievements = !pAchievementMgr->CheckAchievementsEnabled( true );
