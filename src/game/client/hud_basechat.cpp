...
ConVar tf2c_message_alert( "tf2c_message_alert", "1", FCVAR_ARCHIVE, "Plays a sound when a chat message has been received." );
ConVar tf2c_message_volume( "tf2c_message_volume", "1.0", FCVAR_ARCHIVE, "Alters the volume of the chat message sound.", true, 0.0f, true, 1.0f );
...
#define UBER_READY_TOKEN "#Voice_Menu_ChargeReadySubtitle"
#define UBER_ALMOST_READY_TOKEN "#Voice_Menu_ChargeAlmostReadySubtitle"
#define UBER_NOT_READY_TOKEN "#Voice_Menu_ChargeNotReadySubtitle"
...
#ifdef TF_CLASSIC_CLIENT
//-----------------------------------------------------------------------------
// Purpose: Plays a slightly customizable version of the 'HudChat.Message' soundscript.
//-----------------------------------------------------------------------------
void PlayChatSound( void )
{
	if ( tf2c_message_alert.GetBool() )
	{
		CSoundParameters params;
		if ( C_BaseEntity::GetParametersForSound( "HudChat.Message", params, NULL ) )
		{
			params.volume *= tf2c_message_volume.GetFloat();
...
{
	vgui::HScheme scheme = vgui::scheme()->LoadSchemeFromFileEx( enginevgui->GetPanel( PANEL_CLIENTDLL ), ClientSchemesArray[SCHEME_CHAT_PATHSTRING], ClientSchemesArray[SCHEME_CHAT_STRING] );
	SetScheme(scheme);
...
void CBaseHudChat::Init( void )
{
    ...
    bool initedFilterText = steamapicontext->SteamUtils()->InitFilterText(0);
    Assert(initedFilterText);
    DevMsg(2, "initedFilterText = %i\n", initedFilterText);
...
#ifdef TF_CLASSIC_CLIENT
	// TODO: Possibly replace player names with class names instead (optional).
...
void CBaseHudChat::MsgFunc_SayText2( bf_read &msg )
{
    ...
    CSteamID steamid = GetSteamIDForPlayerIndex(client);

	// keep reading strings and using C format strings for subsituting the strings into the localised text string
	ReadChatTextString ( msg, szBuf[1], sizeof( szBuf[1] ) );		// player name
    // TODO: Discuss censoring names?


	ReadChatTextString ( msg, szBuf[2], sizeof( szBuf[2] ) );		// chat text

    wchar_t chatStr[256] = {};
    static_assert( sizeof(chatStr) == sizeof(szBuf[2]) );
    memcpy(chatStr, szBuf[2], sizeof(chatStr));

    char ansiChatStr[512] = {};
    g_pVGuiLocalize->ConvertUnicodeToANSI( ConvertCRtoNL(chatStr), ansiChatStr, sizeof(ansiChatStr) );
    char filteredChatString[512] = {};
    steamapicontext->SteamUtils()->FilterText(k_ETextFilteringContextChat, steamid, ansiChatStr, filteredChatString, sizeof(filteredChatString));
    g_pVGuiLocalize->ConvertANSIToUnicode( filteredChatString, szBuf[2], sizeof(szBuf[2]) );
    ...
    std::string chatMsg(ansiString);
    chatMsg = UTIL_StripCharsFromSTDString(chatMsg, '\n');
    chatMsg = UTIL_StripCharsFromSTDString(chatMsg, '\r');
    chatMsg = UTIL_StripCharsFromSTDString(chatMsg, '\0');


    char finalString[512] = {};
    snprintf(finalString, 512, "%s", chatMsg.c_str());
	if ( bWantsToChat )
	{
		...
		// print raw chat text
		ChatPrintf(client, iFilter, "%s", finalString);

        // !!!
        Color clr = GetClientColor(client);
        ConColorMsg( clr, "%s\n", RemoveColorMarkup( finalString ) );
	...
	{
		// print raw chat text
		ChatPrintf( client, GetFilterForString(untranslated_msg_text), "%s", finalString);
	}
}
...
void CBaseHudChat::MsgFunc_VoiceSubtitle( bf_read &msg )
{
	...
	int iChargeLevel = -1;
	bool bNeedsChargeLevelNumber = false;
	// If we're saying "UberCharge Ready" we should use a different localisation token based on our uber amount
	if ( !strcmp( pszSubtitle, UBER_READY_TOKEN ) ) {
		IClientNetworkable *pClient = cl_entitylist->GetClientEntity( client );
		if ( pClient )
		{
			C_BasePlayer *pPlayer = dynamic_cast<C_BasePlayer*>(pClient);
			if ( pPlayer )
			{
				CTFPlayer *pMedicCalling = ToTFPlayer( pPlayer );
				if ( pMedicCalling )
				{
					ITFHealingWeapon *pMedigun = pMedicCalling->GetMedigun();
					if ( pMedigun )
					{
						float flChargeLevel = pMedigun->GetChargeLevel();

						iChargeLevel = (int)(flChargeLevel * 100);

						// If the charge is 100%, that's conveyed in the default message
						// so we needn't do anything special.
						if ( flChargeLevel < 1 )
						{
							if ( flChargeLevel > tf2c_uber_readiness_threshold.GetFloat() )
							{
								pszSubtitle = UBER_ALMOST_READY_TOKEN;
							}
							else
							{
								pszSubtitle = UBER_NOT_READY_TOKEN;
							}

							bNeedsChargeLevelNumber = true;
						}
					}
				}
			}
	...
	wchar_t *pBuf = g_pVGuiLocalize->Find( pszSubtitle ); // Token in Unicode is now in pBuf
	if ( bNeedsChargeLevelNumber )
	{
		//char pBufASCII[256];
		//g_pVGuiLocalize->ConvertUnicodeToANSI( pBuf, pBufASCII, sizeof( pBufASCII ) );
		//Q_snprintf( pBufASCII, sizeof( pBufASCII ), pBufToken, iChargeLevel );
		//V_snwprintf( pBufTemp, sizeof( pBufTemp ), pBuf, iChargeLevel );
		//pBuf = pBufTemp;

		wchar_t string1[1024];

		wchar_t wszChargeLevel[ 5 ];
		
		V_swprintf_safe( wszChargeLevel, L"%d", iChargeLevel );
...
Color CBaseHudChat::GetTextColorForClient( TextColor colorNum, int clientIndex )
{
	...
	case COLOR_ACHIEVEMENT:
		{
			vgui::IScheme *pSourceScheme = vgui::scheme()->GetIScheme( vgui::scheme()->GetScheme( ClientSchemesArray[SCHEME_SOURCE_STRING] ));
			if ( pSourceScheme )
			{
...
{
	vgui::IScheme *pScheme = vgui::scheme()->GetIScheme( vgui::scheme()->GetScheme( ClientSchemesArray[SCHEME_CLIENT_STRING] ) );
