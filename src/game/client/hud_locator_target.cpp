...
ConVar locator_text_drop_shadow( "locator_text_drop_shadow", "0", FCVAR_NONE, "If enabled, a drop shadow is drawn behind caption text." );
...
public:
	...
	int DrawTargetCaption( CLocatorTarget *pTarget, int x, int y );
	...
	for( int i = 0 ; i < MAX_LOCATOR_TARGETS ; i++ )
	{
		if( m_targets[ i ].m_isActive )
		{
			if (gpGlobals->framecount - m_targets[i].m_frameLastUpdated > 20) {
				...
				if ( m_pBackgroundPanel ) m_pBackgroundPanel->SetVisible( false );
			}
		}
	}
}
...
void CLocatorPanel::DrawStaticIcon( CLocatorTarget *pTarget )
{
	...
	pTarget->m_captionWide = DrawTargetCaption(pTarget, pTarget->GetIconX() + iconWide + ICON_GAP, pTarget->GetIconCenterY()) + YRES(5); // 5 padding.
...
void CLocatorPanel::DrawDynamicIcon( CLocatorTarget *pTarget, bool bDrawCaption, bool bDrawSimpleArrow )
{
	...
	m_pBackgroundPanel->SetVisible(pTarget->m_bOnscreen && bShouldDrawCaption && bDrawCaption);
	...
	else if( bShouldDrawCaption )
	{
		if( bDrawCaption )
		{
			...
			DrawTargetCaption( pTarget, pTarget->GetIconCenterX() + ICON_GAP + pTarget->GetIconWidth() * 0.5, pTarget->GetIconCenterY() );
		}
		if ( pTarget->DrawBindingName() )
		{
...
//-----------------------------------------------------------------------------
int CLocatorPanel::DrawTargetCaption( CLocatorTarget *pTarget, int x, int y )
{
	...
	m_pBackgroundPanel->SetPaintBackgroundEnabled(locator_background_style.GetBool());
