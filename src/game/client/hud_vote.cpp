...
ConVar cl_vote_ui_active_after_voting( "cl_vote_ui_active_after_voting", "0", FCVAR_CLIENTDLL | FCVAR_ARCHIVE );
ConVar cl_vote_ui_show_notification( "cl_vote_ui_show_notification", "0", FCVAR_CLIENTDLL | FCVAR_ARCHIVE );
...
#if defined TF_CLIENT_DLL || defined TF_CLASSIC_CLIENT
	vgui::HScheme scheme = vgui::scheme()->LoadSchemeFromFileEx( enginevgui->GetPanel( PANEL_CLIENTDLL ), ClientSchemesArray[SCHEME_CLIENT_PATHSTRING], ClientSchemesArray[SCHEME_CLIENT_STRING]);
	SetScheme(scheme);
#else
...
void CVoteSetupDialog::OnCommand(const char *command)
{
	...
	if ( !V_stricmp( command, "CallVote" ) )
	{
		...
		if ( iSelectedItem >= 0 )
		{
			...
			else if ( !V_stricmp( "ChangeCivilian", szIssueRaw ) )
			{
				// Get selected Player
				int iSelectedParam = m_pVoteParameterList->GetSelectedItem();
				if ( iSelectedParam >= 0 )
...
void CVoteSetupDialog::OnItemSelected( vgui::Panel *panel )
{
	if ( panel == m_pVoteSetupList )
	{
		...
		if ( iSelectedItem >= 0 )
		{
			...
			else if ( !V_stricmp( "ChangeCivilian", pszIssueRaw ) )
			{
				// Feed the player list to the parameters list
				int nMaxClients = engine->GetMaxClients();
				for ( int playerIndex = 1; playerIndex <= nMaxClients; playerIndex++ )
				{
					...
					// Can't list people on the other team, so don't list them
