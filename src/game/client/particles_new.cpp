...
CNewParticleEffect::CNewParticleEffect( CBaseEntity *pOwner, CParticleSystemDefinition *pEffect )
{
	...
	IMaterial* pImat = pEffect->GetMaterial();
	const char* effectName = pEffect->GetName();
	if (!effectName)
	{
		Warning("NULL ->GetName() on pEffect* %p! Aborting...\n", pEffect);
		AssertMsg(0, "NULL ->GetName() on pEffect* %p! Aborting...\n", pEffect);
		AssertMsg(0, "NULL pEffect->GetName()! FIX THIS!! Aborting...\n");
		return;
	}
	if (!effectName[0])
	{
		Warning("BLANK ->GetName() on pEffect* %p! Aborting...\n", pEffect);
		AssertMsg(0, "BLANK ->GetName() on pEffect* %p! Aborting...\n", pEffect);
		AssertMsgAlways(0, "BLANK pEffect->GetName()! FIX THIS!! Aborting...\n");
		return;
	}
	if (!pImat)
	{
		Warning("NULL IMaterial* for pEffect %p with\n->GetName() %s\n->MaterialName() %s!\nAborting...\n", pEffect, effectName, pEffect->MaterialName());
		AssertMsg(0, "NULL IMaterial* for pEffect %p, see console!\nAborting...\n", pEffect);
		AssertMsgAlways(0, "NULL IMaterial*, see console! FIX THIS!!\nAborting...\n");
		return;
	}
