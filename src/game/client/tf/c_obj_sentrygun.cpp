...
BEGIN_NETWORK_TABLE_NOBASE(C_ObjectFlameSentry, DT_FlameSentryLocalData)
	RecvPropInt(RECVINFO(m_iKills)),
	RecvPropInt(RECVINFO(m_iAssists)),
END_NETWORK_TABLE()

IMPLEMENT_CLIENTCLASS_DT(C_ObjectFlameSentry, DT_ObjectFlameSentry, CObjectFlameSentry)
	RecvPropInt(RECVINFO(m_iAmmoShells)),
	RecvPropInt(RECVINFO(m_iAmmoRockets)),
	RecvPropInt(RECVINFO(m_iState)),
	RecvPropDataTable("FlameSentryLocalData", 0, 0, &REFERENCE_RECV_TABLE(DT_FlameSentryLocalData)),
END_RECV_TABLE()
...
void C_ObjectSentrygun::GetStatusText( wchar_t *pStatus, int iMaxStatusLen )
{
	...
	else if ( m_iUpgradeLevel < 3 )
	{
		...
		V_swprintf_safe( wszShells, L"%d/%d", m_iAmmoShells, m_iMaxAmmoShells );
	...
	else
	{
		...
		V_swprintf_safe( wszShells, L"%d/%d", m_iAmmoRockets, SENTRYGUN_MAX_ROCKETS );
...
void C_ObjectSentrygun::DisplayHintTo( C_BasePlayer *pPlayer )
{
	...
	if ( InSameTeam( pPlayer ) )
	{
		...
		if ( pTFPlayer->IsPlayerClass( TF_CLASS_ENGINEER, true ) )
		{
			...
			{
				if ( pTFPlayer->GetBuildResources() >= GetUpgradeMetalRequired() )
				{
...
void C_ObjectSentrygun::GetTargetIDString( wchar_t *sIDString, int iMaxLenInBytes )
{
	...
	if ( GetTeamNumber() == pLocalPlayer->GetTeamNumber() )
	{
		...
		V_swprintf_safe( wszHealthText, L"%.0f%%", ( (float)GetHealth() / (float)GetMaxHealth() ) * 100 );
...
const char *C_ObjectSentrygun::GetHudStatusIcon( void ) const
{
...
BuildingHudAlert_t C_ObjectSentrygun::GetBuildingAlertLevel( void ) const
{
	...
	if ( !IsBuilding() && flShellPercent < 0.25f )
	{
		alertLevel = BUILDING_HUD_ALERT_VERY_LOW_AMMO;
	}
	else if ( !IsBuilding() && flShellPercent < 0.50f )
	{
...
C_ObjectFlameSentry::C_ObjectFlameSentry()
{
	m_pDamageEffects = NULL;
	m_iOldUpgradeLevel = 0;
	m_iMaxAmmoShells = SENTRYGUN_MAX_SHELLS_1;
...
C_ObjectFlameSentry::~C_ObjectFlameSentry()
{
	...
	StopSound( "Building_Sentrygun.FireFlame" );
}

void C_ObjectFlameSentry::OnDataChanged(DataUpdateType_t updateType)
{
	BaseClass::OnDataChanged(updateType);

	const char* pszEffectName = ConstructTeamParticle("flamesentry", GetTeamNumber());
	if (m_iState == SENTRY_STATE_ATTACKING && m_iAmmoShells > 0) {
		if (!m_bHasFlames) {

			// Start the effect on the viewmodel if our owner is the local player.
			ParticleProp()->Create(pszEffectName, PATTACH_POINT_FOLLOW, "muzzle");
			ParticleProp()->AddControlPoint(m_pFlameEffect, 1, this, PATTACH_ABSORIGIN_FOLLOW, NULL, Vector(0, 0, 50));
			EmitSound("Building_Sentrygun.FireFlame");
			m_bHasFlames = true;
		}
	}
	else if (m_bHasFlames) {
		ParticleProp()->StopEmission();
		StopSound("Building_Sentrygun.FireFlame");
		EmitSound("Building_Sentrygun.StopFlame");
		m_bHasFlames = false;
	}
}
