...
{
	char szEffect[128];
	V_sprintf_safe( szEffect, "teleporter_%s_charged_level%d", GetTeamSuffix( GetTeamNumber() ), m_iUpgradeLevel );

	Assert( !m_pChargedEffect );
...
{
	char szEffect[128];
	V_sprintf_safe( szEffect, "teleporter_%s_%s_level%d",
	...
	// Arm glow effects.
	V_sprintf_safe( szEffect, "teleporter_arms_circle_%s", GetTeamSuffix( GetTeamNumber() ) );
	...
	if ( !m_pSpinSound )
	{
		...
		V_sprintf_safe( szShootSound, "Building_Teleporter.SpinLevel%d", m_iUpgradeLevel );
...
void C_ObjectTeleporter::OnDataChanged( DataUpdateType_t updateType )
{
	...
	if ( m_pSpinSound )
	{
		...
		controller.SoundChangePitch( m_pSpinSound, GetPlaybackRate() * 100.0f, 0.1f );
	}
}
...
void C_ObjectTeleporter::GetTargetIDDataString( wchar_t *sDataString, int iMaxLenInBytes )
{
	...
	else if ( m_iState == TELEPORTER_STATE_RECHARGING && gpGlobals->curtime < m_flRechargeTime )
	{
		...
		V_swprintf_safe( wszRecharging, L"%.0f", 100 - ( flPercent * 100 ) );
