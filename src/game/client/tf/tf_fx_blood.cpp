...
//-----------------------------------------------------------------------------
void TFBloodSprayCallback( Vector vecOrigin, Vector vecNormal, ETFDmgCustom iDamageCustom, ClientEntityHandle_t hEntity )
{
	QAngle	vecAngles;
	VectorAngles( -vecNormal, vecAngles );

	C_TFPlayer *pPlayer = ToTFPlayer( ClientEntityList().GetBaseEntityFromHandle( hEntity ) );
	if ( !pPlayer || pPlayer->IsDormant() )
		return;

	// determine if the bleeding player is underwater
	bool bUnderwater = ( pPlayer->GetWaterLevel() == WL_Eyes );
	bool bHeadshot = ( !pPlayer->IsAlive() && iDamageCustom == TF_DMG_CUSTOM_HEADSHOT );
