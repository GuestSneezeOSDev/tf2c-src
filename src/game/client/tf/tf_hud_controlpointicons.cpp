...
{
	if ( m_pCountdown && ( TFGameRules() && !TFGameRules()->IsInWaitingForPlayers() ) )
	{
		if ( m_pCountdown->GetUnlockTime() > 0 )
		{
			int nTimeToUnlock = ceil( m_pCountdown->GetUnlockTime() - gpGlobals->curtime );
			if ( nTimeToUnlock < 6 )
			{
				return true;
			}
...
void CHudControlPointIcons::PerformLayout( void )
{
	...
	if ( ObjectiveResource() )
	{
		...
		if ( pszLayout && pszLayout[0] )
		{
			...
			V_strcpy_safe( szBuffer, pszLayout );
...
void CControlPointCountdown::OnTick( void )
{
	...
	int iTimeLeft = ceil( m_flUnlockTime - gpGlobals->curtime );
	...
	if ( iTimeLeft > 5 || iTimeLeft <= 0 )
	{
		if ( iTimeLeft <= 0 && m_bFire0SecRemain )
		{
			...
			g_TFAnnouncer.Speak( TF_ANNOUNCER_ARENA_CAPENABLED );
			m_flUnlockTime = 0.0f;
		}
	...
	V_swprintf_safe( wzTimeLeft, L"%i", iTimeLeft );
	...
	if ( iTimeLeft <= 5 && m_bFire5SecRemain )
	{
		...
		g_TFAnnouncer.Speak( TF_ANNOUNCER_ROUNDBEGINS_5SEC );
	}
	else if ( iTimeLeft <= 4 && m_bFire4SecRemain )
	{
		m_bFire4SecRemain = false;
		g_TFAnnouncer.Speak( TF_ANNOUNCER_ROUNDBEGINS_4SEC );
	}
	else if ( iTimeLeft <= 3 && m_bFire3SecRemain )
	{
		m_bFire3SecRemain = false;
		g_TFAnnouncer.Speak( TF_ANNOUNCER_ROUNDBEGINS_3SEC );
	}
	else if ( iTimeLeft <= 2 && m_bFire2SecRemain )
	{
		m_bFire2SecRemain = false;
		g_TFAnnouncer.Speak( TF_ANNOUNCER_ROUNDBEGINS_2SEC );
	}
	else if ( iTimeLeft <= 1 && m_bFire1SecRemain )
	{
		m_bFire1SecRemain = false;
		m_bFire0SecRemain = true;
		g_TFAnnouncer.Speak( TF_ANNOUNCER_ROUNDBEGINS_1SEC );
	}
}
