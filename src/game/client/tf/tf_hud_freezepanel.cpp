...
ConVar tf2c_freezecammodel( "tf2c_freezecammodel", "0", FCVAR_ARCHIVE );
ConVar tf2c_freezecam_show_killer_weapon( "tf2c_freezecam_show_killer_weapon", "1", FCVAR_ARCHIVE );
...
bool IsTakingAFreezecamScreenshot( void )
{
	...
	if ( bInFreezeCam && engine->IsTakingScreenshot() )
		return true;

	CTFFreezePanel *pPanel = GET_HUDELEMENT( CTFFreezePanel );
	if ( pPanel && pPanel->IsHoldingAfterScreenShot() )
		return true;
	...
	: EditablePanel( NULL, "FreezePanel" ), CHudElement( pElementName )
{
	...
	m_pMedalImage = new vgui::ImagePanel( m_pBasePanel, "MedalImage" );
	m_pFreezePanelBG = new CTFImagePanel( m_pBasePanel, "FreezePanelBG" );
	m_pKillerHealth = new CTFFreezePanelHealth( m_pBasePanel, "FreezePanelHealth" );
...
void CTFFreezePanel::ApplySchemeSettings( vgui::IScheme *pScheme )
{
	...
	m_pItemPanel->SetBorder( pScheme->GetBorder( "TFFatLineBorder" ) );
}

#define FREEZEPANEL_ITEMPANEL_SEPERATION YRES( 1.0f )


void CTFFreezePanel::FireGameEvent( IGameEvent * event )
{
	const char *pszEventName = event->GetName();
	if ( Q_strcmp( "player_death", pszEventName ) == 0 )
	{
		...
		if ( pLocalPlayer && iPlayerIndexVictim == pLocalPlayer->entindex() )
		{
			...
			else if ( nDeathFlags & TF_DEATH_REVENGE )
			{
			...
			m_pKillerItem = GetItemSchema()->GetItemDefinition( event->GetInt( "weapon_def_index" ) );
		}
	}
	else if ( Q_strcmp( "hide_freezepanel", pszEventName ) == 0 )
	{
		Hide();
	}
	else if ( Q_strcmp( "freezecam_started", pszEventName ) == 0 )
	{
		ShowCalloutsIn( 1.0f );
		ShowSnapshotPanelIn( 1.25f );
	...
	else if ( Q_strcmp( "show_freezepanel", pszEventName ) == 0 )
	{
		if ( !g_PR )
		{
		...
		int iYBase = TFGameRules()->RoundHasBeenWon() ? m_iYBase - YRES( 50 ) : m_iYBase;
		int xp, yp;
		GetPos( xp, yp );
		SetPos( xp, iYBase );
		...
		if ( pKiller )
		{
			...
			int iMaxBuffedHealth = pPlayer ? pPlayer->m_Shared.GetMaxBuffedHealth() : 0;
			int iKillerHealth = pKiller->IsAlive() ? pKiller->GetHealth() : 0;
			...
			if ( pKiller->IsPlayer() )
			{
				...
				if ( g_TF_PR->IsPlayerDominating( m_iKillerIndex ) && tf2c_show_nemesis_relationships.GetBool() )
				{
					if ( !pKiller->IsAlive() )
					{
				...
				m_pKillerHealth->ShowBuildingBG( false );
				...
				m_pAvatar->SetPlayer( !bStreamerMode ? (C_BasePlayer *)pKiller : NULL );

				if ( m_pKillerItem && tf2c_freezecam_show_killer_weapon.GetBool() )
				{
					m_pItemPanel->SetVisible( true );
					m_pItemPanel->SetDialogVariable( "killername", g_PR->GetPlayerName( m_iKillerIndex ) );
					m_pItemPanel->SetPos( m_pItemPanel->GetXPos(), ( ( m_pFreezePanelBG->GetYPos() + m_pFreezePanelBG->GetTall() ) + m_pItemPanel->GetTall() ) + FREEZEPANEL_ITEMPANEL_SEPERATION );
					m_pItemPanel->SetItem( m_pKillerItem );
				}
			}
			else if ( pKiller->IsBaseObject() )
			{
				C_BaseObject *pObj = assert_cast<C_BaseObject *>( pKiller );
				int iBuilder = pObj->GetBuilderIndex();
				iKillerTeam = pObj->GetTeamNumber();

				if ( IsPlayerIndex( iBuilder ) )
				{
					m_iKillerIndex = iBuilder;

					m_pBasePanel->SetDialogVariable( "killername", g_PR->GetPlayerName( m_iKillerIndex ) );
					iKillerTeam = g_PR->GetTeam( m_iKillerIndex );

					m_pAvatar->SetDefaultAvatar( GetDefaultAvatarImage( iBuilder ) );
					m_pAvatar->SetPlayer( !bStreamerMode ? iBuilder : 0 );

					pKiller = pObj->GetBuilder();
				}

				if ( IsPlayerIndex( m_iKillerIndex ) && !g_PR->IsAlive( m_iKillerIndex ) )
				{
				...
				m_pKillerHealth->ShowBuildingBG( true );
			...
			m_pFreezePanelBG->SetImage( VarArgs( "../hud/color_panel_%s", GetTeamSuffix( iKillerTeam, g_aTeamNamesShort, "brown" ) ) );
			m_pAvatar->SetShouldDrawFriendIcon( false );

			const char *pszMedalImage = GetTeamMedalString( iKillerTeam, g_TF_PR->GetPlayerRank( m_iKillerIndex ) );
		...
		if ( IsPlayerIndex( m_iKillerIndex ) && tf2c_show_nemesis_relationships.GetBool() )
		{
			switch ( m_iShowNemesisPanel )
			{
				case SHOW_NO_NEMESIS:
					//If this was just a regular kill but this guy is our nemesis then just show it.
					if ( g_TF_PR->IsPlayerDominating( m_iKillerIndex ) )
					{
...
void CTFFreezePanel::OnThink( void )
{
	...
	{
		if ( tf2c_freezecammodel.GetBool() && !m_pModelBG->IsVisible() && !engine->IsTakingScreenshot() )
		{
...
int	CTFFreezePanel::HudElementKeyInput( int down, ButtonCode_t keynum, const char *pszCurrentBinding )
{
	if ( ShouldDraw() && pszCurrentBinding )
	{
		if ( FStrEq( pszCurrentBinding, "screenshot" ) || FStrEq( pszCurrentBinding, "jpeg" ) )
		{
			...
			bool bHideDuringFreezeCam = hud_freezecamhide.GetBool();
			...
			if ( m_pItemPanel->IsVisible() )
			{
				int w, h;
				m_pItemPanel->GetSize( w, h );
				m_pItemPanel->SetPos( ScreenWidth() - w, ScreenHeight() - h );

				if ( m_pBasePanel )
				{
					int x, y;
					m_pBasePanel->GetPos( x, y );
					m_pBasePanel->SetPos( x, y - ( h + FREEZEPANEL_ITEMPANEL_SEPERATION ) );
				}
...
void CTFFreezePanelCallout::ApplySchemeSettings( vgui::IScheme *pScheme )
{
	...
	m_pCalloutBG = dynamic_cast<CTFImagePanel *>( FindChildByName( "CalloutBG" ) );
}
...
void CTFFreezePanelCallout::SetTeam( int iTeam )
{
	if ( m_pCalloutBG )
	{
