...
void CHudMenuWeaponSet::UpdateLayout()
{
	...
	for (int iSlot = 0; iSlot < INVENTORY_ROWNUM; iSlot++)
	{
		...
		for (int iPreset = 0; iPreset < INVENTORY_COLNUM; iPreset++)
		{
			...
			ETFWeaponID iWeapon = GetTFInventory()->GetWeapon(pClass->GetClassIndex(), (ETFLoadoutSlot)iSlot, iPreset);
			if (iWeapon > 0)
			{
				iCols++;
				if (iCols > iColCount) 
				...
				V_sprintf_safe(szWeaponName, "#%s", pszWeaponName);
				...
				int iWeaponPreset = GetTFInventory()->GetWeaponPreset(filesystem, iClass, (ETFLoadoutSlot)iSlot);
				...
				V_sprintf_safe(szWeaponNumber, "%d %d", iSlot + 1, iPreset + 1);
...
int	CHudMenuWeaponSet::HudElementKeyInput(int down, ButtonCode_t keynum, const char *pszCurrentBinding)
{
	...
	case KEY_9:
	{
		if (m_iSelectedSlot == TF_LOADOUT_SLOT_INVALID)
		{
			int iSlot = keynum - KEY_1;
			SelectSlot((ETFLoadoutSlot)iSlot);
		}
		else
		{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CHudMenuWeaponSet::SelectSlot(ETFLoadoutSlot iSlot)
{
	C_TFPlayer *pPlayer = C_TFPlayer::GetLocalTFPlayer();
	...
	if (pPlayer && GetTFInventory()->CheckValidSlot(iClass, iSlot, true))
	{
		...
		m_pSelectedSlot->SetPos(0, 0 + ((int)m_iSelectedSlot + 0.7) * m_fTall);
	}
	else
	{
		DefaultLayout();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CHudMenuWeaponSet::SelectWeapon(ETFLoadoutSlot iSlot, int iWeapon)
{
	C_TFPlayer *pPlayer = C_TFPlayer::GetLocalTFPlayer();
	int iClass = pPlayer->m_Shared.GetDesiredPlayerClassIndex();
	...
	{
		char szCmd[64];
		V_sprintf_safe(szCmd, "weaponpreset %d %d", iSlot, iWeapon); //; tf2c_weaponset_show 0
