...
{
	// Show custom text in Arena mode.
	if ( TFGameRules() && TFGameRules()->IsInArenaMode() )
	{
		if ( m_pStalemateLabel )
		{
			m_pStalemateLabel->SetText( "#TF_Arena_SuddenDeathPanel" );
		}

		if ( m_pReasonLabel )
		{
			m_pReasonLabel->SetText( "#TF_Arena_SuddenDeathPanelReason" );
		}
		return;
	}
	...
	if ( m_pStalemateLabel )
	{
		...
		{
			m_pStalemateLabel->SetText( "#TF_suddendeath_mode" );
		}
		else
		{
			m_pStalemateLabel->SetText( "#TF_suddendeath" );
		}
	}
