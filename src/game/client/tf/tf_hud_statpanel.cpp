...
BEGIN_DMXELEMENT_UNPACK( RoundStats_t )
	...
	DMXELEMENT_UNPACK_FIELD( "iNumJumppadJumps", "0", int, m_iStat[TFSTAT_JUMPPAD_JUMPS] )
...
TFSTAT_DAMAGE, TFSTAT_DOMINATIONS, TFSTAT_INVULNS, TFSTAT_BUILDINGSDESTROYED, TFSTAT_CAPTURES, TFSTAT_DEFENSES, TFSTAT_REVENGE, TFSTAT_TELEPORTS, TFSTAT_JUMPPAD_JUMPS, TFSTAT_BUILDINGSBUILT,
...
TFStatType_t g_statUnused[] = { TFSTAT_DEATHS, TFSTAT_UNDEFINED, TFSTAT_SHOTS_FIRED, TFSTAT_SHOTS_HIT, TFSTAT_SUICIDES, TFSTAT_ENV_DEATHS, TFSTAT_BONUS_POINTS, TFSTAT_DAMAGE_ASSIST, TFSTAT_DAMAGE_BLOCKED };
...
void CTFStatPanel::UpdateStats( int iMsgType )
{
	...
	if ( iMsgType == STATMSG_PLAYERDEATH || iMsgType == STATMSG_PLAYERRESPAWN || iMsgType == STATMSG_GAMEEND )
	{
	...
	if ( m_statRecord > TFSTAT_UNDEFINED )
	{
		...
		if ( !bAlive || ( gpGlobals->curtime - m_flTimeLastSpawn < 3.0 ) || iMsgType == STATMSG_GAMEEND )
		{
...
void CTFStatPanel::WriteStats( void )
{
	...
	V_sprintf_safe( szFilename, "tf2_playerstats.dmx" );
