...
public:
	...
	virtual void OnWeaponPickup( C_TFWeaponBase *pWeapon );

	virtual void CycleToNextWeapon( void );
	virtual void CycleToPrevWeapon( void );
	virtual void SwitchToLastWeapon( void );

	virtual C_TFWeaponBase *GetWeaponInSlot( int iSlot, int iSlotPos );
	virtual void SelectWeaponSlot( int iSlot );

	virtual C_TFWeaponBase	*GetFirstPos( int iSlot );
	virtual C_TFWeaponBase	*GetNextActivePos( int iSlot, int iSlotPos );

	virtual C_TFWeaponBase	*GetSelectedWeapon( void );
...
private:
	C_TFWeaponBase *FindNextWeaponInWeaponSelection( int iCurrentSlot, int iCurrentPosition );
	C_TFWeaponBase *FindPrevWeaponInWeaponSelection( int iCurrentSlot, int iCurrentPosition );
	...
	virtual	void SetSelectedWeapon( C_TFWeaponBase *pWeapon )
	{
	...
	int m_iActivePanel;
	vgui::IBorder *m_pItemBorders[TF_TEAM_COUNT];
	CHandle< C_TFWeaponBase > m_hActiveWeapon;
};
...
CHudWeaponSelection::CHudWeaponSelection( const char *pElementName ) : CBaseHudWeaponSelection( pElementName ), EditablePanel( NULL, "HudWeaponSelection" )
{
	...
	{
		m_pModelPanels[i] = new CItemModelPanel( this, VarArgs( "modelpanel%i", i ) );
	}
...
//-----------------------------------------------------------------------------
void CHudWeaponSelection::OnWeaponPickup( C_TFWeaponBase *pWeapon )
{
...
void CHudWeaponSelection::OnThink()
{
	...
	for ( int i = 0; i < MAX_WEAPON_SLOTS; i++ )
	{
		int alpha = ( i == m_iActivePanel ) ? m_flSelectionAlphaOverride : m_flAlphaOverride;
		m_pModelPanels[i]->SetNumberSettings( m_flSelectionNumberXPos, m_flSelectionNumberYPos,
			m_hNumberFont, m_NumberColor, alpha );
	}

	if ( hud_fastswitch.GetInt() == HUDTYPE_FASTSWITCH )
	{
		if ( pPlayer->GetActiveTFWeapon() != m_hActiveWeapon )
		{
			m_hActiveWeapon = pPlayer->GetActiveTFWeapon();
			InvalidateLayout();
		}
	}
}
...
bool CHudWeaponSelection::ShouldDrawInternal()
{
	...
	// We only show the demo mode when not using hud_fastswitch.
	bool bFastswitch = hud_fastswitch.GetBool();
	if ( !bFastswitch && ( m_iDemoModeSlot >= 0 || m_flDemoStartTime > 0 ) )
		return true;
	...
	if ( bFastswitch && ( gpGlobals->curtime - m_flSelectionTime ) < ( FASTSWITCH_DISPLAY_TIMEOUT + FASTSWITCH_FADEOUT_TIME ) )
		return true;
...
//-------------------------------------------------------------------------
void CHudWeaponSelection::PerformLayout()
{
	C_TFPlayer *pPlayer = C_TFPlayer::GetLocalTFPlayer();
	if ( !pPlayer )
		return;
	...
	C_TFWeaponBase *pSelectedWeapon = NULL;
	...
	case HUDTYPE_PLUS:
	{
		...
		int boxWideSelected = m_flPlusStyleBoxWide * ( 1.0f + m_flPlusStyleExpandPercent );
		int boxTallSelected = m_flPlusStyleBoxTall * ( 1.0f + m_flPlusStyleExpandPercent );
		int numWeapons = 0;
		...
		for ( int i = 0; i < MAX_WEAPON_SLOTS; ++i )
		{
			...
			int xpos = screenCenterX;
			int ypos = screenCenterY;
			...
			{
				C_TFWeaponBase *pWeapon = GetWeaponInSlot( i, slotPos );
				if ( pWeapon )
				{
					lastSlotPos = slotPos;
				}
			...
			for ( int slotPos = 0; slotPos <= lastSlotPos; ++slotPos )
			{
				...
				int wide = bSelectedWeapon ? boxWideSelected : boxWide;
				int tall = bSelectedWeapon ? boxTallSelected : boxTall;

				// Offset the box position
				xpos += ( wide + 5 ) * xModifiers[i];
				ypos += ( tall + 5 ) * yModifiers[i];
				...
				m_pModelPanels[numWeapons]->SetVisible( true );

				if ( bSelectedWeapon )
				{
					// draw selected weapon
					m_pModelPanels[numWeapons]->SetBounds( x, y, wide, tall );
					m_pModelPanels[numWeapons]->SetBorder( m_pItemBorders[iTeam] );
					m_iActivePanel = numWeapons;
				}
				else
				{
					// draw weapon in slot	
					m_pModelPanels[numWeapons]->SetBounds( x, y, wide, tall );
					m_pModelPanels[numWeapons]->SetBorder( m_pItemBorders[0] );
				}

				m_pModelPanels[numWeapons]->SetItem( pWeapon->GetItem()->GetStaticData(), -1, !pWeapon->HasAnyAmmo() );
				numWeapons++;
			}
		}
	}
	...
	default:
	{
		...
		// draw the bucket set
		// iterate over all the weapon slots
		for ( int i = 0; i < MAX_WEAPON_SLOTS && numWeapons < MAX_WEAPON_SLOTS; i++ )
		{
			bool bHasWeapons = false;
			bool bActiveSlot = false;
			bool bFirstWeapon = true;
			xpos = xStartPos;

			for ( int slotpos = 0; slotpos < MAX_WEAPON_POSITIONS && numWeapons < MAX_WEAPON_SLOTS; slotpos++ )
			{
				C_TFWeaponBase *pWeapon = GetWeaponInSlot( i, slotpos );
				if ( !pWeapon || !pWeapon->VisibleInWeaponSelection() )
					continue;
				...
				{
					xpos -= m_flLargeBoxWide;

					// draw selected weapon
					m_pModelPanels[numWeapons]->SetBounds( xpos, ypos, m_flLargeBoxWide, m_flLargeBoxTall );
					m_pModelPanels[numWeapons]->SetBorder( m_pItemBorders[iTeam] );
					m_pModelPanels[numWeapons]->SetNumberSettings( m_flSelectionNumberXPos, m_flSelectionNumberYPos,
						m_hNumberFont, m_NumberColor, m_flSelectionAlphaOverride );
					bActiveSlot = true;
					m_iActivePanel = numWeapons;
				}
				else
				{
					xpos -= m_flSmallBoxWide;

					// draw weapon in slot	
					m_pModelPanels[numWeapons]->SetBounds( xpos, ypos, m_flSmallBoxWide, m_flSmallBoxTall );
					m_pModelPanels[numWeapons]->SetBorder( m_pItemBorders[0] );
					m_pModelPanels[numWeapons]->SetNumberSettings( m_flSelectionNumberXPos, m_flSelectionNumberYPos,
						m_hNumberFont, m_NumberColor, m_flAlphaOverride );
				}

				// Only show the number if it's the first item in this slot.
				m_pModelPanels[numWeapons]->SetItem( pWeapon->GetItem()->GetStaticData(), bFirstWeapon ? i : -1, !pWeapon->HasAnyAmmo() );
				bFirstWeapon = false;
...
void CHudWeaponSelection::ApplySchemeSettings( vgui::IScheme *pScheme )
{
	...
	m_pItemBorders[0] = pScheme->GetBorder( "TFFatLineBorder" );

	for ( int i = FIRST_GAME_TEAM; i < TF_TEAM_COUNT; i++ )
	{
		m_pItemBorders[i] = pScheme->GetBorder( VarArgs( "TFFatLineBorder%sBG", g_aTeamNames[i] ) );
	}
}
...
//-----------------------------------------------------------------------------
C_TFWeaponBase *CHudWeaponSelection::FindNextWeaponInWeaponSelection( int iCurrentSlot, int iCurrentPosition )
{
	C_TFPlayer *pPlayer = C_TFPlayer::GetLocalTFPlayer();
	if ( !pPlayer )
		return NULL;
	...
	C_TFWeaponBase *pNextWeapon = NULL;
...
//-----------------------------------------------------------------------------
C_TFWeaponBase *CHudWeaponSelection::FindPrevWeaponInWeaponSelection( int iCurrentSlot, int iCurrentPosition )
{
	C_TFPlayer *pPlayer = C_TFPlayer::GetLocalTFPlayer();
	if ( !pPlayer )
		return NULL;
	...
	C_TFWeaponBase *pWeapon;
	C_TFWeaponBase *pPrevWeapon = NULL;
...
void CHudWeaponSelection::CycleToNextWeapon( void )
{
	...
	if ( IsInSelectionMode() )
	{
		...
		C_TFWeaponBase *pWeapon = GetSelectedWeapon();
		if ( !pWeapon )
			return;
...
//-----------------------------------------------------------------------------
C_TFWeaponBase *CHudWeaponSelection::GetWeaponInSlot( int iSlot, int iSlotPos )
{
	C_TFPlayer *pPlayer = C_TFPlayer::GetLocalTFPlayer();
	if ( !pPlayer )
		return NULL;
...
C_TFWeaponBase *CHudWeaponSelection::GetSelectedWeapon( void )
{
	if ( hud_fastswitch.GetInt() == HUDTYPE_BUCKETS && m_iDemoModeSlot >= 0 )
		return GetFirstPos( m_iDemoModeSlot );

	return assert_cast<C_TFWeaponBase *>( m_hSelectedWeapon.Get() );
}
...
void CHudWeaponSelection::PlusTypeFastWeaponSwitch( int iWeaponSlot )
{
	...
	else
	{
		...
		{
			C_TFWeaponBase *pWeapon = GetWeaponInSlot( newSlot, slotPos );
			if ( pWeapon )
			{
				lastSlotPos = slotPos;
			}
	...
	C_TFWeaponBase *pActiveWeapon = pPlayer->GetActiveTFWeapon();
	C_TFWeaponBase *pWeapon = GetWeaponInSlot( m_iSelectedSlot, m_iSelectedBoxPosition );
...
void CHudWeaponSelection::SelectWeaponSlot( int iSlot )
{
	...
	{
		int slotPos = 0;
		C_TFWeaponBase *pActiveWeapon = GetSelectedWeapon();
...
//-----------------------------------------------------------------------------
C_TFWeaponBase *CHudWeaponSelection::GetFirstPos( int iSlot )
{
	int iLowestPosition = MAX_WEAPON_POSITIONS;
	C_TFWeaponBase *pFirstWeapon = NULL;
	...
	{
		C_TFWeaponBase *pWeapon = player->GetTFWeapon( i );
		if ( !pWeapon )
			continue;
...
C_TFWeaponBase *CHudWeaponSelection::GetNextActivePos( int iSlot, int iSlotPos )
{
