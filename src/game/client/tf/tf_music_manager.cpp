...
void CTFMusicManager::StartMusic( void )
{
	...
	for ( int i = 0; i < TF_MUSIC_LAYERS; i++ )
	{
		...
		char szSound[128];
		V_spwintf_safe( szSound, "music/combat_%d/%d.mp3", m_iTrack, i );
...
void CTFMusicManager::StopMusic( bool bPlayEnding /*= false*/ )
{
	...
	if ( bPlayEnding )
	{
		...
		char szSound[128];
		V_sprintf_safe( szSound, pszFormat, m_iTrack );
...
void CHudMusicManager::Paint( void )
{
	...
	wchar_t buf[64];
	V_swprintf_safe( buf, L"Intensity: %.f%%", g_TFMusicManager.m_flIntensity * 100.0f );
