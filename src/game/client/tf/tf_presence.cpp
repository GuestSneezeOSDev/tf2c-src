...
void CTF_Presence::SetupGameProperties( CUtlVector< XUSER_CONTEXT > &contexts, CUtlVector< XUSER_PROPERTY > &properties )
{
	...
	for ( int i = 0; i < properties.Count(); ++i )
	{
		...
		{
		case PROPERTY_FLAG_CAPTURE_LIMIT:
			V_sprintf_safe( buffer, "tf_flag_caps_per_round %d", prop.value.nData );
			engine->ClientCmd( buffer );
			break;

		case PROPERTY_NUMBER_OF_ROUNDS:
			V_sprintf_safe( buffer, "mp_maxrounds %d", prop.value.nData );
			engine->ClientCmd( buffer );
			break;

		case PROPERTY_WIN_LIMIT:
			V_sprintf_safe( buffer, "mp_winlimit %d", prop.value.nData );
			engine->ClientCmd( buffer );
			break;

		case PROPERTY_GAME_SIZE:
			V_sprintf_safe( buffer, "maxplayers %d", prop.value.nData );
			engine->ClientCmd( buffer );
			break;

		case PROPERTY_AUTOBALANCE:
			V_sprintf_safe( buffer, "mp_autoteambalance %d", prop.value.nData );
			engine->ClientCmd( buffer );		
			break;

		case PROPERTY_MAX_GAME_TIME:
			V_sprintf_safe( buffer, "mp_timelimit %d", prop.value.nData );
			engine->ClientCmd( buffer );		
			break;
