...
mpcontrol_t::mpcontrol_t(Panel *parent, char const *panelName)
	: CTFMenuPanelBase(parent, panelName)
{
	type = O_BADTYPE;
	pControl = NULL;
	pPrompt = NULL;
...
void mpcontrol_t::OnTextChanged(Panel *pPanel, const char *pszText)
{
	PostActionSignal(new KeyValues("TextChanged", "text", pszText));
}

void mpcontrol_t::OnControlModified(Panel *pPanel)
{
	PostActionSignal(new KeyValues("ControlModified"));
}

void mpcontrol_t::OnCheckButtonChecked(Panel *pPanel)
{
...
void CScriptObject::SetCurValue(char const *strValue)
{
	...
	if (type == O_NUMBER || type == O_SLIDER || type == O_BOOL)
	{
