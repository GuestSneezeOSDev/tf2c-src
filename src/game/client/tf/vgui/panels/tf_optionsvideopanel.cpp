...
int GetScreenAspectMode( int width, int height )
{
	...
	for ( int i = 0, c = ARRAYSIZE( g_RatioToAspectModes ); i < c; i++ )
	{
...
//-----------------------------------------------------------------------------
void GetResolutionName( vmode_t *mode, char *sz, int sizeofsz, int desktopWidth, int desktopHeight )
{
...
void CTFOptionsVideoPanel::CreateControls()
{
	...
	Label *pTitleVideo = new Label( this, "DescTextTitle", "#GameUI_TitleVideo" );
	...
	switch ( iAspectMode )
	{
		default:
		case 0:
			m_pAspectRatio->SilentActivateItem( iNormalItemID );
			break;
		case 1:
			m_pAspectRatio->SilentActivateItem( i16x9ItemID );
			break;
		case 2:
			m_pAspectRatio->SilentActivateItem( i16x10ItemID );
			break;
	}
...
/*#if defined( USE_SDL ) && defined( DX_TO_GL_ABSTRACTION )
	...
	m_pWindowed = new ComboBox( this, "DisplayModeCombo", 5 + numVideoDisplays, false );
	...
	else
	{
		...
		m_pWindowed->AddItem( "#GameUI_Windowed", NULL );
		m_pWindowed->AddItem( "#GameUI_WindowedBorderless", NULL );
	}
#else*/
...
//#endif
	...
	m_pGammaSlider = new CTFCvarSlider( this, "Gamma", 1.6f, 2.6f, "mat_monitorgamma", true );
	...
	AddControl( m_pGammaSlider, O_SLIDER, "#GameUI_Gamma", "", &m_pGammaLabel );
	...
	for ( int i = 0, c = ARRAYSIZE( g_DirectXLevels ); i < c; i++ )
	{
		...
		if ( !CommandLine()->CheckParm( "-dxlevel" ) &&
			...
			g_DirectXLevels[i] < 95 ) /*pKeyValues->GetInt("ConVar.mat_dxlevel")*/
			continue;

		KeyValues *pTempKV = new KeyValues( "config" );
		if ( g_DirectXLevels[i] == 95 /*pKeyValues->GetInt("ConVar.mat_dxlevel") */
...
//	m_pHDR->SetVisible( mat_dxlevel.GetInt() >= 80 );
	...
	m_pFOVSlider = new CTFCvarSlider( this, "FOV", 75.0f, MAX_FOV, "fov_desired" );

	m_pBloomSlider = new CTFCvarSlider(this, "Bloom Slider", 0.0f, 2.0f, "mat_bloom_scalefactor_scalar", true);

	m_pDXLevel->SetVisible( false );

	m_pColorCorrection->SetVisible(true /*mat_dxlevel.GetInt() >= 90*/);
	m_pMotionBlur->SetVisible(true /*mat_dxlevel.GetInt() >= 90*/ );

	m_pNewLighting = new CTFCvarToggleCheckButton(this, "tf2c_new_lighting", "#GameUI_NewLighting", "tf2c_new_lighting");

	//MarkDefaultSettingsAsRecommended();
	m_bUseChanges = false;

	AddControl( pTitleVideoAdv, O_CATEGORY );
	AddControl( m_pModelDetail, O_LIST, "#GameUI_Model_Detail", "#GameUI_Model_Detail_Hint" );
	AddControl( m_pTextureDetail, O_LIST, "#GameUI_Texture_Detail", "#GameUI_Texture_Detail_Hint" );
	AddControl( m_pShadowDetail, O_LIST, "#GameUI_Shadow_Detail", "#GameUI_Shadow_Detail_Hint" );
	AddControl( m_pWaterDetail, O_LIST, "#GameUI_Water_Detail", "#GameUI_Water_Detail_Hint" );
	AddControl( m_pShaderDetail, O_LIST, "#GameUI_Shader_Detail", "#GameUI_Shader_Detail_Hint" );
	AddControl( m_pColorCorrection, O_LIST, "#GameUI_Color_Correction", "#GameUI_Color_Correction_Hint" );
	AddControl( m_pAntialiasingMode, O_LIST, "#GameUI_Antialiasing_Mode", "#GameUI_Antialiasing_Mode_Hint" );
	AddControl( m_pFilteringMode, O_LIST, "#GameUI_Filtering_Mode", "#GameUI_Filtering_Mode_Hint" );
	AddControl( m_pVSync, O_LIST, "#GameUI_Wait_For_VSync", "#GameUI_Wait_For_VSync_Hint" );
	AddControl( m_pMotionBlur, O_LIST, "#GameUI_MotionBlur", "#GameUI_MotionBlur_Hint" );
	AddControl( m_pFOVSlider, O_SLIDER, "#GameUI_FOV", "#GameUI_FOV_Hint" );
	AddControl( m_pQueuedMode, O_LIST, "#GameUI_MulticoreRendering", "#GameUI_MulticoreRendering_Hint" );
	AddControl( m_pBloomSlider, O_SLIDER, "#GameUI_BloomSlider", "#GameUI_BloomSlider_Hint");
	AddControl( m_pNewLighting, O_BOOL, "", "#GameUI_NewLighting_Hint");
...
//#endif
	...
	m_pAspectRatio->SetVisible( bFoundWidescreen );

	m_nSelectedMode = selectedItemID;

	if ( selectedItemID != -1 )
	{
		m_pMode->SilentActivateItem( selectedItemID );
	}
	else
	{
...
/*#if defined( USE_SDL ) && defined( DX_TO_GL_ABSTRACTION )
    ...
    m_pWindowed->SilentActivateItem( ItemIndex );
#else*/
...
//#endif
	...
	m_pGammaLabel->SetVisible( !config.Windowed() );
...
//	ConVarRef mat_hdr_level( "mat_hdr_level" );
	...
	ConVarRef tf2c_new_lighting("tf2c_new_lighting");

//	ResetDXLevelCombo();

	m_pModelDetail->SilentActivateItem( 2 - clamp( r_rootlod.GetInt(), 0, 2 ) );
	m_pTextureDetail->SilentActivateItem( 2 - clamp( mat_picmip.GetInt(), -1, 2 ) );
	...
	m_pShaderDetail->SilentActivateItem( mat_reducefillrate.GetBool() ? 0 : 1 );
//	m_pHDR->SilentActivateItem( clamp( mat_hdr_level.GetInt(), 0, 2 ) );
	...
	m_pVSync->SilentActivateItem( mat_vsync.GetInt() );

	m_pColorCorrection->SilentActivateItem( mat_colorcorrection.GetInt() );

	m_pMotionBlur->SilentActivateItem( mat_motion_blur_enabled.GetInt() );

	m_pQueuedMode->SilentActivateItem( abs( mat_queue_mode.GetInt() ) );

	m_pFOVSlider->Reset();

	m_pBloomSlider->Reset();

	m_pNewLighting->Reset();

	// get current hardware dx support level
	char dxVer[64];
	GetNameForDXLevel( 95 /*mat_dxlevel.GetInt()*/, dxVer, sizeof(dxVer));
...
void CTFOptionsVideoPanel::ResetDXLevelCombo()
{
	...
	for ( int i = 0; i < m_pDXLevel->GetItemCount(); i++ )
	{
		...
		{
			m_pDXLevel->SilentActivateItem( i );
			break;
		}
	}
...
/*#if defined( USE_SDL )
	...
	{
		guiroot->LaunchInvalidatePanelsLayout();
	}
...
void CTFOptionsVideoPanel::SetComboItemAsRecommended( vgui::ComboBox *combo, int iItem )
{
	...
	wchar_t newText[512];
	V_swprintf_safe( newText, L"%s *", text );
