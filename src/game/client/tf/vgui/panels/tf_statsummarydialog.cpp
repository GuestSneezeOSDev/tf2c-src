...
CTFStatsSummaryDialog::CTFStatsSummaryDialog( vgui::Panel* parent, const char *panelName ) : CTFDialogPanelBase( parent, panelName )
{
	...
	m_pClassComboBox = new vgui::ComboBox( m_pPlayerData, "ClassCombo", TF_CLASS_COUNT_ALL + 1, false );
...
void CTFStatsSummaryDialog::OnCommand( const char *command )
{
	...
	else if ( !V_stricmp( command, "resetstatsbutton" ) )
	{
		engine->ClientCmd_Unrestricted( "resetplayerstats" );
	}
	else
	{
...
void CTFStatsSummaryDialog::UpdateClassDetails()
{
	...
	ClassDetails_t classDetails[] =
	{
		...
		{ TFSTAT_JUMPPAD_JUMPS, MAKEFLAG(TF_CLASS_ENGINEER), "#TF_ClassRecord_MostJumppadJumps" },
	...
	for ( int i = 0; i < ARRAYSIZE( classDetails ); i++ )
	{
		...
		else
		{
			...
			V_swprintf_safe( wzStatNum, L"%d", iMaxVal );
		}
...
void CTFStatsSummaryDialog::SetValueAsClass( const char *pDialogVariable, int iValue, int iPlayerClass )
{
	if ( iValue > 0 )
	{
		...
		wchar_t wzVal[16];
		wchar_t wzMsg[128];
		V_swprintf_safe( wzVal, L"%d", iValue );
...
//-----------------------------------------------------------------------------
const char *CTFStatsSummaryDialog::FormatSeconds( int seconds )
{
