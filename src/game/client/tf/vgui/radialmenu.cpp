...
//--------------------------------------------------------------------------------------------------------
static char s_radialMenuName[ 64 ];
static bool s_mouseMenuKeyHeld;
...
//--------------------------------------------------------------------------------------------------------------
CRadialMenu::CRadialMenu(const char *pElementName) : CHudElement(pElementName), BaseClass(NULL, PANEL_RADIAL_MENU)
{
	MEM_ALLOC_CREDIT();
...
bool IsRadialMenuOpen( const char *menuName, bool includeFadingOut )
{
	...
	if ( menuName == NULL || FStrEq( s_radialMenuName, menuName ) )
	{
...
//		return;
	...
	if ( FStrEq( s_radialMenuName, menuName ) )
	{
	...
	V_sprintf_safe( s_radialMenuName, menuName );
...
{
	if ( s_mouseMenuKeyHeld )
		return;

	if (!C_TFPlayer::GetLocalTFPlayer())
		return;

	s_mouseMenuKeyHeld = true;
	radialmenu( args );
}
