...
void RoundInfoOverlay::Update( const char *szMapName )
{
	...
	V_strcpy_safe( strFullpath, "resource/roundinfo/" );	// Assume we must play out of the media directory
	V_strcat_safe( strFullpath, szMapName );
...
void CTFRoundInfo::ShowPanel( bool bShow )
{
	...
	if ( bShow )
	{
		...
		char temp[255];
		V_sprintf_safe( temp, "VGUI/%s", m_szMapImage );
...
void CTFRoundInfo::UpdateImage( ImagePanel *pImagePanel, const char *pszImageName )
{
	...
	{
		char szTemp[255];
		V_sprintf_safe( szTemp, "VGUI/%s", pszImageName );
...
void CTFRoundInfo::Update()
{
	...
	{
		char temp[255];
		V_sprintf_safe( temp, "../overviews/%s", szMapName );
		V_strcpy_safe( m_szMapImage, temp );
