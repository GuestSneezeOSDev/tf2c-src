...
CTFStatsSummaryPanel::CTFStatsSummaryPanel() : vgui::EditablePanel( NULL, "TFStatsSummary",
	vgui::scheme()->LoadSchemeFromFile( ClientSchemesArray[SCHEME_CLIENT_PATHSTRING], ClientSchemesArray[SCHEME_CLIENT_STRING] ) )
{
	...
	m_pMapInfo = new vgui::EditablePanel( this, "MapInfo" );
	...
	m_pResetButton = new vgui::Button( this, "ResetStatsButton", "" );
#endif
...
#ifndef _X360
	int x, y, wide, tall;
	m_pTipText->GetBounds( x, y, wide, tall );

	// Put "Next tip" button below the tip.
	m_pNextTipButton->SizeToContents();
	m_pResetButton->SizeToContents();
#endif
}
...
void CTFStatsSummaryPanel::UpdateClassDetails()
{
	...
	ClassDetails_t classDetails[] =
	{
		...
		{ TFSTAT_JUMPPAD_JUMPS, MAKEFLAG(TF_CLASS_ENGINEER), "#TF_ClassRecord_MostJumppadJumps" },	// Not sure if this res ever gets called, will ignore further work
	};
	...
	for ( int i = 0; i < ARRAYSIZE( classDetails ); i++ )
	{
		...
		if ( m_iSelectedClass == TF_CLASS_UNDEFINED )
		{
			...
			for ( int j = 0; j < m_aClassStats.Count(); j++ )
			{
				...
				if ( m_aClassStats[j].iPlayerClass > TF_LAST_NORMAL_CLASS )
					continue;
...
const char *FormatSeconds( int seconds )
{
	...
	if ( hours > 0 )
	{
		Q_snprintf( string, sizeof( string ), "%i:%02i:%02i", hours, minutes, seconds );
	}
	else
	{
...
{
	// Put special classes at the end.
	if ( pStats0->iPlayerClass > TF_LAST_NORMAL_CLASS && pStats1->iPlayerClass <= TF_LAST_NORMAL_CLASS )
		return 1;
...
void CTFStatsSummaryPanel::FireGameEvent( IGameEvent *event )
{
	...
	if ( !m_bInteractive )
	{
		// When we're changing levels.
		if ( !Q_strcmp( event->GetName(), "server_spawn" ) )
		{
