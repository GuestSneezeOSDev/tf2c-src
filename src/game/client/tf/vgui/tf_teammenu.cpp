...
bool CTFTeamButton::IsTeamFull()
{
	return ( m_iTeam > TEAM_UNASSIGNED &&
		( TFGameRules() && TFGameRules()->WouldChangeUnbalanceTeams( m_iTeam, GetLocalPlayerTeam() ) ) );
}
...
void CTFTeamButton::SetMouseEnteredState( bool bState )
{
	if ( bState )
	{
...
//-----------------------------------------------------------------------------
// Purpose: Arena team select
//-----------------------------------------------------------------------------
CTFArenaTeamMenu::CTFArenaTeamMenu( IViewPort *pViewPort, const char *pName ) : BaseClass( pViewPort, pName )
{
}
...
{
	// Skipping CTFTeamMenu since because of the hack below.
	BaseClass::BaseClass::ApplySchemeSettings( pScheme );

	// Terrible hack to make it so RES file is parsed properly.
	SetName( PANEL_TEAM );
	LoadControlSettings( "Resource/UI/HudArenaTeamMenu.res" );
	SetName( PANEL_ARENA_TEAM );
}


void CTFArenaTeamMenu::CreateTeamButtons( void )
{
	m_pTeamButtons[TEAM_UNASSIGNED] = new CTFTeamButton( this, "teambutton2" );
	m_pTeamButtons[TEAM_SPECTATOR] = new CTFTeamButton( this, "teambutton3" );
}
