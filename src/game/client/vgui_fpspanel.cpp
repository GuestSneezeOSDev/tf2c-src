...
static ConVar cl_showfps( "cl_showfps", "0", FCVAR_NONE, "Draw fps meter at top of screen (1 = fps, 2 = smooth fps)" );
static ConVar cl_showpos( "cl_showpos", "0", FCVAR_NONE, "Draw current position at top of screen" );
static ConVar cl_showbattery( "cl_showbattery", "0", FCVAR_NONE, "Draw current battery level at top of screen when on battery power" );
...
// Was used in CFPSPanel::ComputeSize for its Width
...
void CFPSPanel::ComputeSize( void )
{
	...
	int y = 4;
	SetPos( x, y );
	SetSize( wide, 4 * vgui::surface()->GetFontTall( m_hFont ) + 8 );
}
