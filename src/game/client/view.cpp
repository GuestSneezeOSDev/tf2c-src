...
#if defined( TF_CLASSIC_CLIENT )
ConVar v_viewmodel_fov( "viewmodel_fov", "54", FCVAR_ARCHIVE | FCVAR_USERINFO, "Sets the field-of-view for the viewmodel.", true, 0.1, true, 179.9 );
...
void CViewRender::Init( void )
{
	...
	materials->CreateNamedRenderTargetTextureEx(
		CBA_SCRATCH_BUFFER, 
		iW, iH, RT_SIZE_NO_CHANGE,
		GetFullFrameFrameBufferTexture(0)->GetImageFormat(),
...
void CViewRender::OnRenderStart()
{
	...
	{
#ifdef TF_CLASSIC_CLIENT
		default_fov.SetValue( player->GetDefaultFOV() );
#else
		...
		if ( gHUD.m_flMouseSensitivityFactor )
		{
...
#ifdef TF_CLASSIC_CLIENT
			// Include FOV for tranquilized zoomed players
			if ( localFOV != iDefaultFOV )
			{
				...
				gHUD.m_flMouseSensitivity = gHUD.m_flFOVSensitivityAdjust * sensitivity.GetFloat() * gHUD.m_flMouseSensitivityFactor;
			}
#endif
		}
		else
...
#ifdef TF_CLASSIC_CLIENT
	view.fovViewmodel = clamp( g_pClientMode->GetViewModelFOV() - flFOVOffset, 5.0f, 179.0f );
#else
    ...
    for( StereoEye_t eEye = GetFirstEye(); eEye <= GetLastEye(); eEye = (StereoEye_t)(eEye+1) )
	{
	    ...
	    else if ( IsPosix() )
	    {
		    ...
		    if ( adapterInfo.m_VendorID == 0x1002 )
		    {
			    ...
			    {
				    nClearFlags |= VIEW_CLEAR_COLOR;
					// This might be messing up my colourblind effect?
			    }
		    }
	    }
