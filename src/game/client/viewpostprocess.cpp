...
#ifdef TF_CLASSIC_CLIENT
ConVar tf2c_desaturate("tf2c_desaturate", "0", FCVAR_CHEAT);
ConVar tf2c_desaturate_amount("tf2c_desaturate_amount", "0.0", FCVAR_CHEAT);
																...
																bool bPerformSoftwareAA, bool bPerformBloom, bool bPerformColCorrect, float flAAStrength, float flBloomAmount )
{
	...
	// Assert( bPerformSoftwareAA || bPerformBloom || bPerformColCorrect ); // SHUT UP!!!!!!! GOD DAMN!!!! WHY WOULD HAVE AN ASSERT THAT FLOODS MY CONSOLE EVERY FRAME JASOn
...
static float GetBloomAmount( void )
{
	...
	bool bBloomEnabled = true;/*(mat_hdr_level.GetInt() >= 1);*/
...
#ifdef TF_CLASSIC_CLIENT
			C_TFPlayer *pLocalTFPlayer = GetLocalObservedPlayer( true );
			bool bPerformDesaturation = tf2c_desaturate.GetBool() || ( pLocalTFPlayer && pLocalTFPlayer->GetDesaturationAmount() != 0.0f );
			bool bPerformCBA = tf2c_colorblind_pattern_enable.GetBool();
			if ( bPerformBloom || bPerformSoftwareAA || bPerformColCorrect || bPerformDesaturation || bPerformCBA)
#else
...
#ifdef TF_CLASSIC_CLIENT
#ifdef COLOURBLINDBEFOREDESATURATION
...
#endif
					// Desaturation
					float flDesaturationAmount;
					if ( tf2c_desaturate.GetBool() )
					{
						flDesaturationAmount = tf2c_desaturate_amount.GetFloat();
					}
					else if ( pLocalTFPlayer )
					{
						flDesaturationAmount = pLocalTFPlayer->GetDesaturationAmount();
					}
					else
					{
						flDesaturationAmount = 0.0f;
					}

					if ( flDesaturationAmount != 0.0f )
					{
						// See above.
						if ( bFBUpdated )
						{
						...
						static IMaterial *pMat = materials->FindMaterial( "dev/desaturate", TEXTURE_GROUP_OTHER );
						static unsigned int nDesaturateAmountCache = 0;
						IMaterialVar *pAmountVar = pMat->FindVarFast( "$amount", &nDesaturateAmountCache );
						pAmountVar->SetFloatValue( flDesaturationAmount );
						pRenderContext->DrawScreenSpaceRectangle( pMat, 0, 0, w, h, 
																  0, 0, w - 1, h - 1,
																  w, h );
					}
#ifndef COLOURBLINDBEFOREDESATURATION
					if (tf2c_colorblind_pattern_enable.GetBool())
					{
						if (bFBUpdated)
						{
