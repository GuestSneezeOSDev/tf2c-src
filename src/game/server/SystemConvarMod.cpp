...
ConVar sv_allow_gameconvarmod( "sv_allow_gameconvarmod", "mapcycle", FCVAR_NONE, "Allow use of game_convar_mod entity in map. Potentially dangerous for untrusted maps.\n"
																				"  disallow : Always disallow\n"
																				"  mapcycle : Allowed for maps in mapcycle\n"
																				"  always   : Allow for all maps");

bool IsAllowedByServer( void )
{
	const char *pNewValue = sv_allow_gameconvarmod.GetString();
	if ( !V_strcasecmp( pNewValue, "mapcycle" ) )
	{
		if ( TFGameRules() )
		{
			const char *pszMapName = STRING( gpGlobals->mapname );
			if ( pszMapName )
				return TFGameRules()->IsMapInMapCycle( pszMapName );
		}
	}

	return !V_strcasecmp( pNewValue, "always" ) ? true : false;
}
...
#define MAX_CONVARMOD_STRING_SIZE 1024
...
void CVEnt_Activate(CMapbaseCVarModEntity *modent)
{
	...
	{
		Msg("Detected game_convar_mod: Not allowed by server. Not activating.");
		return;
	}

	Msg("Detected game_convar_mod: Allowed by server. Activating...");
