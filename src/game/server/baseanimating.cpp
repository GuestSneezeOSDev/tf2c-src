...
BEGIN_ENT_SCRIPTDESC( CBaseAnimating, CBaseEntity, "Animating models" )
	...
	DEFINE_SCRIPTFUNC_NAMED( ScriptLookupPoseParameter, "LookupPoseParameter", "Looks up a pose parameter index by name" )
	...
	DEFINE_SCRIPTFUNC_NAMED( ScriptSequenceDuration, "GetSequenceDuration", "Get the specified sequence duration" )
	...
	DEFINE_SCRIPTFUNC( SetModelScale, "(scale, change_duration) Changes a model's scale over time" )
	DEFINE_SCRIPTFUNC( IsSequenceFinished, "Ask whether the main sequence is done playing" )
...
const QAngle &CBaseAnimating::ScriptGetAttachmentAngles( int iAttachment )
{
	...
	CBaseAnimating::GetAttachment( iAttachment, absOrigin, absAngles );
...
int CBaseAnimating::ScriptLookupPoseParameter( const char *szName )
{
	CStudioHdr *pHdr = GetModelPtr();
	if ( pHdr == NULL )
		return 0;

	return LookupPoseParameter( pHdr, szName );
}

float CBaseAnimating::ScriptGetPoseParameter( int iPoseParam )
{
	CStudioHdr *pHdr = GetModelPtr();
	if ( pHdr == NULL )
		return 0.0f;
...
void CBaseAnimating::ScriptSetPoseParameter( int iPoseParam, float fValue )
{
	CStudioHdr *pHdr = GetModelPtr();
	if ( pHdr == NULL )
		return;
...
Vector CBaseAnimating::ScriptGetBoneOrigin( int iBone )
{
	...
	GetBonePosition( iBone, origin, angles );
...
void CBaseAnimating::ScriptSetModel( char const *pszModel )
{
	if ( pszModel && pszModel[0] )
	{
		...
		{
			char const *pszSequence = GetSequenceName( GetSequence() );
			
			SetModelName( AllocPooledString( pszModel ) );
			SetModel( pszModel );

			int nSequence = LookupSequence( pszSequence );
			if ( nSequence != -1 )
			{
...
void CBaseAnimating::ScriptDispatchAnimEvents( HSCRIPT hAnimating )
{
	CBaseAnimating *pAnimator = NULL;
	if ( hAnimating )
		pAnimator = HScriptToClass<CBaseAnimating>( hAnimating );

	DispatchAnimEvents( pAnimator );
}


void CBaseAnimating::ScriptHookOnServerRagdoll( CRagdollProp *pRagdoll, bool bSubModel )
{
	if ( m_ScriptScope.IsInitialized() )
	{
		...
		if ( m_hOnServerRagdoll != NULL )
		{
			m_ScriptScope.Call( m_hOnServerRagdoll, NULL, pRagdoll->GetScriptInstance(), bSubModel );
		}
	}
}
