...
public:
	...
	DECLARE_DATADESC();
	DECLARE_SERVERCLASS();
	DECLARE_ENT_SCRIPTDESC( CBaseAnimating );
...
public:
	...
	int		ScriptLookupPoseParameter( const char *szName );
	float	ScriptGetPoseParameter( int iPoseParam );
	void	ScriptSetPoseParameter( int iPoseParam, float fValue );
	...
	void	ScriptSetModel( char const *pszModel );
	void	ScriptDispatchAnimEvents( HSCRIPT hAnimating );

	void	ScriptHookOnServerRagdoll( CRagdollProp *pRagdoll, bool bSubModel );
private:
