...
{
#ifdef NEXT_BOT
	// In live TF2, when a player goes "off-mesh", e.g. climbs onto a prop where no nav areas were generated, that
	// player will retain their old last known area indefinitely, so long as they remain "off-mesh". This is extremely
	// problematic for NextBot/TFBot, because various aspects of that codebase implicitly assume that calling
	// GetLastKnownArea on a player will EITHER return a VALID AND ACCURATE nav area pointer, or nullptr. But the
	// behavior in live TF2 means that GetLastKnownArea will sometimes return a VALID YET INACCURATE nav area pointer.
	// Functions such as IVision::IsAbleToSee will then make decisions about important things (such as whether an enemy
	// is visible to a bot) based on invalid/outdated information about where the relevant entity is.
	// 
	// The solution to this difficulty is to call ClearLastKnownArea in cases where we attempt to update our last known
	// area but discover that we are "off-mesh", rather than just returning and leaving the previous last known area
	// untouched. This way, callers to GetLastKnownArea will actually get nullptr (which they are generally capable of
	// dealing with in a reasonable manner) instead of an apparently-correct-but-actually-completely-wrong nav area ptr.
	// 
	// This modification *might* cause problems with some bot code; it's hard to anticipate. We'll need to keep an eye
	// on that.
	// 
	// This is a virtual function, and we could hypothetically override it in CTFPlayer. Unfortunately, due to how this
	// function works, we'd basically have to duplicate the entire function body in the overridden implementation and
	// modify just a few lines of it. So given the choice between that option, and making changes to base code, I'm
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
HSCRIPT CBaseCombatCharacter::ScriptGetLastKnownArea()
{
	return m_lastNavArea ? m_lastNavArea->GetScriptInstance() : NULL;
}
