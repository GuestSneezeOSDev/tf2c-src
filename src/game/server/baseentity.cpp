...
BEGIN_ENT_SCRIPTDESC_ROOT( CBaseEntity, "Root class of all server-side entities" )
	...
	DEFINE_SCRIPTFUNC_NAMED( ScriptDisableDraw, "DisableDraw", "" )
	DEFINE_SCRIPTFUNC_NAMED( ScriptEnableDraw, "EnableDraw", "" )
	...
	DEFINE_SCRIPTFUNC_NAMED( GetAbsOrigin, "GetOrigin", SCRIPT_HIDE )
	DEFINE_SCRIPTFUNC( GetAbsOrigin, "GetAbsOrigin" )
	...
	DEFINE_SCRIPTFUNC_NAMED( GetAbsVelocity, "GetVelocity", SCRIPT_HIDE )
	DEFINE_SCRIPTFUNC( SetAbsVelocity, "Sets the current absolute velocity of the entity" )
	DEFINE_SCRIPTFUNC_NAMED( SetAbsVelocity, "SetVelocity", SCRIPT_HIDE )
	...
	DEFINE_SCRIPTFUNC_NAMED( ScriptSpawn, "DispatchSpawn", "Spawns the entity into the game." )
	...
	DEFINE_SCRIPTFUNC( KeyValueFromInt, "Executes KeyValue with a integer" )
	...
	DEFINE_SCRIPTFUNC_NAMED( ScriptTakeDamage, "TakeDamage", "" )
	DEFINE_SCRIPTFUNC_NAMED( ScriptTakeDamageParams, "TakeDamageEx", "" )
...
int CBaseEntity::TakeDamage( const CTakeDamageInfo &inputInfo )
{
	...
	else
	{
		...
		// Compat with Live, perform script hooks outside of our system
		if ( ScriptHookEnabled( "OnTakeDamage" ) )
		{
			ScriptVariant_t table;
			g_pScriptVM->CreateTable( table );

			g_pScriptVM->SetValue( table, "const_entity", this->GetScriptInstance() );
			g_pScriptVM->SetValue( table, "inflictor", ToHScript( info.GetInflictor() ) );
			g_pScriptVM->SetValue( table, "weapon", ToHScript( info.GetWeapon() ) );
			g_pScriptVM->SetValue( table, "attacker", ToHScript( info.GetAttacker() ) );
			g_pScriptVM->SetValue( table, "damage", info.GetDamage() );
			g_pScriptVM->SetValue( table, "max_damage", info.GetMaxDamage() );
			g_pScriptVM->SetValue( table, "damage_bonus", info.GetDamageBonus() );
			g_pScriptVM->SetValue( table, "damage_bonus_provider", ToHScript( info.GetDamageBonusProvider() ) );
			g_pScriptVM->SetValue( table, "const_base_damage", info.GetBaseDamage() );
			g_pScriptVM->SetValue( table, "damage_force", info.GetDamageForce() );
			g_pScriptVM->SetValue( table, "damage_for_force_calc", info.GetDamageForForceCalc() );
			g_pScriptVM->SetValue( table, "damage_position", info.GetDamagePosition() );
			g_pScriptVM->SetValue( table, "reported_position", info.GetReportedPosition() );
			g_pScriptVM->SetValue( table, "damage_type", info.GetDamageType() );
			g_pScriptVM->SetValue( table, "damage_custom", info.GetDamageCustom() );
			g_pScriptVM->SetValue( table, "damage_stats", info.GetDamageStats() );
			g_pScriptVM->SetValue( table, "force_friendly_fire", info.IsForceFriendlyFire() );
			g_pScriptVM->SetValue( table, "ammo_type", info.GetAmmoType() );
			g_pScriptVM->SetValue( table, "player_penetration_count", info.GetPlayerPenetrationCount() );
			g_pScriptVM->SetValue( table, "damaged_other_players", info.GetDamagedOtherPlayers() );
			g_pScriptVM->SetValue( table, "crit_type", info.GetCritType() );
			g_pScriptVM->SetValue( table, "early_out", false );

			if ( !RunScriptHook( "OnTakeDamage", table ) )
				return OnTakeDamage( info );

			ScriptVariant_t value;

			g_pScriptVM->GetValue( table, "inflictor", &value );
			info.SetInflictor( ToEnt( value.Get<HSCRIPT>() ) );

			g_pScriptVM->GetValue( table, "weapon", &value );
			info.SetWeapon( ToEnt( value.Get<HSCRIPT>() ) );

			g_pScriptVM->GetValue( table, "attacker", &value );
			info.SetAttacker( ToEnt( value.Get<HSCRIPT>() ) );

			g_pScriptVM->GetValue( table, "damage", &value );
			info.SetDamage( value.Get<float>() );

			g_pScriptVM->GetValue( table, "max_damage", &value );
			info.SetMaxDamage( value.Get<float>() );

			g_pScriptVM->GetValue( table, "damage_bonus", &value );
			info.SetDamageBonus( value.Get<float>() );

			g_pScriptVM->GetValue( table, "damage_bonus_provider", &value );
			info.SetDamageBonusProvider( ToEnt( value.Get<HSCRIPT>() ) );

			g_pScriptVM->GetValue( table, "damage_force", &value );
			info.SetDamageForce( value.Get<Vector>() );

			g_pScriptVM->GetValue( table, "damage_for_force_calc", &value );
			info.SetDamageForForceCalc( value.Get<float>() );

			g_pScriptVM->GetValue( table, "damage_position", &value );
			info.SetDamagePosition( value.Get<Vector>() );

			g_pScriptVM->GetValue( table, "reported_position", &value );
			info.SetReportedPosition( value.Get<Vector>() );

			g_pScriptVM->GetValue( table, "damage_type", &value );
			info.SetDamageType( value.Get<int>() );

			g_pScriptVM->GetValue( table, "damage_custom", &value );
			info.SetDamageCustom( value.Get<int>() );

			g_pScriptVM->GetValue( table, "damage_stats", &value );
			info.SetDamageStats( value.Get<int>() );

			g_pScriptVM->GetValue( table, "force_friendly_fire", &value );
			info.SetForceFriendlyFire( value.Get<bool>() );

			g_pScriptVM->GetValue( table, "ammo_type", &value );
			info.SetAmmoType( value.Get<int>() );

			g_pScriptVM->GetValue( table, "player_penetration_count", &value );
			info.SetPlayerPenetrationCount( value.Get<int>() );

			g_pScriptVM->GetValue( table, "damaged_other_players", &value );
			info.SetDamagedOtherPlayers( value.Get<int>() );

			g_pScriptVM->GetValue( table, "crit_type", &value );
			info.SetCritType( value.Get<int>() );

			g_pScriptVM->GetValue( table, "early_out", &value );
			if( value.Get<bool>() )
				return 0;
		}
...
//-----------------------------------------------------------------------------
int CBaseEntity::ScriptTakeDamage( float flDamage, int nDamageType, HSCRIPT hAttacker )
{
	CBaseEntity *pAttacker = ToEnt( hAttacker );
	CTakeDamageInfo info( this, pAttacker, flDamage, nDamageType );
...
void CBaseEntity::ScriptInputKill()
{
	inputdata_t empty;
	InputKill( empty );
}
...
#ifdef TF_CLASSIC
//------------------------------------------------------------------------------
// Purpose: Input handler for setting move type e.g. for parenting vehicles
//------------------------------------------------------------------------------
void CBaseEntity::InputSetMoveType( inputdata_t &inputdata )
{
	SetMoveType( static_cast<MoveType_t>( inputdata.value.Int() ) );
}
#endif
...
void CC_Ent_SetName( const CCommand& args )
{
	...
	else
	{
		...
		{
			// Otherwise set bits based on name or classname
			pEntity = gEntList.FindEntityGeneric( NULL, args[2], UTIL_GetCommandClient() );
		}
	...
	virtual void CommandCallback( const CCommand &command )
	{
		...
		else
		{
			...
			if ( !sv_allow_ent_fire.GetBool() )
			{
				...
				{
					// Only allow the host to issue ent_fire on listen servers.
...
void CBaseEntity::InputRunScript( inputdata_t &inputdata )
{
	...
	{
		RunScript(szQuotableCode, "InputRunScript");
	}
	else
	{
