...
void CFlexCycler::Think( void )
{
	...
	if (GetNumFlexControllers() > 2)
	{
		...
		else if ( pszExpression && (pszExpression[0] == '^') && (pszExpression[1] == '\0') ) // ^ for sine wave
		{
			for ( LocalFlexController_t i = LocalFlexController_t(0); i < GetNumFlexControllers(); i++ )
			{
                int32 numflexcontrollers = GetNumFlexControllers();
				// Throw a differently offset sine wave on all of the flex controllers
				float fFlexTime = static_cast<int>(i) * (1.0f / numflexcontrollers) + gpGlobals->curtime;
		...
		{
            int32 flexControllers = static_cast<int32>(GetNumFlexControllers());
			// m_flextime = gpGlobals->curtime + 1.0; // RandomFloat( 0.1, 0.5 );
			m_flextime = gpGlobals->curtime + random->RandomFloat( 0.3, 0.5 ) * (30.0 / flexControllers);
			m_flexnum = (LocalFlexController_t)random->RandomInt( 0, flexControllers - 1 );
