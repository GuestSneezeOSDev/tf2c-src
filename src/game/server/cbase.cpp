...
CEventAction::CEventAction( const char *ActionData )
{
	...
	char szToken[256];
	char sep = strchr( ActionData, '' ) == NULL ? ',' : '';

	//
	// Parse the target name.
	//
	const char *psz = nexttoken(szToken, ActionData, sep);
	if (szToken[0] != '\0')
	{
...
//-----------------------------------------------------------------------------
SERVERONLY_DLL_EXPORT void CBaseEntityOutput::FireOutput(variant_t Value, CBaseEntity *pActivator, CBaseEntity *pCaller, float fDelay)
{
	// DevMsg("[SM] ->CBaseEntityOutput::FireOutput<-\n");
	//
...
void CBaseEntityOutput::ScriptRemoveEventAction( CEventAction *pEventAction, const char *pszTargetName, const char *pszInputName, const char *pszParameter )
{
	CEventAction *pAction = GetFirstAction();
	CEventAction *pPrevAction = NULL;
	if ( pszInputName && pszInputName[0] )
	{
		string_t iTarget = AllocPooledString( pszTargetName );
		string_t iInput = AllocPooledString( pszInputName );
		string_t iParameter = AllocPooledString( pszParameter );
		while ( pAction )
		{
			if ( pAction->m_iTarget == iTarget && pAction->m_iTargetInput == iInput && pAction->m_iParameter == iParameter )
			{
				if ( pPrevAction )
				{
					pPrevAction->m_pNext = pAction->m_pNext;
				...
				CEventAction *pNext = pAction->m_pNext;
				g_EntityListPool.Free( pAction );
				pAction = pNext;
			}
			else
			{
