...
#if defined( TF_DLL ) || defined ( TF_CLASSIC )
ConVar sv_quota_stringcmdspersecond( "sv_quota_stringcmdspersecond", "16", FCVAR_NONE, "How many string commands per second clients are allowed to submit before getting kicked, 0 to disallow all string commands." );

ConVar sv_spew_stringcmds( "sv_spew_stringcmds", "0", FCVAR_NONE, "Print all string commands from all clients. Useful for debugging sv_quota_stringcmdspersecond." );
#endif
...
{
	// Is the client valid?
	// Warning: if disconecting clients find a way to spam stringcmds somehow,
	// tighten this up!
	// -sappho
	if ( !pPlayer || !pPlayer->IsConnected() || pPlayer->IsDisconnecting() )
	{
		if ( sv_spew_stringcmds.GetBool() )
		{
			Warning( "Someone [pPlayer=%p] sent some weird command='%s'\n", pPlayer, args[0] );
		}
		return;
	}
	...
	if ( sv_spew_stringcmds.GetBool() )
	{
		Warning( "pPlayer->GetUserID                 = %i\n", pPlayer->GetUserID() );
		Warning( "pCmd                               = %s\n", pCmd);
		Warning( "args.GetCommandString()            = %s\n", args.GetCommandString() );
		Warning( "args.ArgC() [total args]           = %i\n", args.ArgC() );
		Warning( "pPlayer->m_iStringcmds             = %i\n", pPlayer->m_iStringcmds );
		Warning( "pPlayer->m_fStringcmdsResetTime    = %f\n", pPlayer->m_fStringcmdsResetTime );
		Warning( "gpGlobals->curtime                 = %f\n", gpGlobals->curtime );
	}
...
#endif
	{
		...
		// We are testing validity here so that we can safely ignore setitempreset commands from contributing to our stringcmd spam limiter
		// ^ note from future sappho: we probably don't need to, since net_chan_proctime_limit_ms should theoretically take care of this...?
		if ( FStrEq(pCmd, "setitempreset") )
		{
			/*
			if (args.ArgC() != 4)
			{
				Warning("Invalid loadout preset -> args %i!\n", args.ArgC());
				const char* kickcmd = UTIL_VarArgs("kickid %i %s", pPlayer->GetUserID(), "Invalid loadout preset\n");
				engine->ServerCommand(kickcmd);
				return;
			}

			int playerclass = V_atoi(args[1]);
			int loadoutslot = V_atoi(args[2]);
			int itempreset  = V_atoi(args[3]);

			if
			(
				// class
				   playerclass <  TF_FIRST_NORMAL_CLASS
				|| playerclass >= TF_CLASS_COUNT_ALL
				// slot
				|| loadoutslot <  TF_LOADOUT_SLOT_PRIMARY
				|| loadoutslot >= TF_LOADOUT_SLOT_COUNT
				// item
				|| itempreset  < 0
			)
			{
				Warning("Invalid loadout preset -> %i %i %i\n", playerclass, loadoutslot, itempreset);
				const char* kickcmd = UTIL_VarArgs("kickid %i %s", pPlayer->GetUserID(), "Invalid loadout preset\n");
				engine->ServerCommand(kickcmd);
				return;
			}
		...
		else if (pPlayer->m_fStringcmdsResetTime < gpGlobals->curtime)
		{
		...
		else
		{
			...
			{
				const char* kickcmd = UTIL_VarArgs("kickid %d %s", pPlayer->GetUserID(), "Too many string cmds per second\n");
				engine->ServerCommand(kickcmd);
