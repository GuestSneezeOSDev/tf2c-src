...
#ifdef TF_CLASSIC
	// Injects paths into the filesystem of the engine.
	ApplyFilesystemModifications();
#endif
...
SERVERONLY_DLL_EXPORT void CServerGameDLL::LevelShutdown( void )
{
#ifndef NO_STEAM
...
// implemented purely so we can control whether server hibernation is allowed
class CTF2CGCLobby : public IServerGCLobby
{
public:
	virtual bool HasLobby() const override                                       { return false; }
	virtual bool SteamIDAllowedToConnect( const CSteamID& steamID ) const override { return true; }
	virtual void UpdateServerDetails() override                                  {  }
	
	virtual bool ShouldHibernate() override
	{
		return ( engine->IsDedicatedServer() && tf_allow_server_hibernation.GetBool() );
	}
};
CTF2CGCLobby g_GCLobby;
...
#define QUICKGETCVARVALUE(v) (engine->GetClientConVarValue( player->entindex(), v ))
	...
	// Why? So cheats can't abuse it, duh
	// -sappho
	#ifdef DEBUG
	...
	int dropped_packets, bool ignore, bool paused )
{
	...
	// Too many commands?
	// ftr, numcmds > totalcmds is another fix for seq freezing that was reversed from LTF2 -sappho
	if ( totalcmds < 0 || totalcmds >= ( CMD_MAXBACKUP - 1 ) || numcmds > totalcmds )
	{
