...
BEGIN_SCRIPTENUM( ENavCornerType, "" )
	DEFINE_ENUMCONST( NORTH_WEST, "" )
	DEFINE_ENUMCONST( NORTH_EAST, "" )
	DEFINE_ENUMCONST( SOUTH_EAST, "" )
	DEFINE_ENUMCONST( SOUTH_WEST, "" )
	DEFINE_ENUMCONST( NUM_CORNERS, "" )
END_SCRIPTENUM()

BEGIN_SCRIPTENUM( ENavDirType, "" )
	DEFINE_ENUMCONST( NORTH, "" )
	DEFINE_ENUMCONST( EAST, "" )
	DEFINE_ENUMCONST( SOUTH, "" )
	DEFINE_ENUMCONST( WEST, "" )
	DEFINE_ENUMCONST( NUM_DIRECTIONS, "" )
END_SCRIPTENUM()

BEGIN_SCRIPTENUM( ENavRelativeDirType, "" )
	DEFINE_ENUMCONST( FORWARD, "" )
	DEFINE_ENUMCONST( RIGHT, "" )
	DEFINE_ENUMCONST( BACKWARD, "" )
	DEFINE_ENUMCONST( LEFT, "" )
	DEFINE_ENUMCONST( UP, "" )
	DEFINE_ENUMCONST( DOWN, "" )
	DEFINE_ENUMCONST( NUM_RELATIVE_DIRECTIONS, "" )
END_SCRIPTENUM()

BEGIN_SCRIPTENUM( ENavTraverseType, "" )
	DEFINE_ENUMCONST( GO_NORTH, "" )
	DEFINE_ENUMCONST( GO_EAST, "" )
	DEFINE_ENUMCONST( GO_SOUTH, "" )
	DEFINE_ENUMCONST( GO_WEST, "" )
	DEFINE_ENUMCONST( GO_LADDER_UP, "" )
	DEFINE_ENUMCONST( GO_LADDER_DOWN, "" )
	DEFINE_ENUMCONST( GO_ELEVATOR_UP, "" )
	DEFINE_ENUMCONST( GO_ELEVATOR_DOWN, "" )
	DEFINE_ENUMCONST( NUM_TRAVERSE_TYPES, "" )
END_SCRIPTENUM()

BEGIN_SCRIPTENUM( FNavAttributeType, "Attributes applied to nav areas" )
	DEFINE_ENUMCONST( NAV_MESH_INVALID, "" )
	DEFINE_ENUMCONST( NAV_MESH_CROUCH, "" )
	DEFINE_ENUMCONST( NAV_MESH_JUMP, "" )
	DEFINE_ENUMCONST( NAV_MESH_PRECISE, "" )
	DEFINE_ENUMCONST( NAV_MESH_NO_JUMP, "" )
	DEFINE_ENUMCONST( NAV_MESH_STOP, "" )
	DEFINE_ENUMCONST( NAV_MESH_RUN, "" )
	DEFINE_ENUMCONST( NAV_MESH_WALK, "" )
	DEFINE_ENUMCONST( NAV_MESH_AVOID, "" )
	DEFINE_ENUMCONST( NAV_MESH_TRANSIENT, "" )
	DEFINE_ENUMCONST( NAV_MESH_DONT_HIDE, "" )
	DEFINE_ENUMCONST( NAV_MESH_STAND, "" )
	DEFINE_ENUMCONST( NAV_MESH_NO_HOSTAGES, "" )
	DEFINE_ENUMCONST( NAV_MESH_STAIRS, "" )
	DEFINE_ENUMCONST( NAV_MESH_NO_MERGE, "" )
	DEFINE_ENUMCONST( NAV_MESH_OBSTACLE_TOP, "" )
	DEFINE_ENUMCONST( NAV_MESH_CLIFF, "" )
	DEFINE_ENUMCONST( NAV_MESH_FIRST_CUSTOM, "" )
	DEFINE_ENUMCONST( NAV_MESH_LAST_CUSTOM, "" )
	DEFINE_ENUMCONST( NAV_MESH_FUNC_COST, "" )
	DEFINE_ENUMCONST( NAV_MESH_HAS_ELEVATOR, "" )
	DEFINE_ENUMCONST( NAV_MESH_NAV_BLOCKER, "" )
END_SCRIPTENUM()

//--------------------------------------------------------------------------------------------------------------

static class CNavAreaScriptInstanceHelper : public IScriptInstanceHelper
{
public:
...
} g_NavAreaScriptInstanceHelper;
DEFINE_SCRIPT_INSTANCE_HELPER( CNavArea, &g_NavAreaScriptInstanceHelper )

BEGIN_SCRIPTDESC_ROOT( CNavArea, "Navigation areas" )
	...
	DEFINE_SCRIPTFUNC( RemoveAttributes, "Remove area attribute bits" )
	...
	DEFINE_SCRIPTFUNC_NAMED( ScriptComputeClosestPointInPortal, "ComputeClosestPointInPortal", "( toArea, dir, fromPos ) Compute closest point within the portal between to adjacent areas." )
END_SCRIPTDESC()
...
ScriptClassDesc_t *CNavArea::GetScriptDesc( void )
{
	return ::GetScriptDesc( this );
}

HSCRIPT CNavArea::GetScriptInstance( void )
{
...
int CNavArea::ScriptGetID( void )
{
	return m_id;
}

Vector CNavArea::ScriptGetCorner( int corner )
{
	return GetCorner( (NavCornerType)corner );
}

void CNavArea::ScriptConnectToArea( HSCRIPT hArea, int dir )
{
	CNavArea *area = HScriptToClass<CNavArea>( hArea );
	if ( dir < NUM_DIRECTIONS && area )
		ConnectTo( area, (NavDirType)dir );
}
...
bool CNavArea::ScriptIsConnectedArea( HSCRIPT hArea, int dir )
{
	CNavArea *area = HScriptToClass<CNavArea>( hArea );
	if ( dir < NUM_DIRECTIONS && area )
		return IsConnected( area, (NavDirType)dir );

	return false;
}

void CNavArea::ScriptMarkAsBlocked( int team )
{
	MarkAsBlocked( team, NULL );
}

void CNavArea::ScriptUnblockArea()
{
	UnblockArea();
}

int CNavArea::ScriptGetAdjacentCount( int dir )
{
	return GetAdjacentCount( (NavDirType)dir );
}

void CNavArea::ScriptGetAdjacentAreas( int dir, HSCRIPT hTable )
{
	if ( dir >= NUM_DIRECTIONS || hTable == INVALID_HSCRIPT )
		return;

	for ( int i = 0; i < NUM_DIRECTIONS; ++i )
	{
		FOR_EACH_VEC( m_connect[i], j )
		{
			HSCRIPT hInstance = m_connect[i][j].area->GetScriptInstance();
			g_pScriptVM->SetValue( hTable, CFmtStr( "area%i", i ), hInstance );
		}
	}
}
...
{
	CNavArea *adj = GetAdjacentArea( (NavDirType)dir, i );
	return adj ? adj->GetScriptInstance() : NULL;
}
...
HSCRIPT CNavArea::ScriptGetRandomAdjacentArea( int dir )
{
	if ( dir < NUM_DIRECTIONS )
	{
		CNavArea *random = GetRandomAdjacentArea( (NavDirType)dir );
		if ( random )
			return random->GetScriptInstance();
	}
...
void CNavArea::ScriptGetIncomingConnections( int dir, HSCRIPT hTable )
{
	...
	for ( int i = 0; i < NUM_DIRECTIONS; ++i )
	{
		FOR_EACH_VEC( m_incomingConnect[i], j )
		{
			HSCRIPT hInstance = m_incomingConnect[i][j].area->GetScriptInstance();
			g_pScriptVM->SetValue( hTable, CFmtStr( "area%i", i ), hInstance );
		}
	}
}

void CNavArea::ScriptAddIncomingConnection( HSCRIPT hArea, int dir )
{
	CNavArea *area = HScriptToClass<CNavArea>( hArea );
	if( area )
	{
		if ( dir < NUM_DIRECTIONS )
			AddIncomingConnection( area, (NavDirType)dir );
	}
}

const char *CNavArea::ScriptGetPlaceName( void )
{
	return TheNavMesh->PlaceToName( m_place );
}
...
void CNavArea::ScriptSetPlaceName( const char *pszName )
{
	if ( pszName )
	{
		int place = TheNavMesh->PartialNameToPlace( pszName );
...
int CNavArea::ScriptComputeDirection( Vector const &vecPoint )
{
	Vector temp = vecPoint;
	return ComputeDirection( &temp );
}

int CNavArea::ScriptGetPlayerCount( int team )
{
	return GetPlayerCount( team );
}

bool CNavArea::ScriptIsOverlapping( HSCRIPT hArea )
{
	CNavArea *area = HScriptToClass<CNavArea>( hArea );
	return area ? IsOverlapping( area ) : false;
}

bool CNavArea::ScriptIsOverlappingOrigin( Vector const &vecPos, float tolerance )
{
	return IsOverlapping( vecPos, tolerance );
}

bool CNavArea::ScriptIsEdge( int dir )
{
	return IsEdge( (NavDirType)dir );
}

bool CNavArea::ScriptContains( HSCRIPT hArea )
{
	CNavArea *area = HScriptToClass<CNavArea>( hArea );
	return area ? Contains( area ) : false;
}

bool CNavArea::ScriptContainsOrigin( Vector const &vecPoint )
{
	return Contains( vecPoint );
}

HSCRIPT CNavArea::ScriptGetParent( void )
{
	return m_parent ? m_parent->GetScriptInstance() : NULL;
}

int CNavArea::ScriptGetParentHow( void )
{
	return m_parentHow;
}

bool CNavArea::ScriptIsVisible( Vector const &vecPoint )
{
	return IsVisible( vecPoint );
}

float CNavArea::ScriptGetZ( Vector const &vecPos )
{
	return GetZ( vecPos );
}

bool CNavArea::ScriptIsCoplanar( HSCRIPT hArea )
{
	CNavArea *area = HScriptToClass<CNavArea>( hArea );
	return area ? IsCoplanar( area ) : false;
}
...
HSCRIPT CNavArea::ScriptGetElevator( void )
{
	return m_elevator ? ((CBaseEntity *)m_elevator)->GetScriptInstance() : NULL; // Evil
}
...
void CNavArea::ScriptGetElevatorAreas( HSCRIPT hTable )
{
	...
	FOR_EACH_VEC( m_elevatorAreas, i )
	{
		HSCRIPT hInstance = m_elevatorAreas[i].area->GetScriptInstance();
		g_pScriptVM->SetValue( hTable, CFmtStr( "area%i", i ), hInstance );
	}
}

HSCRIPT CNavArea::ScriptGetDoor( void )
{
	return HSCRIPT();
}

Vector CNavArea::ScriptComputeClosestPointInPortal( HSCRIPT hTo, int dir, const Vector &vecFrom )
{
	CNavArea *to = HScriptToClass<CNavArea>( hTo );
	if ( to )
	{
		static Vector point = vec3_origin;
		ComputeClosestPointInPortal( to, (NavDirType)dir, vecFrom, &point );
		return point;
	}

	DevMsg( "ComputeClosestPointInPortal: the 'to' CNavArea was invalid. Returning origin.\n" );
	return vec3_origin;
}
