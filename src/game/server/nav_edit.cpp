			...
			if ( m_showAreaInfoTimer.HasStarted() && !m_showAreaInfoTimer.IsElapsed() )
			{
				...
				if ( m_isPlacePainting )
				{
					if ( m_selectedArea->GetPlace() != TheNavMesh->GetNavPlace() )
					{
						...
						player->EmitSound( NAV_EDIT_ENABLE );
					}
				}
			}
...
void CNavMesh::CommandNavDelete( void )
{
	...
	if ( IsSelectedSetEmpty() )
	{
		...
		if( markedArea )
		{
			player->EmitSound( NAV_EDIT_DELETE );
...
void CNavMesh::CommandNavAddToSelectedSet( void )
{
	...
	{
		AddToSelectedSet( m_selectedArea );
		player->EmitSound( NAV_EDIT_MARK_ENABLE );
	}
}
...
void CNavMesh::CommandNavRemoveFromSelectedSet( void )
{
	...
	{
		RemoveFromSelectedSet( m_selectedArea );
		player->EmitSound( NAV_EDIT_MARK_DISABLE );
	}
}
...
void CNavMesh::CommandNavBeginSelecting( void )
{
	...
	player->EmitSound( NAV_EDIT_BEGIN_AREA_CREATING );
}
...
void CNavMesh::CommandNavEndSelecting( void )
{
	...
	player->EmitSound( NAV_EDIT_END_AREA_CREATING );
}
...
void CNavMesh::CommandNavBeginDragSelecting( void )
{
	...
	if ( IsEditMode( DRAG_SELECTING ) )
	{
		ClearDragSelectionSet();
		SetEditMode( NORMAL );
		player->EmitSound( NAV_EDIT_BEGIN_AREA_NOT_CREATING );
	}
	else
	{
		player->EmitSound( NAV_EDIT_BEGIN_AREA_NOT_CREATING );
...
void CNavMesh::CommandNavEndDragSelecting( void )
{
	...
	{
		player->EmitSound( NAV_EDIT_END_AREA_NOT_CREATING );
	}
...
void CNavMesh::CommandNavSplit( void )
{
	...
	if ( m_selectedArea )
	{
		if ( m_selectedArea->SplitEdit( m_splitAlongX, m_splitEdge ) )
			player->EmitSound( NAV_EDIT_SPLIT );
		else
			player->EmitSound( NAV_EDIT_SPLIT_NO_MARKED_AREA );
	}
	...
	if ( m_selectedArea )
	{
		...
		if ( other && other != m_selectedArea )
		{
			if ( m_selectedArea->MergeEdit( other ) )
				player->EmitSound( NAV_EDIT_MERGE_ENABLE );
			else
				player->EmitSound( NAV_EDIT_MERGE_DISABLE );
		}
		else
		{
...
void CNavMesh::CommandNavConnect( void )
{
	...
	if ( m_selectedSet.Count() > 1 )
	{
		...
		for ( int i = 1; i < m_selectedSet.Count(); ++i )
		{
			...
			{
				player->EmitSound( NAV_EDIT_CONNECT_ALL_DIRECTIONS );
				bValid = false;
				break;
			}
		...
		if ( bValid )
		{
			for ( int i = 1; i < m_selectedSet.Count(); ++i )
			{
				...
				player->EmitSound( NAV_EDIT_CONNECT );
			}
		}
	}
	else if ( m_selectedArea )
...
void CNavMesh::CommandNavDisconnect( void )
{
	...
	if ( m_selectedSet.Count() > 1 )
	{
		...
		if ( bValid )
		{
			...
			player->EmitSound( NAV_EDIT_DISCONNECT );
		}
	}
	else if ( m_selectedArea )
	{
		...
		else
		{
			...
			else
			{
				...
				player->EmitSound( NAV_EDIT_DISCONNECT_NO_MARKED_AREA );
			}
		}
	}
	else if ( m_selectedLadder )
	...
	if ( m_selectedArea )
	{
		if ( GetMarkedArea() )
		{
			if ( m_selectedArea->SpliceEdit( GetMarkedArea() ) )
				player->EmitSound( NAV_EDIT_SPLICE );
			else
				player->EmitSound( NAV_EDIT_SPLICE_NO_MARKED_AREA );
		}
		else
		{
...
void CNavMesh::CommandNavToggleAttribute( NavAttributeType attribute )
{
	...
	if ( IsSelectedSetEmpty() )
	{
		...
		if ( m_selectedArea )
		{
			player->EmitSound( NAV_EDIT_ATTRIBUTE_TOGGLE );
...
void CNavMesh::CommandNavTogglePlaceMode( void )
{
	...
	player->EmitSound( NAV_EDIT_TOGGLE_PLACEMENT_MODE );
...
void CNavMesh::CommandNavPlacePick( void )
{
	...
	if ( m_selectedArea )
	{
		player->EmitSound( NAV_EDIT_PICK_PLACEMENT );
...
void CNavMesh::CommandNavTogglePlacePainting( void )
{
	...
	if ( m_selectedArea )
	{
		if ( m_isPlacePainting )
		{
			...
			player->EmitSound( NAV_EDIT_DISABLE );
		}
		else
		{
...
void CNavMesh::CommandNavMarkUnnamed( void )
{
	...
	if ( m_selectedArea )
	{
		if ( GetMarkedArea() )
		{
			player->EmitSound( NAV_EDIT_MARK_UNNAMED_ENABLE );
			SetMarkedArea( NULL );
		}
		else
		{
			...
			if ( !GetMarkedArea() )
			{
				player->EmitSound( NAV_EDIT_UNNAMED_NO_MARKED_AREA );
			}
			else
			{
				player->EmitSound( NAV_EDIT_UNNAMED );
...
void CNavMesh::CommandNavCornerSelect( void )
{
	...
	if ( m_selectedArea )
	{
		if ( GetMarkedArea() )
		{
			...
			player->EmitSound( NAV_EDIT_SELECT_CORNER );
		}
		else
		{
			player->EmitSound( NAV_EDIT_SELECT_CORNER_NO_MARKED_AREA );
		}
	}
}
	...
	if ( IsSelectedSetEmpty() )
	{
		...
		if ( m_selectedArea )
		{
			if ( GetMarkedArea() )
			{
				...
				player->EmitSound( NAV_EDIT_MOVE_CORNER );
			}
			else
			{
				player->EmitSound( NAV_EDIT_MOVE_CORNER_NO_MARKED_AREA );
			}
		}
	}
	else
...
void CNavMesh::CommandNavWarpToMark( void )
{
	...
	if ( targetArea )
	{
		...
		if ( ( player->IsDead() || player->IsObserver() ) && player->GetObserverMode() == OBS_MODE_ROAMING )
		{
			...
			player->EmitSound( NAV_EDIT_WARP_TO_MARK );
		}
		else
		{
