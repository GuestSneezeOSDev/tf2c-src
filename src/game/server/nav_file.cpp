...
NavErrorType CNavMesh::Load( void )
{
	...
	NavErrorType loadResult = LoadCustomDataPreArea( fileBuffer, subVersion );
	...
	for( i=0; i<count; ++i )
	{
		...
		loadResult = area->Load( fileBuffer, version, subVersion );
	...
	//
	loadResult = LoadCustomData( fileBuffer, subVersion );
	if ( loadResult != NAV_OK )
		return loadResult;
