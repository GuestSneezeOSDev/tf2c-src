...
// Convert raw brush sides info from the engine into an AABB, if possible
static bool ConvertBrushSidesToAABB( const CUtlVector<Vector4D>& sides, Extent& ext )
{
	// An AABB will always have exactly 6 sides
	if ( sides.Count() != 6 )
		return false;

	// Expected XYZ coords for each of the 6 axis normals
	constexpr Vector axis_pos_x(  1.0f,  0.0f,  0.0f );
	constexpr Vector axis_pos_y(  0.0f,  1.0f,  0.0f );
	constexpr Vector axis_pos_z(  0.0f,  0.0f,  1.0f );
	constexpr Vector axis_neg_x( -1.0f,  0.0f,  0.0f );
	constexpr Vector axis_neg_y(  0.0f, -1.0f,  0.0f );
	constexpr Vector axis_neg_z(  0.0f,  0.0f, -1.0f );

	// Bitmask of axes we have successfully found
	unsigned int axes = 0x00;

	for ( const auto& side : sides )
	{
		if ( side.AsVector3D() == axis_pos_x )
		{
			axes |= ( 1 << 0 );
			ext.hi.x = side.w;
		}
		else if ( side.AsVector3D() == axis_pos_y )
		{
			axes |= ( 1 << 1 );
			ext.hi.y = side.w;
		}
		else if ( side.AsVector3D() == axis_pos_z )
		{
			axes |= ( 1 << 2 );
			ext.hi.z = side.w;
		}
		else if ( side.AsVector3D() == axis_neg_x )
		{
			axes |= ( 1 << 3 );
			ext.lo.x = -side.w;
		}
		else if ( side.AsVector3D() == axis_neg_y )
		{
			axes |= ( 1 << 4 );
			ext.lo.y = -side.w;
		}
		else if ( side.AsVector3D() == axis_neg_z )
		{
			axes |= ( 1 << 5 );
			ext.lo.z = -side.w;
		}
		...
		{
			// Encountered a non-axis-normal side; not an AABB
			return false;
		}
	}

	// Ensure that we did actually encounter all six axes (no repeats)
	if ( axes != 0x3f )
		return false;
...
// Find the AABBs of all the brushes in the map that have CONTENTS_LADDER
void GetAllLadderBrushExtents( CUtlVector<Extent>& exts )
{
	CWorld *worldspawn = GetWorldEntity();
	Assert( worldspawn != nullptr );
	if ( worldspawn == nullptr )
		return;

	Extent world_ext;
	worldspawn->GetWorldBounds( world_ext.lo, world_ext.hi );

	CUtlVector<int> brush_ids;
	enginetrace->GetBrushesInAABB( world_ext.lo, world_ext.hi, &brush_ids, CONTENTS_LADDER );

	for ( auto id : brush_ids )
	{
		int contents;
		CUtlVector<Vector4D> sides;

		if ( enginetrace->GetBrushInfo( id, &sides, &contents ) )
		{
			Extent ext;
			if ( ConvertBrushSidesToAABB( sides, ext ) )
				exts.AddToTail( ext );
		}
	}
}
...
void CNavMesh::BuildLadders( void )
{
	...
	// we don't have fancy ladder entities, so we have to reconstruct the ladder brushes created in Hammer
	// to find out where the ladders in the map are located
	CUtlVector<Extent> exts;
	GetAllLadderBrushExtents( exts );
	for ( const auto& ext : exts )
	{
		CreateLadder( ext.lo, ext.hi, 0.0f );
	}
#endif
}
...
#if defined(TF_CLASSIC)
	// enable the LOS check so that we don't get an idiotic connections to a nav area on the other side of a wall
	m_bottomArea = TheNavMesh->GetNearestNavArea( center, true, 2 * nearLadderRange, true, true );
#else
