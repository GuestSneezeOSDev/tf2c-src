...
#if defined(TF_CLASSIC)
template< typename CostFunctor >
float NavAreaTravelDistance( CNavArea *startArea, CNavArea *endArea, CostFunctor &&costFunc, float maxPathLength = 0.0f, int team = TEAM_ANY, bool ignoreNavBlockers = false )
{
	if ( startArea == NULL )
		return -1.0f;
	...
	if ( NavAreaBuildPath( startArea, endArea, NULL, costFunc, NULL, maxPathLength, team, ignoreNavBlockers ) == false )
		return -1.0f;
...
{
#ifdef TF_CLASSIC
	// TF2C: passing nullptr for nearbyAreaVector means you don't care about collection
	if ( nearbyAreaVector != nullptr )
...
// Same as CollectSurroundingAreas; but instead of collecting AND marking the areas, this version eliminates the
// collection aspect (no vector argument) and only does the marking part.
// - This saves some performance/memory due to eliminating unnecessary CUtlVector operations.
// - The different name also gets across the actual semantic purpose of this function for mark-only cases, where the
//   caller would otherwise have to pass in a dummy vector argument and then not use its contents, which makes it hard
//   to understand what the caller is doing ("why is this code collecting all these areas and then not using them?").
inline void MarkSurroundingAreas( CNavArea *startArea, float travelDistanceLimit = 1500.0f, float maxStepUpLimit = StepHeight, float maxDropDownLimit = 100.0f )
{
	CollectSurroundingAreas( nullptr, startArea, travelDistanceLimit, maxStepUpLimit, maxDropDownLimit );
}
