...
{
	if (!vprof_scope_entity_thinks.GetBool())
	{
		VPROF_ENTER_SCOPE("CBaseEntity::PhysicsDispatchThink");
	}
	else
	{
		VPROF_ENTER_SCOPE( EntityFactoryDictionary()->GetCannonicalName(GetClassname()) );
	}
...
{
	if (!vprof_scope_entity_gamephys.GetBool())
	{
		VPROF("Physics_SimulateEntity");
	}
	else
	{
		VPROF_unobfus( EntityFactoryDictionary()->GetCannonicalName(pEntity->GetClassname()));
	}
