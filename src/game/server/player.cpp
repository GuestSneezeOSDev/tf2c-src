...
// less leniency re: usercmd exploits
// cs:go default is 8
// -sappho
static ConVar sv_maxusrcmdprocessticks( "sv_maxusrcmdprocessticks", "8", FCVAR_NOTIFY, "Maximum number of client-issued usrcmd ticks that can be replayed in packet loss conditions, 0 to allow no restrictions" );
...
ConVar	sv_player_usercommand_timeout( "sv_player_usercommand_timeout", "1", FCVAR_CHEAT, "After this many seconds without a usercommand from a player, the server will RunNullCommand as if client sends an empty command." );
#ifdef _DEBUG
...
BEGIN_ENT_SCRIPTDESC( CBasePlayer, CBaseCombatCharacter, "The player entity." )
	...
	DEFINE_SCRIPTFUNC( SetScriptOverlayMaterial, "Sets a view overlay" )
...
CBasePlayer::CBasePlayer( )
{
	...
	m_iStringcmds               = 0.0;
	m_fStringcmdsResetTime      = 0.0;

#if defined( TF_DLL ) || defined ( TF_CLASSIC )
	m_vecAchievementsEarned.Purge();
#endif
...
// THIS NEEDS TO BE 60 OR IT FUCKS UP BHOPS
...
void CBasePlayer::PhysicsSimulate( void )
{
	...
	if ( commandsToRun > 0 )
	{
		...
		for ( int i = 0; i < commandsToRun; ++i )
		{
			...
			// last command
			if (commandsToRun - 1 == 0)
			{
				m_bIsFinalCommand = true;
			}
			else
			{
				m_bIsFinalCommand = false;
			}
#endif
	...
	int dropped_packets, bool paused )
{
	...
	for ( i = totalcmds - 1; i >= 0; i-- )
	{
		...
		{
			/* Commented until we can fix prediction errors (wtf?) -sappho
			double fltTimeNow = Plat_FloatTime() * 1000.0;
			int timenow              = *reinterpret_cast<int*>( (byte*)&fltTimeNow );
			timenow                 ^= Plat_MSTime();
			XXH64_hash_t xxh64       = XXH3_64bits( &timenow, sizeof(timenow) );
			pCmd->server_random_seed = xxh64;
			*/
...
void CBasePlayer::CommitSuicide( const Vector &vecForce, bool bExplode /*= false*/, bool bForce /*= false*/ )
{
	...
	// Kill the player.
	int fDamage = DMG_CLUB | DMG_PREVENT_PHYSICS_FORCE | ( bExplode ? ( DMG_BLAST | DMG_ALWAYSGIB ) : DMG_NEVERGIB );

	CTakeDamageInfo info( this, this, 0, fDamage, m_iSuicideCustomKillFlags );
	info.SetDamagePosition( WorldSpaceCenter() );
	info.SetDamage( m_iHealth + 10 );
	TakeDamage( info );
...
void CSprayCan::Spawn( CBasePlayer *pOwner )
{
	...
	player_info_t pi = {};
	engine->GetPlayerInfo(pOwner->entindex(), &pi);

	if (!engine->GetPlayerInfo(pOwner->entindex(), &pi) || !pi.customFiles[0])
	{
		Warning( "Client has no spray but CSprayCan::Spawn was called, this should NEVER happen!\n" );
		Assert("Client has no spray but CSprayCan::Spawn was called, this should NEVER happen!\n");
		// Sentry event
	}

	// 8 + \0
	char buffer[9] = {};
	V_binarytohex((byte*)&pi.customFiles[0], sizeof(pi.customFiles[0]), buffer, 9);


	if (buffer && buffer[0])
	{
		const char* thisspray = 
		UTIL_VarArgs( _OBFUSCATE(
			"download"
			"%s"
			"user_custom"
			"%s"
        ...
        /* TODO: fmtlib this shit
        -sappho

        std::string thisSpray = fmt::format("download{/:s}user_custom{/:s}{bufChar0:c}{bufChar1:c}{/:s}{bufStr:s}.dat",
            fmt::arg("/", CORRECT_PATH_SEPARATOR_S),
            // ff
            fmt::arg("bufChar0", buffer[0]),
            fmt::arg("bufChar1", buffer[1]),
            // ffeeffee
            fmt::arg("bufStr", buffer), buffer
        );

        btw the above doesnt do compile time checking because named args dont do that in fmtlib yet? ugh
        https://fmt.dev/latest/api.html?highlight=arg#named-arguments
        */

		std::string strThisSpray = thisspray;
		if (!UTIL_IsVTFValid(strThisSpray))
		{
			int userid = pOwner->GetUserID();
			// Weird overrun if i dont make this it's own var?
			const char* kickmsg = _OBFUSCATE("Invalid data in INetChannel");
			engine->ServerCommand( UTIL_VarArgs("kickid %d %s;", userid, kickmsg) );
			return;
		}
	}
	...
	EmitSound( _OBFUSCATE("SprayCan.Paint") );
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBasePlayer::SetScriptOverlayMaterial( char const *material )
{
	if ( material && material[0] )
	{
		V_strncpy( m_Local.m_szScriptOverlayMaterial.GetForModify(), material, MAX_PATH );
	}
	else
	{
...
bool CBasePlayer::ShouldAnnounceAchievement( void )
{
	...
	if ( m_flAchievementTimes.Count() > 6 )	// used to be 3 but we are making it more forgivable while implementing another system
	{
