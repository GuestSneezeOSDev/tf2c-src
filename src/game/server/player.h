...
public:
	DECLARE_DATADESC();
	DECLARE_SERVERCLASS();
	DECLARE_ENT_SCRIPTDESC( CBasePlayer );
	...
	void					SetForceLocalDraw( bool bSet ) { m_Local.m_bForceLocalPlayerDraw = bSet; }
	bool					GetForceLocalDraw( void ) { return m_Local.m_bForceLocalPlayerDraw; }

	char const*				GetScriptOverlayMaterial( void ) { return m_Local.m_szScriptOverlayMaterial; }
	void					SetScriptOverlayMaterial( char const *material );
...
public:
	...
	CUtlVector<int> m_vecAchievementsEarned;
#endif
...
#if defined ( USES_ECON_ITEMS ) || defined ( TF_CLASSIC )
	CEconWearable			*GetWearable( int i ) const { return m_hMyWearables[i]; }
...
protected:
	...
	// For now, Mapbase (and TF2 Classic) use Tony Sergi's Source 2007 tonemap fixes.
...
{
	return m_weaponFiredTimer.HasStarted() && m_weaponFiredTimer.IsLessThan( 1.0f );
}
