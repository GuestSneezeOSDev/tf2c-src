...
static ConVar sv_tickbase_max_future_time("sv_tickbase_max_future_time", "5.0", FCVAR_NONE, "If non-zero, deal with clients that go too far in the future, in seconds. Fixes tickbase exploits");
...
void CPlayerMove::SetupMove( CBasePlayer *player, CUserCmd *ucmd, IMoveHelper *pHelper, CMoveData *move )
{
	...
	// squash any roll related exploits
	// thank you pelipoika
	// -sappho
	move->m_vecViewAngles.z     = 0.0;
	move->m_vecAbsViewAngles.z  = 0.0;
}
...
{
	// Thanks Dysphie/NMRIH team
	// -sappho
	const float flMaxFutureTime = sv_tickbase_max_future_time.GetFloat();
	if (flMaxFutureTime > 0.0)
	{
		const int maxAheadTicks = TIME_TO_TICKS(flMaxFutureTime);
		const int aheadTicks = gpGlobals->tickcount - player->m_nTickBase;
		if (aheadTicks > maxAheadTicks)
		{
			// Reset their tickbase
			player->m_nTickBase = gpGlobals->tickcount;

			Warning("[%f] Client %d too far in the future! (%f)\n",
				gpGlobals->curtime, player->entindex(), TICKS_TO_TIME(aheadTicks));
		}
	}
