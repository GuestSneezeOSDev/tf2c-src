...
// please don't change this from 0.2 unless you know exactly what you're doing.
// -sappho
ConVar sv_maxunlag( "sv_maxunlag", "0.2", FCVAR_DEVELOPMENTONLY, "Maximum lag compensation in seconds", true, 0.0f, true, 1.0f );
...
public:
	...
	LagRecord( const LagRecord& src )
	{
		...
		int i;
		for ( i = 0; i < MAX_LAYER_RECORDS; ++i )
		{
			m_layerRecords[i] = src.m_layerRecords[i];
		}
		m_masterSequence = src.m_masterSequence;
		m_masterCycle = src.m_masterCycle;
		for ( i = 0; i < MAX_POSE_PARAMETERS; ++i )
		{
...
void CLagCompensationManager::FrameUpdatePostEntityThink()
{
	...
	for ( int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		int j;
		for ( j = 0; j < pPlayer->GetNumAnimOverlays(); ++j )
		{
			CAnimationLayer *currentLayer = pPlayer->GetAnimOverlay( j );
			if ( currentLayer )
			{
				record.m_layerRecords[j].m_cycle = currentLayer->m_flCycle;
				record.m_layerRecords[j].m_order = currentLayer->m_nOrder;
				record.m_layerRecords[j].m_sequence = currentLayer->m_nSequence;
				record.m_layerRecords[j].m_weight = currentLayer->m_flWeight;
			}
		}
		...
		if ( hdr )
		{
			for ( j = 0; j < hdr->GetNumPoseParameters(); j++ )
			{
				record.m_poseParameters[j] = pPlayer->GetPoseParameter( j );
			}
		}
	}
...
void CLagCompensationManager::StartLagCompensation( CBasePlayer *player, CUserCmd *cmd )
{
	...
	if ( m_pCurrentPlayer )
	{
        ...
        {
                Assert( cmd );
                Warning( "Trying to start a new lag compensation session with a nullcmd!?!\n" );
                return;
        }
...
void CLagCompensationManager::BacktrackPlayer( CBasePlayer *pPlayer, float flTargetTime )
{
	...
	int i;
	for ( i = 0; i < pPlayer->GetNumAnimOverlays(); ++i )
	{
		CAnimationLayer *currentLayer = pPlayer->GetAnimOverlay( i );
		if ( currentLayer )
		{
			restore->m_layerRecords[i].m_cycle = currentLayer->m_flCycle;
			restore->m_layerRecords[i].m_order = currentLayer->m_nOrder;
			restore->m_layerRecords[i].m_sequence = currentLayer->m_nSequence;
			restore->m_layerRecords[i].m_weight = currentLayer->m_flWeight;

			bool interpolated = false;
			if ( frac > 0.0f && interpolationAllowed )
			{
				LayerRecord &recordsLayerRecord = record->m_layerRecords[i];
				LayerRecord &prevRecordsLayerRecord = prevRecord->m_layerRecords[i];
			...
			if ( !interpolated )
			{
				...
				currentLayer->m_flCycle = record->m_layerRecords[i].m_cycle;
				currentLayer->m_nOrder = record->m_layerRecords[i].m_order;
				currentLayer->m_nSequence = record->m_layerRecords[i].m_sequence;
				currentLayer->m_flWeight = record->m_layerRecords[i].m_weight;
			}
		}
	}
	...
	if ( hdr )
	{
		...
		{
			restore->m_poseParameters[i] = pPlayer->GetPoseParameter( i );
			...
			if ( frac > 0.0f && interpolationAllowed )
			{
				...
				pPlayer->SetPoseParameter( hdr, i, Lerp( frac, record->m_poseParameters[i], prevRecord->m_poseParameters[i] ) );
			}
			else
			{
				pPlayer->SetPoseParameter( hdr, i, record->m_poseParameters[i] );
			}
		}
	}
	...
	for ( int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		if ( restore->m_fFlags & LC_ANIMATION_CHANGED )
		{
			...
			for ( j = 0; j < pPlayer->GetNumAnimOverlays(); ++j )
			{
				...
				if ( currentLayer )
				{
					currentLayer->m_flCycle = restore->m_layerRecords[j].m_cycle;
					currentLayer->m_nOrder = restore->m_layerRecords[j].m_order;
					currentLayer->m_nSequence = restore->m_layerRecords[j].m_sequence;
					currentLayer->m_flWeight = restore->m_layerRecords[j].m_weight;
				}
			}
		}
		...
		if ( restore->m_fFlags & LC_POSE_PARAMS_CHANGED )
		{
			...
			if ( hdr )
			{
				for ( j = 0; j < hdr->GetNumPoseParameters(); j++ )
				{
					pPlayer->SetPoseParameter( hdr, j, restore->m_poseParameters[j] );
				}
			}
		}
