...
//-----------------------------------------------------------------------------
class CFuncRespawnRoomVisualizer : public CFuncBrush, public TAutoList<CFuncRespawnRoomVisualizer>
{
...
bool PointsCrossRespawnRoomVisualizer( const Vector &vecStart, const Vector &vecEnd, int iTeam )
{
	...
	for (CFuncRespawnRoomVisualizer *pVisualizer : CFuncRespawnRoomVisualizer::AutoList())
	{
...
//-----------------------------------------------------------------------------
bool PointInRespawnRoom( CBaseEntity *pTarget, const Vector &vecOrigin, bool bCheckTouchingSameTeam /*= false*/ )
{
	...
	while ((pEntity = gEntList.FindEntityByClassname( pEntity, "func_respawnroom" )) != NULL)
	{
		...
		if ( pRespawnRoom->GetActive() )
		{
			...
			else 
			{
				...
				{
					if ( !bCheckTouchingSameTeam || pRespawnRoom->GetTeamNumber() == TEAM_UNASSIGNED || pRespawnRoom->InSameTeam( pTarget ) )
						return true;
				}
			}
		}
...
BEGIN_DATADESC( CFuncRespawnRoomVisualizer )
	...
	DEFINE_KEYFIELD( m_bSolidToEnemies, FIELD_BOOLEAN, "solid_to_enemies" ),
	// inputs
	DEFINE_INPUTFUNC( FIELD_VOID, "RoundActivate", InputRoundActivate ),
...
{
	m_bSolidToEnemies = inputdata.value.Bool();
}
