...
BEGIN_DATADESC( CTeamControlPoint )
	...
	DEFINE_KEYFIELD( m_iDominationRate, FIELD_INTEGER, "point_dom_rate" ),
...
{
	// Fill it up with default data. Shuts up errors about missing GRN and YLW icons in
	// the original TF2 maps.
	for ( int i = 0; i < TF_TEAM_COUNT; i++ )
	{
		if ( i == TEAM_SPECTATOR )
			continue;

		m_TeamData[i].iszModel = AllocPooledString( "models/effects/cappoint_hologram.mdl" );
		m_TeamData[i].iszIcon = AllocPooledString( UTIL_VarArgs( "sprites/obj_icons/icon_obj_%s", GetTeamSuffix( i, g_aTeamNamesShort, "neutral" ) ) );
	}
...
void CTeamControlPoint::Spawn( void )
{
	...
	TFGameRules()->RecalculateControlPointState();
...
{
	if ( TFGameRules() && !TFGameRules()->ShouldScorePerRound() )
	{
		GetGlobalTeam( iTeam )->AddScore( 1 );
		TFGameRules()->HandleTeamScoreModify( iTeam, 1 );
...
{
	for ( int i = LAST_SHARED_TEAM + 1; i < TF_TEAM_COUNT; i++ )
	{
		// Skip spectator
		if ( i == TEAM_SPECTATOR )
			continue;
...
void CTeamControlPoint::UnlockThink( void )
{
	if ( m_flUnlockTime > 0 &&
		...
		TFGameRules()->PointsMayBeCaptured() )
	{
		InternalSetLocked( false );
		return;
	}
