...
bool CTeamControlPointMaster::FindControlPoints( void )
{
	...
	while( newIndex < numPoints )
	{
		...
		lowestIndex = Clamp(lowestIndex, 0, MAX_CONTROL_POINTS-1);
	...
	for ( unsigned int i = 0; i < m_ControlPoints.Count(); i++ )
	{
		...
		ObjectiveResource()->SetDominationRate( iPointIndex, pPoint->GetDominationRate() );
		ObjectiveResource()->SetCPGroup( iPointIndex, pPoint->GetCPGroup() );
...
bool CTeamControlPointMaster::SelectSpecificRound( void )
{
	...
	if ( TFGameRules()->GetRoundToPlayNext() != NULL_STRING )
	{
		// do we have the name of a round?
		pRound = dynamic_cast<CTeamControlPointRound*>( gEntList.FindEntityByName( NULL, STRING( TFGameRules()->GetRoundToPlayNext() ) ) );
		...
		TFGameRules()->SetRoundToPlayNext( NULL_STRING );
	}
	...
	if ( pRound )
	{
		...
		TFGameRules()->SetRoundOverlayDetails();
...
void CTeamControlPointMaster::RegisterRoundBeingPlayed( void )
{
	...
	TFGameRules()->AddPlayedRound( iszEntityName );

	if ( m_bFirstRoundAfterRestart )
	{
		TFGameRules()->SetFirstRoundPlayed( iszEntityName );
...
bool CTeamControlPointMaster::GetControlPointRoundToPlay( void )
{
	...
	string_t iszLastRoundPlayed = TFGameRules()->GetLastPlayedRound();
	int iLastRoundPlayed = -1;

	string_t iszFirstRoundPlayed = TFGameRules()->GetFirstRoundPlayed();
	...
	if ( nRounds.Count() > 1 )
	{
		...
		while ( TFGameRules()->IsPreviouslyPlayedRound( m_ControlPointRounds[nRounds[index]]->GetEntityName() ) &&
				nRounds.Count() > 1 )
		{
	...
	if ( args.ArgC() > 1 )
	{
		...
		if ( pMaster )
		{
			if ( pMaster->PlayingMiniRounds() )
			{
				...
				if ( pRound )
				{
					TFGameRules()->SetRoundToPlayNext( pRound->GetEntityName() );
