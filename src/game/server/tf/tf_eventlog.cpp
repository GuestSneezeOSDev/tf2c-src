...
protected:
	...
	bool PrintTFEvent( IGameEvent *event )	// print Mod specific logs
	{
		...
		if ( !Q_strncmp( eventName, "player_death", Q_strlen( "player_death" ) ) )
		{
			...
			auto iCustomDamage = (ETFDmgCustom)event->GetInt( "customkill" );
		...
		else if ( FStrEq( eventName, "teamplay_flag_event" ) )
		{
			...
			auto iEventType = (ETFFlagEventTypes)event->GetInt( "eventtype" );
		...
		else if ( FStrEq( eventName, "teamplay_point_captured" ) )
		{
			...
			V_sprintf_safe( buf, "Team \"%s\" triggered \"pointcaptured\" (cp \"%d\") (cpname \"%s\") (numcappers \"%d\") ",
				pTeam->GetName(),
				event->GetInt( "cp" ),
				event->GetString( "cpname" ),
				iNumCappers );
			...
			for ( int i = 0; i < iNumCappers; i++ )
			{
				...
				char playerBuf[256];
				V_sprintf_safe( playerBuf, "(player%d \"%s<%i><%s><%s>\") (position%d \"%d %d %d\") ",
					i + 1,
					pPlayer->GetPlayerName(),
					pPlayer->GetUserID(),
					pPlayer->GetNetworkIDString(),
