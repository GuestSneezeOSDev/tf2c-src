...
ConVar tf2c_use_old_payload_physics( "tf2c_use_old_payload_physics", "0", FCVAR_CHEAT, "Disables new payload physics" );
...
bool CTFPhysicsPushEntities::UsingNewPushPhysics( void )
{
	if ( tf2c_use_old_payload_physics.GetBool() )
		return false;

	return ( TFGameRules()->GetGameType() == TF_GAMETYPE_ESCORT );
}


bool CTFPhysicsPushEntities::RotationPushTFPlayer( CPhysicsPushedEntities::PhysicsPushedInfo_t &info, const Vector &vecAbsPush, const CPhysicsPushedEntities::RotatingPushMove_t &rotPushMove )
{
...
bool CTFPhysicsPushEntities::LinearPushTFPlayer( CPhysicsPushedEntities::PhysicsPushedInfo_t &info, const Vector &vecAbsPush )
{
	CBaseEntity *pBlocker = info.m_pEntity;
	info.m_vecStartAbsOrigin = pBlocker->GetAbsOrigin();
	CBaseEntity *pRootHighestParent = m_rgPusher[0].m_pEntity->GetRootMoveParent();
...
void CTFPhysicsPushEntities::RotationCheckPush( CPhysicsPushedEntities::PhysicsPushedInfo_t &info )
{
	CBaseEntity *pBlocker = info.m_pEntity;
	CBaseEntity *pRootHighestParent = m_rgPusher[0].m_pEntity->GetRootMoveParent();
...
void CTFPhysicsPushEntities::LinearCheckPush( CPhysicsPushedEntities::PhysicsPushedInfo_t &info )
{
	CBaseEntity *pBlocker = info.m_pEntity;
	CBaseEntity *pRootHighestParent = m_rgPusher[0].m_pEntity->GetRootMoveParent();
...
void CTFPhysicsPushEntities::MovePlayer( CBaseEntity *pEntity, CPhysicsPushedEntities::PhysicsPushedInfo_t &info, float flFraction, bool bRidingTrain )
{
	...
	for ( int i = 0; i < 4; i++ )
	{
		...
		if ( bRidingTrain && PointsCrossRespawnRoomVisualizer( vecOriginalPos, info.m_Trace.endpos, pEntity->GetTeamNumber() ) )
		{
			// HACK: If we run into respawn room visualizer, kill the player immediately.
...
void CTFPhysicsPushEntities::FindNewPushDirection( const Vector &vecDir, const Vector &vecNormal, Vector &vecNewDir )
{
