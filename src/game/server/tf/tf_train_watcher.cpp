...
{		
	...
	bool bDownHillData[TF_TRAIN_MAX_HILLS];
		...
		while ( pNode )
		{
			...
			if ( pNode->GetHillType() != iHillType )
			{
				if ( !bOnHill ) // we're at the start of a hill
				{
					...
					if ( iHillCount < TF_TRAIN_MAX_HILLS )
					{
	...
	if ( ObjectiveResource() )
	{
		...
		if ( m_flTotalPathDistance > 0 && hillData.Count() > 0 )
		{
			...
			{
				if ( i < TF_TRAIN_HILLS_ARRAY_SIZE - 1 ) // - 1 because we want to use 2 entries
				{
...
void CTeamTrainWatcher::PlayCaptureAlert( CTeamControlPoint *pPoint, bool bFinalPointInMap )
{
	...
	TFGameRules()->PlayTrainCaptureAlert( pPoint, bFinalPointInMap );
}
...
void CTeamTrainWatcher::WatcherThink( void )
{
	...
	bool bDisableAlarm = ( TFGameRules()->State_Get() != GR_STATE_RND_RUNNING );
	if ( bDisableAlarm )
	{
		StopCaptureAlarm();
	}
	...
	if ( pTrain )
	{
		...
		if ( m_iTrainSpeedLevel != iOldTrainSpeedLevel )
		{
			...
			if ( m_iTrainSpeedLevel == 0 && iOldTrainSpeedLevel != 0 )
			{
				TFGameRules()->HaveAllPlayersSpeakConceptIfAllowed( MP_CONCEPT_CART_STOP );
				m_flNextSpeakForwardConceptTime = 0;
			}
			else if ( m_iTrainSpeedLevel < 0 && iOldTrainSpeedLevel == 0 )
			{
				TFGameRules()->HaveAllPlayersSpeakConceptIfAllowed( MP_CONCEPT_CART_MOVING_BACKWARD );
				m_flNextSpeakForwardConceptTime = 0;
			}
		}
