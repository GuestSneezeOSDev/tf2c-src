...
void CTriggerAreaCapture::CaptureThink( void )
{
	...
	if ( m_bCapturing )
	{
		...
		else if ( m_nOwningTeam != m_nTeamInZone && m_nTeamInZone != TEAM_UNASSIGNED )
		{
			SetCapTimeRemaining( m_fTimeRemaining + flReduction );
		}
		else
		{
			...
			if ( m_hPoint && TFGameRules()->TeamMayCapturePoint( m_nCapturingTeam, m_hPoint->GetPointIndex() ) )
			{
				// VIP teammates can stop deterioration, but not capture it themselves
				if ( TFGameRules()->IsVIPMode() && ( m_TeamData[m_nCapturingTeam].iNumTouching + iNumBlockablePlayers[m_nCapturingTeam] ) > 0 )
				{
					m_hPoint->UpdateCapPercentage();
					ObjectiveResource()->SetCappingTeam( m_hPoint->GetPointIndex(), m_nCapturingTeam );
					ObjectiveResource()->SetCapHalted( m_hPoint->GetPointIndex(), true );

					SetCapTimeRemaining( m_fTimeRemaining );
				}
				else
				{
					ObjectiveResource()->SetCapHalted( m_hPoint->GetPointIndex(), false );
...
void CTriggerAreaCapture::HandleRespawnTimeAdjustments( int oldTeam, int newTeam )
{
	if ( oldTeam > LAST_SHARED_TEAM )
	{
		...
		{
			TFGameRules()->AddTeamRespawnWaveTime( oldTeam, -m_TeamData[oldTeam].iSpawnAdjust );
		}
	}
	...
	if ( newTeam > LAST_SHARED_TEAM )
	{
		...
		{
			TFGameRules()->AddTeamRespawnWaveTime( newTeam, m_TeamData[newTeam].iSpawnAdjust );
		}
	}
}
...
void CTriggerAreaCapture::EndCapture( int team )
{
	...
	TFGameRules()->PlaySpecialCapSounds( m_nOwningTeam, m_hPoint.Get() );
