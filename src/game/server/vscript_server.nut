...
function __CollectEventCallbacks( scope, prefix, globalTableName, regFunc )
{
	...
	{
		printl( "__CollectEventCallbacks[" + prefix + "]: NOT TABLE! : " + typeof(scope) );
		return;
	}
...
function __RunEventCallbacks( event, params, prefix, globalTableName, bWarnIfMissing )
{
	...
	if( !(event in useTable) )
	{
		if( bWarnIfMissing )
		{
			printl( "__RunEventCallbacks[" + prefix + "]: Invalid 'event' name: " + event + ". No listeners registered for that event." );
			return;
		}
	}
...
// kinda want to rename this "SendScriptEvent" - since we just send it to a script
...
function ScriptDebugDraw()
{
	...
	{
		ScriptDebugDrawWatches( ScriptDebugWatchFirstLine );
	}
...
function ScriptDebugAddWatches( watchArray )
{
	...
	{
		throw "ScriptDebugAddWatches() expected and array!";
	}
}
...
function ScriptDebugRemoveWatches( watchArray )
{
	...
	{
		throw "ScriptDebugRemoveWatches() expected and array!";
	}
}
...
function ScriptDebugAddTrace( traceTarget )
{
	...
	if( type == "string" || typ == "table" || type == "instance" )
	{
...
function ScriptDebugTextTrace( text, color = [255, 255, 255] )
{
	...
	if( !bPrint && ScriptDebugTraces.len() > 0 )
	{
		local stackinfo = getstackinfos( 2 );
		if( stackinfo != null )
		{
			if( ( stackinfo.func in ScriptDebugTraces ) || ( stackinfo.src in ScriptDebugTraces ) || ( stackinfo.locals["this"] in ScriptDebugTraces ))
			{
				bPrint = true;
			}
		}
...
function ScriptDebugTextDraw( line )
{
	...
	for( i = 0; i < ScriptDebugText.len(); i++ )
	{
		...
		if( age < ScriptDebugTextTime )
		{
			...
			DebugDrawScreenTextLine( 0.5, 0.0, line++, ScriptDebugText[i][1], ScriptDebugText[i][2][1], ScriptDebugText[i][2][2], alpha, NDEBUG_PERSIST_TILL_NEXT_SERVER );
		}
	}
...
{
	setdebughook( __VScriptDebugHook );
}
