	...
	case PLAYERANIMEVENT_RELOAD:
		{
			...
			float flPlaybackRate = 1.0f;
				...
				CTFPlayer* pTFPlayer = dynamic_cast<CTFPlayer*>(GetBasePlayer());
				if (pTFPlayer)
				{
					if (pTFPlayer->m_Shared.InCond(TF_COND_TRANQUILIZED))
					{
						flPlaybackRate *= TF2C_TRANQ_DEPLOY_FACTOR;
					}
					if (pTFPlayer->m_Shared.InCond(TF_COND_CIV_SPEEDBUFF))
					{
						flPlaybackRate *= TF2C_HASTE_RELOAD_FACTOR;
					}
				}
			#endif
			if (flPlaybackRate == 0.0f) 
				flPlaybackRate = 1.0f;
			m_aGestureSlots[GESTURE_SLOT_ATTACK_AND_RELOAD].m_pAnimLayer->m_flPlaybackRate = 1.0f / flPlaybackRate;
