...
void WaveTrace( char const *wavname, char const *funcname )
{
    ...
    auto waveTraceSym = s_WaveTrace.Find(wavname);
	// Make sure we only show the message once
	if ( !waveTraceSym.IsValid() )
	{
...
public:
	...
	void LogPrecache( char const *soundname )
	{
        ...
        auto sym = m_PrecachedScriptSounds.Find(soundname);
		// Make sure we only show the message once
		if ( !sym.IsValid() )
			return;
	...
	virtual void LevelInitPreEntity()
	{
		...
		else
		{
			...
			const char* pszGlobalScriptFile = "maps/custom_level_sounds.txt"
			if ( filesystem->FileExists( pszGlobalScriptFile, "GAME" ) )
			{
				soundemitterbase->AddSoundOverrides( pszGlobalScriptFile );
			}
		}
#else
		Q_StripExtension( mapname, scriptfile, sizeof( scriptfile ) );
	...
	HSOUNDSCRIPTHANDLE PrecacheScriptSound( const char *soundname )
	{
		...
		if ( !soundemitterbase->IsValidIndex( soundIndex ) )
		{
			...
			if ( soundname[ 0 ] )
			{
                ...
                auto sym = s_PrecacheScriptSoundFailures.Find(soundname);
				// Make sure we only show the message once
				if ( !sym.IsValid() )
				{
...
#else
CON_COMMAND_F( sv_soundemitter_flush, "Flushes the sounds.txt system (server only)", FCVAR_NONE )
#endif
{
...
CON_COMMAND_F( sv_soundemitter_filecheck, "Report missing wave files for sounds and game_sounds files.", FCVAR_NONE )
{
	if ( !UTIL_IsCommandIssuedByServerAdmin() )
		return;
...
CON_COMMAND_F( sv_findsoundname, "Find sound names which reference the specified wave files.", FCVAR_NONE )
{
	if ( !UTIL_IsCommandIssuedByServerAdmin() )
		return;
