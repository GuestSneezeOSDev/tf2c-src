...
#if defined( TF_CLASSIC ) || defined( TF_CLASSIC_CLIENT )
ConVar	tf2c_achievement_mode("tf2c_achievement_mode", "0", FCVAR_NOTIFY | FCVAR_REPLICATED, "0 - default achievement gain behavior. 1 - achievements enabled even with cheats or randomizer on. 2 - achievements disabled.");
#endif
...
// unhack hack for updated steam api
// -sappho
#if 0
#ifdef SWDS
...
void CAchievementMgr::AwardAchievement( int iAchievementID )
{
...
#ifdef CLIENT_DLL
		// why was this running on the server?
		// -sappho
	...
	{		
		...
		if ( steamapicontext->SteamUserStats() )
		{
			...
			steamapicontext->SteamUserStats()->SetAchievement( pAchievement->GetName() );
			//		Assert( bRet );
		}
#endif
		m_AchievementsAwarded.AddToTail( iAchievementID );

		// Not on Steam, so we'll have to do this locally and fake the callback.
		UserStatsStored_t *pLocalStats = new UserStatsStored_t;
		pLocalStats->m_nGameID = engine->GetAppID();
		pLocalStats->m_eResult = k_EResultOK;
		Steam_OnUserStatsStored( pLocalStats );
    }
}
...
#define AchMsg( message ) if ( !bGag ) Msg( message )

//-----------------------------------------------------------------------------
// Purpose: checks if achievements are enabled
//-----------------------------------------------------------------------------
bool CAchievementMgr::CheckAchievementsEnabled( bool bGag /*= false*/ )
{
	// Not needed for TF2Classic, as the achievements are completely local.
#if !defined( TF_CLASSIC ) && !defined( TF_CLASSIC_CLIENT )
	// if PC, Steam must be running and user logged in
	if ( IsPC() && !LoggedIntoSteam() )
	{
		AchMsg( "Achievements disabled: Steam not running\n" );
		return false;
	}
#endif
	...
	{
		AchMsg( "Achievements disabled: Not signed in to XBox user account\n" );
		return false;
	}
#endif
	...
	{
		AchMsg( "Achievements disabled: In Commentary Mode\n" );
		return false;
	}
	...
	{
		AchMsg( "Achievements disabled: Demo playing\n" );
		return false;
	}
#endif // CLIENT_DLL
	...
	if ( sv_nostats.GetBool() )
	{
		...
		{
			AchMsg( "Achievements and stats disabled: sv_nostats is set\n" );
	...
	{
		AchMsg( "Achievements disabled: In Training Mode\n" );
		return false;
	}
	...
	{
		AchMsg( "Achievements disabled: Practicing offline\n" );
		return false;
	}
#endif
...
#if defined( TF_CLASSIC ) || defined( TF_CLASSIC_CLIENT )
	int iAchievementMode = tf2c_achievement_mode.GetInt();
	switch (iAchievementMode)
	{
	case 0:
		break;
	case 1:
...
#if defined( TF_CLASSIC ) || defined( TF_CLASSIC_CLIENT )
	if ( GetRandomizerManager()->RandomizerWasEverOn() )
	{
		AchMsg( "Achievements disabled: Randomizer Mode on in this app session\n" );
		return false;
	}
#endif
	...
	if ( IsPC() )
	{
		...
		if ( WereCheatsEverOn() )
		{
			...
			// on Steam public
			if ( developer.GetInt() == 0 || !steamapicontext->SteamUtils() || steamapicontext->SteamUtils()->GetConnectedUniverse() == k_EUniversePublic )
			{
				AchMsg( "Achievements disabled: cheats turned on in this app session\n" );
				return false;
			}
#endif
		}
...
void CAchievementMgr::FireGameEvent( IGameEvent *event )
{
	...
	else if ( 0 == Q_strcmp( name, "player_death" ) )
	{
		...
		CBaseEntity *pInflictor = ClientEntityList().GetEnt(event->GetInt("entindex_inflictor")); // Not too amazing, I wouldn't trust projectiles, but better than nothing
		OnKillEvent( pVictim, pAttacker, pInflictor, event );
	}
