...
void CBaseAchievement::HandleProgressUpdate()
{
	...
	if (m_flProgressMsgIncrement <= 0.0f || m_iCount <= 0)
		return;

	// Best way to handle cases when m_flProgressMsgIncrement could be either float or pretty int
	// I didn't change m_iProgressShown naming because I don't want to mess in parts of code I don't yet understand
	// Ceil2Int(m_iProgressShown * m_flProgressMsgIncrement) represent the next desire count target
	// Upon hitting this target, we calculate the next one by adding m_flProgressMsgIncrement steps until we are over current count
	// We use Ceil2Int in case of float so we are less likely to get "last target of 1001 is 1000" due to float sum error
	// Progress being shown from the first time count changes from zero is a feature, not a bug
	if (m_iCount >= Ceil2Int(m_iProgressShown * m_flProgressMsgIncrement))
	{
		ShowProgressNotification();

		for (int i = m_iProgressShown + 1; i <= 8; ++i) // 8 is hardcoded due to CalcProgressMsgIncrement logic as sanity check
		{
			if (Ceil2Int(i * m_flProgressMsgIncrement) > m_iCount)
			{
				m_iProgressShown = i;
				break;
			}
	...
	{
#ifndef GAME_DLL
		IGameEvent* event = gameeventmanager->CreateEvent("achievement_event_update");
		if (event)
		{
...
{
	// if we change logic here, make sure to change loop max count in HandleProgressUpdate

	if (m_iGoal < 10)
	{
		m_flProgressMsgIncrement = 1;
		return;
	}

	// Try to see if we can evenly divide by 4, then try to evenly divide by 5
	// Otherwise do 25% if goal below 100, else 20%
	if (0 == (m_iGoal % 4))
	{
		m_flProgressMsgIncrement = float(m_iGoal / 4);
	}
	else if (0 == (m_iGoal % 5))
	{
		m_flProgressMsgIncrement = float(m_iGoal / 5);
	}
	else if (m_iGoal < 100)
	{
		m_flProgressMsgIncrement = m_iGoal / 4.0f;
	}
	else
	{
		m_flProgressMsgIncrement = m_iGoal / 5.0f;
	}
