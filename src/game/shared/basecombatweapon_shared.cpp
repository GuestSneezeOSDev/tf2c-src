...
void CBaseCombatWeapon::Spawn( void )
{
	...
	CollisionProp()->UseTriggerBounds( true, 32 );
...
void CBaseCombatWeapon::Precache( void )
{
	...
	if ( ReadWeaponDataFromFileForSlot( filesystem, GetClassname(), &m_hWeaponFileInfo, GetEncryptionKey() ) )
	{
		...
		if ( GetWpnData().szAmmo1[0] )
		{
			...
			int iModUseCustomAmmoOverride = 0;
			CALL_ATTRIB_HOOK_INT( iModUseCustomAmmoOverride, mod_use_custom_ammo_type );
			if ( iModUseCustomAmmoOverride )
			{
				if( iModUseCustomAmmoOverride >= TF_AMMO_PRIMARY && iModUseCustomAmmoOverride < TF_AMMO_COUNT )
					m_iPrimaryAmmoType = iModUseCustomAmmoOverride;
			}
#endif
 		}
...
// Input  : model - 
//-----------------------------------------------------------------------------
void CBaseCombatWeapon::SetCustomViewModel( char const *pszViewModel )
{
	...
	m_nCustomViewmodelModelIndex = PrecacheModel( pszViewModel, false );
...
// Input  : index - 
//-----------------------------------------------------------------------------
void CBaseCombatWeapon::SetCustomViewModelModelIndex( int iViewModel )
{
	m_nCustomViewmodelModelIndex = iViewModel;
}
#endif
//-----------------------------------------------------------------------------
// Purpose: 
...
void CBaseCombatWeapon::SetViewModel()
{
	...
	{
		model_t const *pModel = modelinfo->GetModel( m_nCustomViewmodelModelIndex );
		if ( pModel != NULL )
		{
			vm->SetWeaponModel( modelinfo->GetModelName( pModel ), this );
		}
	}
	else
	{
...
{ 
	//check if the weapon is in the middle of a burst. If so, cancel
	if (m_iRoundsLeftInBurst > 0)
	{
		return false;
	}
...
void CBaseCombatWeapon::ItemPostFrame( void )
{
	...
	if ( !bFired && (pOwner->m_nButtons & IN_ATTACK) && (m_flNextPrimaryAttack <= gpGlobals->curtime))
	{
		...
		else
		{
			...
			if (m_iRoundsLeftInBurst < 1)
			{
				CALL_ATTRIB_HOOK_INT(m_iRoundsLeftInBurst, set_burstfire);
			}

			PrimaryAttack();

			m_iRoundsLeftInBurst--;
...
bool CBaseCombatWeapon::IsBursting(void)
{
	return (m_iRoundsLeftInBurst > 1);
}
...
bool CBaseCombatWeapon::ReloadsSingly( void ) const
{
	...
	int iReloadSingle = 0;
	CALL_ATTRIB_HOOK_INT(iReloadSingle, set_reload_single);
	if (iReloadSingle)
	{
		return true;
	}
...
void CBaseCombatWeapon::UpdateBurstFire(void)
{
	CBasePlayer* pOwner = ToBasePlayer(GetOwner());
	if (!pOwner)
		return;
	...
	if (m_iRoundsLeftInBurst > 1)
	{
		// Don't allow attack button if we're mid burst
		pOwner->m_nButtons &= ~IN_ATTACK;
		
	}

	bool bReleaseClip = (m_iClip1 || !UsesClipsForAmmo1()) && !(pOwner->m_nButtons & IN_ATTACK);
	if (bReleaseClip)
	{
		// Fake the attack key
		pOwner->m_nButtons |= IN_ATTACK;
...
void CDmgAccumulator::AccumulateMultiDamage( const CTakeDamageInfo &info, CBaseEntity *pEntity )
{
	...
	else
	{
		...
		if ( pInfo )
		{
			...
			m_TargetsDmgInfo[iIndex].SetMultiCount(pInfo->GetMultiCount() + 1);
		}
...
BEGIN_PREDICTION_DATA( CBaseCombatWeapon )
	...
	DEFINE_PRED_FIELD( m_iRoundsLeftInBurst, FIELD_INTEGER, FTYPEDESC_INSENDTABLE),

	DEFINE_PRED_FIELD( m_nViewModelIndex, FIELD_INTEGER, FTYPEDESC_INSENDTABLE ),
	DEFINE_PRED_FIELD( m_nCustomViewmodelModelIndex, FIELD_INTEGER, FTYPEDESC_INSENDTABLE ),
...
BEGIN_NETWORK_TABLE_NOBASE( CBaseCombatWeapon, DT_LocalWeaponData )
	...
	SendPropInt(SENDINFO(m_iRoundsLeftInBurst), 8),

	SendPropInt( SENDINFO( m_nViewModelIndex ), VIEWMODEL_INDEX_BITS, SPROP_UNSIGNED ),
	SendPropInt( SENDINFO( m_nCustomViewmodelModelIndex ), 13, SPROP_UNSIGNED ),
	...
	RecvPropInt( RECVINFO(m_iRoundsLeftInBurst )),
...
BEGIN_ENT_SCRIPTDESC( CBaseCombatWeapon, CEconEntity, "The base class for all equippable weapons." )
	...
	DEFINE_SCRIPTFUNC( GetWeight, "Get the weapon's weighting/importance." )
	...
	DEFINE_SCRIPTFUNC( GetPrintName, "Gets the weapons print name." )
