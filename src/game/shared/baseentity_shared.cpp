...
ConVar tf2c_voice_pitch_vip_haste("tf2c_voice_pitch_vip_haste", "1.075", FCVAR_NOTIFY | FCVAR_REPLICATED, "Voice shift multiplier when for player under effect of VIP Haste");
ConVar tf2c_voice_pitch_tranq("tf2c_voice_pitch_tranq", "0.925", FCVAR_NOTIFY | FCVAR_REPLICATED, "Voice shift multiplier when for player under effect of Tranq");
...
bool CBaseEntity::ShouldCollide( int collisionGroup, int contentsMask, const IHandleEntity *pEnt ) const
{
	return ShouldCollide( collisionGroup, contentsMask );
}
#endif
...
#else
#ifdef TF_CLASSIC
			if (info.m_pCustomFilter)
				AI_TraceHull( info.m_vecSrc, vecEnd, Vector( -3, -3, -3 ), Vector( 3, 3, 3 ), MASK_SHOT, info.m_pCustomFilter, &tr );
			else
...
#ifdef TF_CLASSIC
			if (info.m_pCustomFilter)
				AI_TraceLine(info.m_vecSrc, vecEnd, MASK_SHOT, info.m_pCustomFilter, &tr);
			else
				AI_TraceLine(info.m_vecSrc, vecEnd, MASK_SHOT, &traceFilter, &tr);
#else
			AI_TraceLine(info.m_vecSrc, vecEnd, MASK_SHOT, &traceFilter, &tr);
...
#ifdef TF_CLASSIC
				if (info.m_pCustomFilter)
					bHitWater = HandleShotImpactingWater(info, vecEnd, info.m_pCustomFilter, &vecTracerDest);
				else
...
#ifdef TF_CLASSIC
			if (info.m_pCustomFilter)
				HandleShotImpactingGlass(info, tr, vecDir, info.m_pCustomFilter);
			else
...
{
// This function gets called twice if done in actually networked game, weird. This server-side only now.
#ifdef GAME_DLL
	CTFPlayer* pTFPlayer = ToTFPlayer(this);
	if (pTFPlayer && params.m_nChannel == CHAN_VOICE)
	{
		//float flModifiedPitch = params.m_nPitch;
		float flModifiedPitch = 100.0f;

		float flVoicePitchScale = 1.f;
		CALL_ATTRIB_HOOK_FLOAT_ON_OTHER(pTFPlayer, flVoicePitchScale, voice_pitch_scale);

		int iHalloweenVoiceSpell = 0;
		CALL_ATTRIB_HOOK_INT_ON_OTHER(pTFPlayer, iHalloweenVoiceSpell, halloween_voice_modulation);
		if (iHalloweenVoiceSpell > 0)
		{
			flModifiedPitch *= 0.8f;
		}
		else if (flVoicePitchScale != 1.f)
		{
			flModifiedPitch *= flVoicePitchScale;
		}

		if (pTFPlayer->m_Shared.InCond(TF_COND_TRANQUILIZED))
			flModifiedPitch *= tf2c_voice_pitch_tranq.GetFloat();
		if (pTFPlayer->m_Shared.InCond(TF_COND_CIV_SPEEDBUFF))
			flModifiedPitch *= tf2c_voice_pitch_vip_haste.GetFloat();

		if (flModifiedPitch > 0 && flModifiedPitch != 100.0f)
		{
			params.m_nPitch = flModifiedPitch;
