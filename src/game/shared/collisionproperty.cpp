...
void CCollisionProperty::DestroyPartitionHandle()
{
	if ( m_Partition != PARTITION_INVALID_HANDLE )
	{
		if (this && partition)
		{
			// crash on C_BaseEntity dtor? https://gl.ofdev.xyz/tf2c/source/tf2c-src/-/issues/17 ???
			// if partition isn't garbage and `this` isnt null this crash will require engine reversing
			// -sappho
