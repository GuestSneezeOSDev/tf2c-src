...
void __MsgFunc_SavedConvar( bf_read &msg )
{
	...
	if ( bReadKey && bReadValue )
	{
		...
		if ( cvar.IsValid() && cvar.IsFlagSet( FCVAR_REPLICATED ) )
			GameRules()->m_SavedConvars[ szKey ] = AllocPooledString( szValue );
	}
}
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CGameRules::HaveSavedConvar( ConVarRef const &cvar )
{
	Assert( cvar.IsValid() );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CGameRules::SaveConvar( ConVarRef const &cvar )
{
	Assert( cvar.IsValid() );
