...
#if !defined( TF_DLL ) && !defined( TF_CLASSIC )		// TF overrides the default value of this convar
...
#if defined( CSTRIKE_DLL ) || defined( TF_DLL ) || defined( TF_CLASSIC )
	...
	void CMultiplayRules::FrameUpdatePostEntityThink()
	{
		...
		if ( mp_mapcycle_empty_timeout_seconds.GetInt() > 0 )
		{
			...
			if ( iIdleSeconds >= mp_mapcycle_empty_timeout_seconds.GetInt() )
			{
				...
				Msg( "Server has been empty for %d seconds on this map, cycling map as per mp_mapcycle_empty_timeout_seconds\n", iIdleSeconds );
				ChangeLevel();
			}
		}
	}
	...
	*/
	...
	bool CMultiplayRules::ClientCommand( CBaseEntity *pEdict, const CCommand &args )
	{
		...
		if ( FStrEq( pcmd, "voicemenu" ) )
		{
			...
			if ( pMultiPlayerPlayer && pMultiPlayerPlayer->ShouldRunRateLimitedCommand( args ) )
			{
