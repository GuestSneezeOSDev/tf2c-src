...
void ParseParticleEffects( bool bLoadSheets, bool bPrecache )
{
	...
	{
		g_pParticleSystemMgr->ReadParticleConfigFile( files[i], bPrecache, true );
	}
...
void ParseParticleEffectsMap( const char *pMapName, bool bLoadSheets, IFileList *pFilesToReload )
{
	...
	// UNDONE: Deleted manually, don't bother!
	// -sappho
	// KeyValues::AutoDelete autodelete(manifest);

	// No particle file?
	if ( !manifest->LoadFromFile( filesystem, szMapManifestFilename, "GAME" ) )
	{
		manifest->deleteThis();
		// Fallback to custom global
		const char *szGlobalManifestFilename = "maps/custom_particles.txt";
		manifest = new KeyValues( szGlobalManifestFilename );
		if ( !manifest->LoadFromFile( filesystem, szGlobalManifestFilename, "GAME" ) )
		{
			manifest->deleteThis();
			return;
		}
	...
	DevMsg( "Loading particle effects manifest '%s' for map '%s'...\n", szMapManifestFilename, pMapName );
	
	int totallines = 0;
	for ( KeyValues *sub = manifest->GetFirstSubKey(); sub != NULL; sub = sub->GetNextKey() )
	{
		// Doesn't have the "file" thing at the beginning of the string
		if ( V_stricmp( sub->GetName(), "file" ) != 0 )
		{
			Warning("CParticleMgr::LevelInit:  Manifest '%s' with bogus file type '%s', expecting 'file'. Bailing!\n", szMapManifestFilename, sub->GetName());
			g_pParticleSystemMgr->DecommitTempMemory();
			manifest->deleteThis();
			return;
		}
		...
		// knock off the exclaimation point
		char *pszPath = (szPath[0] == '!') ? &szPath[1] : &szPath[0];

		if ( !pszPath || !pszPath[0] || V_stricmp( pszPath, "particles" ) != 0 )
		{
			Warning("CParticleMgr::LevelInit:  Manifest '%s' with bogus particle data, probably not in the particles directory, check your syntax. Bailing!\n", szMapManifestFilename );
			g_pParticleSystemMgr->DecommitTempMemory();
			manifest->deleteThis();
			return;
		}

		// DevMsg("Particle string -> %s\n", sub->GetString());
		// why? servers are being naughty and doing dumb shit like this:
		// https://cdn.discordapp.com/attachments/196364568811929602/1026710841443233813/unknown.png
		// i.e. loading duplicate particles over and over, basically stunlocking clients
		// don't let em!
		// -sappho
		if ( files.HasElement( sub->GetString() ) )
		{
#ifdef CLIENT_DLL
				Warning("CParticleMgr::LevelInit:  Manifest '%s' contains duplicate particles '%s'! Ignoring particles manifest...", szMapManifestFilename, sub->GetString());
				char ipadr[64] = {};
				if ( UTIL_GetRealRemoteAddr(ipadr) && ipadr && ipadr[0] )
				{
					sentry_value_t ctxinfo = sentry_value_new_object();
					sentry_value_set_by_key(ctxinfo, "remote ip", sentry_value_new_string(ipadr));
					SentryEvent("info", __FUNCTION__, "ParseParticles_DupParticles", ctxinfo);
				}
#else
				if ( engine->IsDedicatedServer() )
				{
					Error("CParticleMgr::LevelInit:  Manifest '%s' contains duplicate particles '%s'! This is not allowed! Bailing!", szMapManifestFilename, sub->GetString());
				}
#endif
				manifest->deleteThis();
				g_pParticleSystemMgr->DecommitTempMemory();
		...
		if (totallines >= max_particles_per_map)
		{
			Warning("CParticleMgr::LevelInit:  Hit maximum number of particles! Not adding any more...\n");
#ifdef CLIENT_DLL
			char ipadr[64] = {};
			if ( UTIL_GetRealRemoteAddr(ipadr) && ipadr && ipadr[0] )
			{
				sentry_value_t ctxinfo = sentry_value_new_object();
				sentry_value_set_by_key(ctxinfo, "remote ip", sentry_value_new_string(ipadr));
				SentryEvent("info", __FUNCTION__, "ParseParticles_MaxParticles", ctxinfo);
			}
#endif
	...
	DevMsg("Loaded particle effects manifest '%s' for map '%s'...\n", szMapManifestFilename, pMapName);
