...
CNewParticleEffect *CParticleProperty::Create( const char *pszParticleName, ParticleAttachment_t iAttachType, int iAttachmentPoint, Vector vecOriginOffset )
{
	...
	if (!newEffect)
	{
		Warning("NULL newEffect at &m_ParticleEffects = %p [iIndex = %i]!\n", &(m_ParticleEffects), iIndex);
		AssertMsg(0, "NULL newEffect at &m_ParticleEffects = %p [iIndex = %i]!\n", &(m_ParticleEffects), iIndex);
		AssertMsgAlways(0, "NULL newEffect! FIX THIS!!\n");
	...
	// not calling newEffect->pParticleEffect PROBABLY causes a memory leak
	// but this is better than crashing...
	if ( !newEffect->pParticleEffect )
	{
		Warning("NULL newEffect->pParticleEffect! newEffect = %p!\n", newEffect);
		AssertMsg(0, "NULL newEffect->pParticleEffect! newEffect = %p!\n", newEffect);
		AssertMsgAlways(0, "NULL newEffect->pParticleEffect! FIX THIS!!\n");
	...
	{
		Warning("newEffect = %p ->pParticleEffect FAILED ->IsValid()! Did you precache this??\n", newEffect);
		AssertMsg(0, "newEffect = %p ->pParticleEffect FAILED ->IsValid()! Did you precache this??\n", newEffect);
		AssertMsgAlways(0, "newEffect->pParticleEffect FAILED ->IsValid()! FIX THIS!!\n");
