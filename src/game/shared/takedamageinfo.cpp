...
BEGIN_SIMPLE_DATADESC( CTakeDamageInfo )
	...
	DEFINE_FIELD (m_iMultiCount, FIELD_INTEGER),
END_DATADESC()
...
void CTakeDamageInfo::ScriptSetInflictor( HSCRIPT hInflictor )
{
...
void CTakeDamageInfo::ScriptSetAttacker( HSCRIPT hAttacker )
{
...
void CTakeDamageInfo::ScriptSetWeapon( HSCRIPT hWeapon )
{
...
void AddMultiDamage( const CTakeDamageInfo &info, CBaseEntity *pEntity )
{
	...
	g_MultiDamage.SetMultiCount(g_MultiDamage.GetMultiCount() + 1);
...
#if defined( TF_CLASSIC ) || defined( TF_CLASSIC_CLIENT )
void CTakeDamageInfo::SetCritType( int eType )
{
	if ( eType == CRIT_NONE )
	{
		// always let CRIT_NONE override the current setting
		m_eCritType = (ECritType)eType;
	}
	else
	{
		// don't let CRIT_MINI override CRIT_FULL
		m_eCritType = ( eType > m_eCritType ) ? (ECritType)eType : m_eCritType;
	}
}
#endif
