...
public:
	...
	void			SetDamageBonusProvider( CBaseEntity *pProvider );
	...
	bool			GetDamageForForceCalcOverriden() const;
	void			SetDamageForForceCalcOverriden( bool bOverriden );
	...
	int				GetMultiCount() const { return m_iMultiCount; };
	void			SetMultiCount(int iMultiCount) { m_iMultiCount = iMultiCount; }
...
#if defined( TF_CLASSIC ) || defined( TF_CLASSIC_CLIENT )
	void			SetCritType( int eType ); 

	int				GetCritType() const { return m_eCritType; }
#endif
...
protected:
	...
	int				m_iMultiCount;			// If attack was part of MultiDamage, accumulate amount of times we hit. Zero otherwise.

	float			m_flDamageForForce;
	bool			m_bDamageForForceOverriden;	// Added this to allow damage to specify its own force amount without gamerules overriding it.
...
inline void CTakeDamageInfo::SetDamageBonusProvider( CBaseEntity *pProvider )
{
	m_hDamageBonusProvider = pProvider;
}
...
inline void CTakeDamageInfo::SetDamageForForceCalcOverriden(bool bOverriden)
{
	m_bDamageForForceOverriden = bOverriden;
}
inline bool CTakeDamageInfo::GetDamageForForceCalcOverriden() const
{
	return m_bDamageForForceOverriden;
}
