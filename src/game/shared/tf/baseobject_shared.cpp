...
ConVar tf_obj_build_rotation_speed( "tf_obj_build_rotation_speed", "250", FCVAR_REPLICATED | FCVAR_CHEAT, "Degrees per second to rotate building when player alt-fires during placement." );
...
CBaseObject* CBaseObject::GetBuildPointObject( int iPoint ) const
{
...
//-----------------------------------------------------------------------------
bool CBaseObject::CanBuildObjectOnBuildPoint( int iPoint, int iObjectType ) const
{
	Assert( iPoint >= 0 && iPoint <= GetNumBuildPoints() );
...
float CBaseObject::GetMaxSnapDistance( int iPoint ) const
{
...
//-----------------------------------------------------------------------------
int CBaseObject::GetNumObjectsOnMe( void ) const
{
	int iObjects = 0;
...
//-----------------------------------------------------------------------------
int CBaseObject::FindObjectOnBuildPoint( CBaseObject *pObject ) const
{
...
CBaseObject *CBaseObject::GetObjectOfTypeOnMe( int iObjectType ) const
{
...
//-----------------------------------------------------------------------------
bool CBaseObject::ShouldBeActive( void ) const
{
...
//-----------------------------------------------------------------------------
bool CBaseObject::ShouldPlayersAvoid( void ) const
{
...
ConVar tf2c_no_arena_suddendeath_sentries("tf2c_no_arena_suddendeath_sentries", "0", FCVAR_REPLICATED, "Controls if sentries can be built in arena mode and sudden death mode.");
ConVar tf2c_no_sentries("tf2c_no_sentries", "0", FCVAR_REPLICATED, "Controls if sentries can be built at all.");
//-----------------------------------------------------------------------------
// Purpose: Check that the selected position is buildable
//-----------------------------------------------------------------------------
bool CBaseObject::IsPlacementPosValid( void )
{
	if ( !CalculatePlacementPos() )
		return false;
    ...
    // Prevent building sentries if we're in arena
    // this should probably just be in a virtual function but i am sleepy
    // -sappho
    if
    (
        (
	...
	static const Vector vecPlacementMaxs( 2, 2, 2 ), vecPlacementMins( -2, -2, -2 );

	Vector vecPlacementOrigin( 0, 0, m_vecBuildMaxs[2] * 0.5f );
	UTIL_TraceHull( pPlayer->EyePosition(), m_vecBuildOrigin + vecPlacementMaxs, vecPlacementMins, vecPlacementMaxs, MASK_PLAYERSOLID_BRUSHONLY, pPlayer, COLLISION_GROUP_NONE, &tr );
	if ( tr.fraction < 1.0f )
		return false;
...
void CBaseObject::UpdateDesiredBuildRotation( float flFrameTime )
{
	...
	m_flCurrentBuildRotation = ApproachAngle( 90.0f * m_iDesiredBuildRotations, m_flCurrentBuildRotation, tf_obj_build_rotation_speed.GetFloat() * flFrameTime );
}