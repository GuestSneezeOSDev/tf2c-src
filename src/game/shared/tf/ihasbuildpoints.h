...
public:
	...
	virtual bool		CanBuildObjectOnBuildPoint( int iPoint, int iObjectType ) const = 0;
	...
	virtual int			GetNumObjectsOnMe( void ) const = 0;

	// Get the first object of type, return NULL if no such type available
	virtual CBaseObject *GetObjectOfTypeOnMe( int iObjectType ) const = 0;
	...
	virtual float		GetMaxSnapDistance( int iPoint ) const = 0;

	// Return true if it's possible that build points on this entity may move in local space (i.e. due to animation)
	virtual bool		ShouldCheckForMovement( void ) const = 0;

	// I've finished building the specified object on the specified build point
	virtual int			FindObjectOnBuildPoint( CBaseObject *pObject ) const = 0;
};
