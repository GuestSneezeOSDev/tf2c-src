...
BEGIN_SEND_TABLE_NOBASE( CTFConditionList, DT_TFPlayerConditionListExclusive )
	SendPropInt( SENDINFO( _condition_bits ), Min<int>( TF_COND_LAST, 32 ), SPROP_UNSIGNED ),
END_SEND_TABLE()
#endif
...
void CTFConditionList::ServerThink()
{
	...
	for ( int i = 0, c = _conditions.Count(); i < c; ++i )
	{
			 ...
			 cond->GetMinDuration() > PERMANENT_CONDITION )
		{
			...
			{
				cond->SetMinDuration( Max( cond->GetMinDuration() - reduction, 0.0f ) );
			}

			// Decrement max duration.
			if ( cond->GetMaxDuration() > PERMANENT_CONDITION )
			{
				cond->SetMaxDuration( Max( cond->GetMaxDuration() - reduction, 0.0f ) );
...
{
#ifdef CLIENT_DLL
	if ( GetOuter()->m_Shared.m_pCritEffect )
	{
		...
		if ( GetOuter()->m_Shared.m_pCritEffect->GetOwner() != pWeapon )
		{
			GetOuter()->m_Shared.UpdateCritBoostEffect();
		}
	}
