...
BEGIN_DATADESC(CTeamRoundTimer)
	...
	DEFINE_INPUTFUNC( FIELD_INTEGER,	"RemoveTime",		InputRemoveTime ),
...
void CTeamRoundTimer::Precache( void )
{
	...
	PrecacheScriptSound( "music.setup" );
#endif
}
...
bool CTeamRoundTimer::IsPreRoundTimer( void )
{
	return ( ( TFGameRules()->IsInArenaMode() ) && TFGameRules()->State_Get() == GR_STATE_PREROUND );
}
...
void CTeamRoundTimer::CalculateOutputMessages( void )
{
	...
	{
		// Round up when below 10 seconds. Helps the timers that start at 10 and 5 seconds.
		flTime = ceil( flTime );
	}
#endif
...
void CTeamRoundTimer::OnPreDataChanged( DataUpdateType_t updateType )
{
	...
	m_flOldTimerEndTime = m_flTimerEndTime;
}
...
void CTeamRoundTimer::OnDataChanged( DataUpdateType_t updateType )
{
	...
	if ( m_nOldTimerLength != m_nTimerLength || m_flOldTimerEndTime != m_flTimerEndTime )
	{
...
void CTeamRoundTimer::SendTimeWarning( int nWarning )
{
	...
	// Don't play time warnings above 5 seconds in freezetime.
	if ( IsPreRoundTimer() && nWarning < RT_WARNING_5SECS )
		return;

	// don't play sounds if the level designer has turned them off or if it's during the WaitingForPlayers time
	if ( !m_bTimerPaused && m_bAutoCountdown && !TFGameRules()->IsInWaitingForPlayers() )
	{
		bool bShouldPlaySound = false;

		if ( TFGameRules()->IsInTournamentMode() == true && TFGameRules()->IsInStopWatch() == true )
		{
			int iStopWatchTimer = ObjectiveResource()->GetStopWatchTimer();

			if ( IsStopWatchTimer() == true && IsWatchingTimeStamps() == false )
			{
				CTeamRoundTimer *pTimer = ObjectiveResource()->GetTimerToShowInHUD();
			...
			else
			{
				...
				if ( ObjectiveResource()->GetTimerToShowInHUD() == this )
				{
					if ( pStopWatch )
					{
		...
		if ( bShouldPlaySound == true )
		{
			...
			{
				g_TFMusicManager.PlayMusic( TF_MUSIC_SETUP );
			}
		}
	}
}
...
void CTeamRoundTimer::RoundTimerSetupThink( void )
{
	...
	if ( TFGameRules()->IsInPreMatch() && !IsDisabled() )
	{
		inputdata_t data;
		InputDisable( data );
	...
	if ( flTime <= 0.0f && m_bFireFinished )
	{
		...
		if ( ObjectiveResource()->IsActiveTimer( this ) && !TFGameRules()->IsInWaitingForPlayers() )
		{
			UTIL_LogPrintf( "World triggered \"Round_Setup_End\"\n" );
		}
		return;
	}
	else if ( flTime <= 60.0f && m_bFire1MinRemain )
	{
		m_On1MinRemain.FireOutput( this, this );
		m_bFire1MinRemain = false;
	}
	else if ( flTime <= 30.0f && m_bFire30SecRemain )
	{
		m_On30SecRemain.FireOutput( this, this );
		m_bFire30SecRemain = false;
	}
	else if ( flTime <= 10.0f && m_bFire10SecRemain )
	{
		m_On10SecRemain.FireOutput( this, this );
		m_bFire10SecRemain = false;
	}
	else if ( flTime <= 5.0f && m_bFire5SecRemain )
	{
		m_On5SecRemain.FireOutput( this, this );
		m_bFire5SecRemain = false;
	}
	else if ( flTime <= 4.0f && m_bFire4SecRemain )
	{
		m_On4SecRemain.FireOutput( this, this );
		m_bFire4SecRemain = false;
	}
	else if ( flTime <= 3.0f && m_bFire3SecRemain )
	{
		m_On3SecRemain.FireOutput( this, this );
		m_bFire3SecRemain = false;
	}
	else if ( flTime <= 2.0f && m_bFire2SecRemain )
	{
		m_On2SecRemain.FireOutput( this, this );
		m_bFire2SecRemain = false;
	}
	else if ( flTime <= 1.0f && m_bFire1SecRemain )
	{
...
void CTeamRoundTimer::RoundTimerThink( void )
{
	...
	if ( TFGameRules()->RoundHasBeenWon() ||
		TFGameRules()->IsLoadingBugBaitReport() )
	{
...
void CTeamRoundTimer::InputRoundSpawn( inputdata_t &input )
{
	...
	if ( !m_bStartPaused && !TFGameRules()->IsInWaitingForPlayers() )
	{
		ResumeTimer();
	}
}
...
void CTeamRoundTimer::ResumeTimer( void )
{
	...
	if ( m_bTimerPaused )
	{
		...
		if ( IsStopWatchTimer() && m_bInCaptureWatchState )
		{
			m_flTimerEndTime = gpGlobals->curtime;
		}
		else
...
void CTeamRoundTimer::AddTimerSeconds( int iSecondsToAdd, int iTeamResponsible /* = TEAM_UNASSIGNED*/ )
{
	...
	if ( TFGameRules()->State_Get() != GR_STATE_RND_RUNNING && TFGameRules()->State_Get() != GR_STATE_TEAM_WIN )
		return;
	...
	if ( ( ObjectiveResource() && ObjectiveResource()->IsActiveTimer( this ) ) || ( TFGameRules()->IsInKothMode() ) )
	{
		if ( !TFGameRules()->InStalemate() && !TFGameRules()->RoundHasBeenWon() && !TFGameRules()->IsInKothMode() )
		{
...
void CTeamRoundTimer::InputRemoveTime( inputdata_t &input )
{
	if ( IsDisabled() )
		return;
	...
	// Only while round is running or won.
	if ( TFGameRules()->State_Get() != GR_STATE_RND_RUNNING && TFGameRules()->State_Get() != GR_STATE_TEAM_WIN )
		return;
	...
	if ( m_bTimerPaused )
	{
		m_flTimeRemaining -= (float)nSeconds;
	}
	else
	{
		m_flTimerEndTime -= (float)nSeconds;
	}
	...
	if ( ( ObjectiveResource() && ObjectiveResource()->IsActiveTimer( this ) ) || ( TFGameRules()->IsInKothMode() ) )
	{
		...
		{
			event->SetInt( "timer", entindex() );
			event->SetInt( "seconds_added", -nSeconds );
			gameeventmanager->FireEvent( event );
		}
	}
}
...
{ 
	...
	if ( !nShow && ObjectiveResource()->IsActiveTimer( this ) )
	{
		SetActiveTimer( NULL );
	}
	else if ( nShow == 1 )
...
{
	ObjectiveResource()->SetTimerInHUD( pNewlyActive );
}
#endif
