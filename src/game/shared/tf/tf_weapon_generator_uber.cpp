...
#define TF_WEAPON_UBERGENERATOR_MODEL		"models/items/mine/mine.mdl"
...
{
	const char *pszEffect = ConstructTeamParticle( "stickybombtrail_%s", GetTeamNumber() );
...
void CTFGeneratorUber::UpdateEffects( void )
{
	...
	for ( i = 0, c = m_hUberTargets.Count(); i < c; i++ )
	{
		...
		const char *pszEffectName = ConstructTeamParticle( "nader_uber_%s", GetTeamNumber() );
...
{
	if (m_flActivationTime && ((gpGlobals->curtime - m_flActivationTime) >= 7.0f)) {
		CTFPlayer* pOwner = ToTFPlayer(GetOwnerEntity());		
