...
ConVar tf2c_medicgl_generator_duration("tf2c_medicgl_generator_duration", "5",							FCVAR_REPLICATED, "The duration of the Uber Generator.");
...
void CTFHealLauncher::OnHealedPlayer( CTFPlayer *pPatient, float flAmount, HealerType tType )
{
	...
	if ( gpGlobals->curtime > m_flNextSuccessCue && tType == HEALER_TYPE_BURST)
	{
		...
		pOwner->EmitSound( filterOwner, pOwner->entindex(), "HealGrenade.Success" );

		CRecipientFilter filterPatient;
		filterPatient.AddRecipient( pPatient );
		pPatient->EmitSound( filterPatient, pPatient->entindex(), "HealGrenade.Success" );
...
void CTFHealLauncher::OnDataChanged( DataUpdateType_t updateType )
{
	...
	if ( m_bMainTargetParityOld != m_bMainTargetParity && UTIL_GetHSM(this) )
	{
