...
// This is nonsense, just to get this function to not be inlined.
// We're obfuscating here just in case there's some stupid bullshit,
// where you can somehow toggle cheats with this.
// that should never happen, but in case it is somehow possible
// let's give people less of a reason to look in this function...
#ifdef _WIN32
__declspec(noinline)
#endif
void
...
reloadMats()
{
	ConCommand* pCommand = g_pCVar->FindCommand(_OBFUSCATE("mat_reloadallmaterials"));
	if (pCommand)
	{
		pCommand->Dispatch(CCommand());
	}
}

void reloadmaterialscallback(IConVar* var, const char* pOldString, float flOldValue)
{
	// guard against being called on startup lol
...
static ConVar tf2c_new_lighting("tf2c_new_lighting", "2", FCVAR_MATERIAL_THREAD_MASK | FCVAR_ARCHIVE, "Enables Full-Lambertian shading, which causes models to only highlight directly-lit areas", true, 0, true, 1, reloadmaterialscallback);
							...
							CBasePerMaterialContextData **pContextDataPtr )
{
	...
	bool bPhongHalfLambert = true;
	if ( IS_PARAM_DEFINED(info.m_nPhongDisableHalfLambert) )
	{
		bPhongHalfLambert = !( params[info.m_nPhongDisableHalfLambert]->GetIntValue() || tf2c_new_lighting.GetBool() );
	}
#endif
