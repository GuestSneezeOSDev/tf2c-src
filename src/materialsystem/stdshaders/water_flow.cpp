...
DEFINE_FALLBACK_SHADER( WaterFlow, WaterFlow_DX9_HDR )

BEGIN_VS_SHADER( WaterFlow_DX90,
			  "Help for Water" )
	...
	BEGIN_SHADER_PARAMS
		...
		SHADER_PARAM( NORMALMAP, SHADER_PARAM_TYPE_TEXTURE, "dev/water_flow_normal", "normal map" )
		...
		SHADER_PARAM( REFLECTANCE, SHADER_PARAM_TYPE_FLOAT, "0", "" )
	...
	SHADER_INIT_PARAMS()
	{
		...
		InitFloatParam( REFLECTANCE, params, 0.2f );
		...
		IShaderDynamicAPI* pShaderAPI, bool bReflection, bool bRefraction ) 
	{
		...
		SHADOW_STATE
		{
			...
			DECLARE_STATIC_VERTEX_SHADER( water_flow_vs20 );
			...
			SET_STATIC_VERTEX_SHADER( water_flow_vs20 );
			...
			{
				DECLARE_STATIC_PIXEL_SHADER( water_flow_ps20b );
				...
				SET_STATIC_PIXEL_SHADER( water_flow_ps20b );
			}
			else
			{
				DECLARE_STATIC_PIXEL_SHADER( water_flow_ps20 );
				SET_STATIC_PIXEL_SHADER_COMBO( REFLECT,  bReflection );
				SET_STATIC_PIXEL_SHADER_COMBO( REFRACT,  bRefraction );
				SET_STATIC_PIXEL_SHADER_COMBO( ABOVEWATER,  params[ABOVEWATER]->GetIntValue() );
				SET_STATIC_PIXEL_SHADER_COMBO( MULTITEXTURE, bHasMultiTexture );
				...
				SET_STATIC_PIXEL_SHADER( water_flow_ps20 );
			}
		...
		DYNAMIC_STATE
		{
			...
			c7[3] = params[REFLECTANCE]->GetFloatValue();
			if (g_pHardwareConfig->GetHDRType() == HDR_TYPE_INTEGER )
			{
			...
			DECLARE_DYNAMIC_VERTEX_SHADER( water_flow_vs20 );
			SET_DYNAMIC_VERTEX_SHADER( water_flow_vs20 );
			...
			if ( g_pHardwareConfig->SupportsPixelShaders_2_b() )
			{
				DECLARE_DYNAMIC_PIXEL_SHADER( water_flow_ps20b );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( FLASHLIGHTSHADOWS, bFlashlightShadows );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, ( pShaderAPI->GetSceneFogMode() == MATERIAL_FOG_LINEAR_BELOW_FOG_Z ) );
				SET_DYNAMIC_PIXEL_SHADER( water_flow_ps20b );
			}
			else
			{
				DECLARE_DYNAMIC_PIXEL_SHADER( water_flow_ps20 );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( FLASHLIGHTSHADOWS, 0 );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, ( pShaderAPI->GetSceneFogMode() == MATERIAL_FOG_LINEAR_BELOW_FOG_Z ) );
				SET_DYNAMIC_PIXEL_SHADER( water_flow_ps20 );
			}
		                        ...
		                        IShaderDynamicAPI* pShaderAPI, bool bBlend, bool bRefraction )
	{
		...
		SHADOW_STATE
		{
			...
			DECLARE_STATIC_VERTEX_SHADER( watercheap_flow_vs20 );
			SET_STATIC_VERTEX_SHADER_COMBO( BLEND,  bBlend && bRefraction );
			SET_STATIC_VERTEX_SHADER( watercheap_flow_vs20 );

			if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
			{
				DECLARE_STATIC_PIXEL_SHADER( watercheap_flow_ps20b );
				...
				SET_STATIC_PIXEL_SHADER_COMBO( FLOWMAP, bHasFlowmap );
				SET_STATIC_PIXEL_SHADER_COMBO( FLOW_DEBUG, clamp( params[ FLOW_DEBUG ]->GetIntValue(), 0, 2 ) );
				SET_STATIC_PIXEL_SHADER( watercheap_flow_ps20b );
			}
			else
			{
				DECLARE_STATIC_PIXEL_SHADER( watercheap_flow_ps20 );
				SET_STATIC_PIXEL_SHADER_COMBO( FRESNEL,  params[NOFRESNEL]->GetIntValue() == 0 );
				SET_STATIC_PIXEL_SHADER_COMBO( BLEND,  bBlend );
				SET_STATIC_PIXEL_SHADER_COMBO( REFRACTALPHA,  bRefraction );
				SET_STATIC_PIXEL_SHADER_COMBO( HDRTYPE,  g_pHardwareConfig->GetHDRType() );
				...
				SET_STATIC_PIXEL_SHADER_COMBO( FLOWMAP, bHasFlowmap );
				SET_STATIC_PIXEL_SHADER_COMBO( FLOW_DEBUG, clamp( params[ FLOW_DEBUG ]->GetIntValue(), 0, 2 ) );
				SET_STATIC_PIXEL_SHADER( watercheap_flow_ps20 );
			}
		...
		DYNAMIC_STATE
		{
			...
			DECLARE_DYNAMIC_VERTEX_SHADER( watercheap_flow_vs20 );
			SET_DYNAMIC_VERTEX_SHADER( watercheap_flow_vs20 );

			if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
			{
				DECLARE_DYNAMIC_PIXEL_SHADER( watercheap_flow_ps20b );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( HDRENABLED,  IsHDREnabled() );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, ( pShaderAPI->GetSceneFogMode() == MATERIAL_FOG_LINEAR_BELOW_FOG_Z ) );
				SET_DYNAMIC_PIXEL_SHADER( watercheap_flow_ps20b );
			}
			else
			{
				DECLARE_DYNAMIC_PIXEL_SHADER( watercheap_flow_ps20 );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( HDRENABLED,  IsHDREnabled() );
				SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, ( pShaderAPI->GetSceneFogMode() == MATERIAL_FOG_LINEAR_BELOW_FOG_Z ) );
				SET_DYNAMIC_PIXEL_SHADER( watercheap_flow_ps20 );
			}
		}
		Draw();
	}
...
//-----------------------------------------------------------------------------
// This allows us to use a block labelled 'water_flow_DX9_HDR' in the water materials
//-----------------------------------------------------------------------------
BEGIN_INHERITED_SHADER( WaterFlow_DX9_HDR, WaterFlow_DX90,
			  "Help for water_flow_DX9_HDR" )

	SHADER_FALLBACK
	{
		if( g_pHardwareConfig->GetHDRType() == HDR_TYPE_NONE )
		{
			return "waterflow_DX90";
		}
		return 0;
	}
