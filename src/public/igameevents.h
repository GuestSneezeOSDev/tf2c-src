...
#else
	// if you use this in server code it causes hellish shitcode that results in leaks and impossible to track down bugs
	// this is stubbed to prevent programmers from footgun-ing themselves
	// it's also stubbed in this way to prevent fucking up the vtable order. probably. i can't tell but i'd rather try to maintain bcompat.
	// -sappho
	virtual bool FireEventClientSide_DONT_USE_THIS_IN_SERVER_CODE(IGameEvent* event) = 0;
	#define FireEventClientSide DONT_USE_THIS_IN_SERVER_CODE
#endif
