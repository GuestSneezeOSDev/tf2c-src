...
class Color;
	...
	#define _OBFUSCATE
#else
	#define _OBFUSCATE AY_OBFUSCATE
#endif

#define FORCE_OBFUSCATE AY_OBFUSCATE
...
#include <../game/shared/sdk13-gigalib/src/obfuscate.h>
...
#ifdef _WIN32
    #define Msg(pMsg, ...)      Msg     (_OBFUSCATE(pMsg), __VA_ARGS__)
    #define Warning(pMsg, ...)  Warning (_OBFUSCATE(pMsg), __VA_ARGS__)
    #define Log(pMsg, ...)      Log     (_OBFUSCATE(pMsg), __VA_ARGS__)
#else
    #if defined (__GNUC__) && !defined (_DEBUG)
        #pragma GCC diagnostic push
        /* The following allows us to use these pragmas irrespective
           of the gcc version. */
...
/* TF2 Classic: assert_cast<> for reference types */
#if defined(TF_CLASSIC) || defined(TF_CLASSIC_CLIENT)
#include <type_traits>
template<typename DEST_REFERENCE_TYPE, typename SOURCE_REFERENCE_TYPE>
inline DEST_REFERENCE_TYPE assert_cast(SOURCE_REFERENCE_TYPE& rSource)
{
	// if we ever get a non-ancient GCC build with C++14 support, prefer the non-typedef version:
	// using DEST_POINTER_TYPE = std::add_pointer_t<DEST_REFERENCE_TYPE>;
	typedef typename std::add_pointer<DEST_REFERENCE_TYPE>::type DEST_POINTER_TYPE;
	
	// do the static/dynamic check using pointers: while we could do the dynamic_cast with references,
	// the failure condition for references is a std::bad_cast exception, which we can't actually catch
	SOURCE_REFERENCE_TYPE *pSource = &rSource;
	Assert( static_cast<DEST_POINTER_TYPE>(pSource) == dynamic_cast<DEST_POINTER_TYPE>(pSource) );
	
	// ultimately do the static_cast directly with references
	return static_cast<DEST_REFERENCE_TYPE>(rSource);
}
#endif
