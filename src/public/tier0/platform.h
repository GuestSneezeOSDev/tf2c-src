...
#ifdef _WIN32
        #define DONT_OPTIMIZE_START    __pragma (optimize( "", off ) )
        #define DONT_OPTIMIZE_END      __pragma (optimize( "", on ) )
#else
        #define DONT_OPTIMIZE_START    /* _Pragma("GCC push_options") _Pragma("GCC optimize O0") */
        #define DONT_OPTIMIZE_END      /* _Pragma("GCC pop_options") */
#endif
...
#define MAX_UNICODE_PATH_IN_UTF8 MAX_UNICODE_PATH*4
	...
	// hack to make c++ stdlib not name collide (VALVEEE WHY)
	// i do not know why we arent using stdlib.h
	// TODO PUT THIS IN SDKLIB
	// -sappho
	//#define offsetof_HACK(s,m)	(size_t)&(((s *)0)->m)
	//#define __builtin_offsetof(TYPE, MEMBER) offsetof_HACK(TYPE, MEMBER)
// win32
#else
	#undef offsetof
	#define offsetof(s,m)	(size_t)&(((s *)0)->m)
...
// C functions for external declarations that call the appropriate C++ methods [SERVER ONLY]
#ifndef SERVERONLY_EXPORT
	#ifdef GAME_DLL
		#ifdef _WIN32
			#define SERVERONLY_EXPORT	_declspec( dllexport )
		#else
			#define SERVERONLY_EXPORT	/* */
		#endif
	#else
		#define SERVERONLY_EXPORT
	#endif
...
#ifdef GAME_DLL
	#define SERVERONLY_DLL_EXPORT extern "C" __declspec( dllexport )
#else
	#define SERVERONLY_DLL_EXPORT
#endif
...
#ifdef GAME_DLL
	#define SERVERONLY_DLL_EXPORT extern "C" __attribute__ ((visibility("default")))
#else
	#define SERVERONLY_DLL_EXPORT
#endif
...
#define strcmpi(s1,s2) V_tier0_stricmp( s1, s2 )
	...
	/* sigsegv: workarounds to enable modern C++ features on the shitty old
	 * compiler versions Valve forces us to be compatible with */

//	#include <boost/config.hpp>

	/* NOTE: 20220729: we have all of Boost in this project, but we literally
	 * only actually use a few features of Boost.Config; so for now we'll just
	 * reimplement those manually and ditch Boost until we ever decide we want
	 * the whole shebang again */

	//////////////////////////////////////////////////////
	// BEGIN: ABBREVIATED LOCAL VERSION OF BOOST.CONFIG //
	//////////////////////////////////////////////////////
	...
	#elif defined(__has_cpp_attribute) && __has_cpp_attribute(noreturn)
	...
	//////////////////////////////////////////////////////
	// END:   ABBREVIATED LOCAL VERSION OF BOOST.CONFIG //
	//////////////////////////////////////////////////////
	...
	// GCC:   4.6+
	// Clang: 3.1+
	// MSVC:  2015+ <--- seriously wtf
	#if defined(BOOST_NO_CXX11_CONSTEXPR)
	//	#define constexpr BOOST_CONSTEXPR
		#define constexpr BOOST_CONSTEXPR_OR_CONST
	#endif

	#undef  NORETURN
	#define NORETURN BOOST_NORETURN

//	#undef  DECL_ALIGN
//	#define DECL_ALIGN(x) BOOST_ALIGNMENT(x)
