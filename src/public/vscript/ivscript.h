...
DECLARE_DEDUCE_FIELDTYPE( FIELD_EHANDLE,    const CBaseHandle& );
...
DECLARE_NAMED_FIELDTYPE( const char *,       "string" );
DECLARE_NAMED_FIELDTYPE( char *,             "string" );
DECLARE_NAMED_FIELDTYPE( Vector,             "vector" );
DECLARE_NAMED_FIELDTYPE( const Vector&,      "vector" );
DECLARE_NAMED_FIELDTYPE( QAngle,             "angle" );
DECLARE_NAMED_FIELDTYPE( const QAngle&,      "angle" );
...
DECLARE_NAMED_FIELDTYPE( int,                "int" );
DECLARE_NAMED_FIELDTYPE( bool,               "bool" );
DECLARE_NAMED_FIELDTYPE( char,               "character" );
DECLARE_NAMED_FIELDTYPE( CBaseHandle,        "ehandle" );
DECLARE_NAMED_FIELDTYPE( const CBaseHandle&, "ehandle" );
DECLARE_NAMED_FIELDTYPE( HSCRIPT,            "handle" );
...
struct ScriptVariant_t
{
	...
	ScriptVariant_t( unsigned int val ) :	m_flags( 0 ), m_type( FIELD_INTEGER )	{ m_int = val;}
	...
	ScriptVariant_t( const CBaseHandle *val ) :	m_flags( 0 ), m_type( FIELD_EHANDLE ) { m_pEHandle = val; }
	ScriptVariant_t( const CBaseHandle &val ) : m_flags( 0 ), m_type( FIELD_EHANDLE ) { m_pEHandle = &val; }
	...
	ScriptVariant_t( const Vector *val, bool bCopy = false ) :	m_flags( 0 ),	m_type( FIELD_VECTOR )		{ if ( !bCopy ) { m_pVector = val; } else { m_pVector = new Vector( *val ); m_flags |= SV_FREE; } }
	ScriptVariant_t( const QAngle &val, bool bCopy = false ) :	m_flags( 0 ),	m_type( FIELD_QANGLE )		{ if ( !bCopy ) { m_pAngle = &val; } else { m_pAngle = new QAngle( val ); m_flags |= SV_FREE; } }
	ScriptVariant_t( const QAngle *val, bool bCopy = false ) :	m_flags( 0 ),	m_type( FIELD_QANGLE )		{ if ( !bCopy ) { m_pAngle = val; } else { m_pAngle = new QAngle( *val ); m_flags |= SV_FREE; } }
	...
	operator const QAngle &() const			{ Assert( m_type == FIELD_QANGLE );		static QAngle vecNull(0, 0, 0); return (m_pAngle) ? *m_pAngle : vecNull; }
	...
	operator const CBaseHandle &( ) const	{ Assert( m_type == FIELD_EHANDLE );	static CBaseHandle nullHandle; return (m_pEHandle) ? *m_pEHandle : nullHandle; }
	operator HSCRIPT() const				{ Assert( m_type == FIELD_HSCRIPT );	return m_hScript; }

	void operator=( int i ) 				{ m_type = FIELD_INTEGER; m_int = i; }
	void operator=( unsigned int i ) 		{ m_type = FIELD_INTEGER; m_int = i; }
	...
	void operator=( const Vector *vec )		{ m_type = FIELD_VECTOR; m_pVector = vec; }
	void operator=( const QAngle &vec )		{ m_type = FIELD_QANGLE; m_pAngle = &vec; }
	void operator=( const QAngle *vec )		{ m_type = FIELD_QANGLE; m_pAngle = vec; }
	...
	void operator=( const CBaseHandle *h )  { m_type = FIELD_EHANDLE; m_pEHandle = h; }
	void operator=( const CBaseHandle &h )  { m_type = FIELD_EHANDLE; m_pEHandle = &h; }
	void operator=( HSCRIPT h ) 			{ m_type = FIELD_HSCRIPT; m_hScript = h; }

	void Free()								{ if ( ( m_flags & SV_FREE ) && ( m_type == FIELD_HSCRIPT || m_type == FIELD_VECTOR || m_type == FIELD_QANGLE || m_type == FIELD_CSTRING || m_type == FIELD_QUATERNION || m_type == FIELD_EHANDLE ) ) delete m_pszString; } // Generally only needed for return results
	...
	bool AssignTo( HSCRIPT *pDest )
	{
		if ( m_type != FIELD_VOID )
		{
			...
			DevWarning( "No free conversion of %s variant to HSCRIPT right now\n", ScriptFieldTypeName( m_type ) );
		}
		*pDest = NULL;
		return false;
	}
	...
	bool AssignTo( Vector *pDest )
	{
		...
		case FIELD_QANGLE:	*pDest = *m_pVector; return true; // Dirty type promotion
		case FIELD_CSTRING:	if ( sscanf( m_pszString, "%f %f %f", &pDest->x, &pDest->y, &pDest->z ) == 3 ) return true;
		default:
			DevWarning( "No free conversion of %s variant to Vector right now\n", ScriptFieldTypeName( m_type ) );
			*pDest = vec3_origin;
			return false;
		}
	}
	...
	bool AssignTo( float *pDest )
	{
		...
		case FIELD_FLOAT:		*pDest = m_float; return true;
		case FIELD_BOOLEAN:		*pDest = m_bool; return true;
		case FIELD_CSTRING:		*pDest = strtof( m_pszString, NULL ); return true;
		default:
	...
	bool AssignTo( int *pDest )
	{
		...
		case FIELD_FLOAT:		*pDest = m_float; return true;
		case FIELD_BOOLEAN:		*pDest = m_bool; return true;
		case FIELD_CSTRING:		*pDest = strtol( m_pszString, NULL, 10 ); return true;
		default:
	...
	bool AssignTo( bool *pDest )
	{
		...
		case FIELD_BOOLEAN:		*pDest = m_bool; return true;
		case FIELD_CSTRING:		*pDest = strtob( m_pszString ); return true;
		default:
	...
	bool AssignTo( ScriptVariant_t *pDest )
	{
		...
		switch ( m_type )
		{
			case FIELD_VECTOR:
				...
				break;
			case FIELD_QANGLE:
				pDest->m_pAngle = new QAngle;
				((QAngle *)(pDest->m_pAngle))->Init( m_pAngle->x, m_pAngle->y, m_pAngle->z );
				pDest->m_flags |= SV_FREE;
				break;
			...
			case FIELD_MATRIX3X4:
				...
				break;
			case FIELD_EHANDLE:
				pDest->m_pEHandle = new CBaseHandle( m_pEHandle->ToInt() );
				pDest->m_flags |= SV_FREE;
				break;
			case FIELD_CSTRING:
...
private:
	bool strtob( const char *pszString )
	{
		if ( !V_strcmp( pszString, "0" ) || !V_strcmp( pszString, "false" ) || !V_strcmp( pszString, "no" ) )
			return false;
		if ( !V_strcmp( pszString, "1" ) || !V_strcmp( pszString, "true" ) || !V_strcmp( pszString, "yes" ) )
			return true;
...
template <typename T> 
IScriptInstanceHelper *GetScriptInstanceHelper( IScriptInstanceHelper *pHelper )
{
	return pHelper;
}
...
#define BEGIN_SCRIPTDESC_NAMED( className, baseClass, scriptName, description ) \
	extern IScriptInstanceHelper *GetScriptInstanceHelper_##baseClass(); \
	IScriptInstanceHelper *GetScriptInstanceHelper_##className() \
	{ \
		return GetScriptInstanceHelper<className>( GetScriptInstanceHelper_##baseClass() ); \
	}; \
...
#define DEFINE_SCRIPT_INSTANCE_HELPER( className, p )										template <> IScriptInstanceHelper *GetScriptInstanceHelper<className>( IScriptInstanceHelper * ) { return p; }
