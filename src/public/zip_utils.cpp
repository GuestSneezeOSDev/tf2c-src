...
public:
	...
	bool			FileExistsInZip( const char *relativename, bool fuzzy = false);
...
// Input  : *relativename - name to look for
//        : fuzzy         - if we should look for filenames that contain pRelativeName
//-----------------------------------------------------------------------------
bool CZipFile::FileExistsInZip( const char *pRelativeName, bool fuzzy )
{
	// Lower case only
	char pName[512];
	Q_strncpy( pName, pRelativeName, 512 );
...
public:
	...
	// Whether a file is contained in a zip - maintains alignment. fuzzy will strstr search all files contained in the zip if an exact match is not found -sappho
	virtual bool			FileExistsInZip( const char *pRelativeName, bool fuzzy = false ) OVERRIDE;
...
bool CZip::FileExistsInZip( const char *pRelativeName, bool fuzzy )
{
	return m_ZipFile.FileExistsInZip( pRelativeName, fuzzy );
}
