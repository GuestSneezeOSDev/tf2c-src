...
#include <vgui_controls/CircularProgressBarSegmented.h>
...
DECLARE_BUILD_FACTORY( CircularProgressBarSegmented );

//-----------------------------------------------------------------------------
// Purpose: Constructor
//-----------------------------------------------------------------------------
CircularProgressBarSegmented::CircularProgressBarSegmented(Panel *parent, const char *panelName) : ProgressBar(parent, panelName)
{
...
// Purpose: Destructor
//-----------------------------------------------------------------------------
CircularProgressBarSegmented::~CircularProgressBarSegmented()
{
	for ( int i = 0; i < NUM_PROGRESS_TEXTURES; i++ )
	{
...
void CircularProgressBarSegmented::ApplySettings(KeyValues *inResourceData)
{
	for ( int i = 0; i < NUM_PROGRESS_TEXTURES; i++ )
	{
		delete [] m_pszImageName[i];
...
void CircularProgressBarSegmented::ApplySchemeSettings(IScheme *pScheme)
{
...
//-----------------------------------------------------------------------------
void CircularProgressBarSegmented::SetImage(const char *imageName, progress_textures_t iPos)
{
...
void CircularProgressBarSegmented::PaintBackground()
{
...
void CircularProgressBarSegmented::Paint()
{
...
typedef struct
{
	...
	int swipe_dir_x;
	int swipe_dir_y;
} circular_progress_segment_segmented_t;

namespace vgui
{
	// This defines the properties of the 8 circle segments
	// in the circular progress bar.
	circular_progress_segment_segmented_t Segments2[8] = 
	{
		{ M_PI * 0.25,	0.5, 0.0, 1.0, 0.0, 1, 0 },
		{ M_PI * 0.5,	1.0, 0.0, 1.0, 0.5, 0, 1 },
		{ M_PI * 0.75,	1.0, 0.5, 1.0, 1.0, 0, 1 },
		{ M_PI,			1.0, 1.0, 0.5, 1.0, -1, 0 },
		{ M_PI * 1.25,	0.5, 1.0, 0.0, 1.0, -1, 0 },
		{ M_PI * 1.5,	0.0, 1.0, 0.0, 0.5, 0, -1 },
		{ M_PI * 1.75,	0.0, 0.5, 0.0, 0.0, 0, -1 },
		{ M_PI * 1.98,	0.0, 0.0, 0.5, 0.0, 1, 0 },
	};
...
void CircularProgressBarSegmented::DrawCircleSegment( Color c, float flEndProgress, bool bClockwise )
{
	...
	for ( int i=0;i<8;i++ )
	{
		if ( flEndProgressRadians > Segments2[cur_wedge].minProgressRadians)
		{
			vgui::Vertex_t v[3];
		...
		//	float flInternalProgress = flEndProgressRadians - Segments[cur_wedge].minProgressRadians;
			...
			v[2].m_Position.Init( flHalfWide + flWide * ( Segments2[i].vert2x - 0.5 ), flHalfTall + flTall * ( Segments2[i].vert2y - 0.5 ) );
			v[2].m_TexCoord.Init( Segments2[i].vert2x, Segments2[i].vert2y );
		//	} 

			// vert 2 is ( Segments[i].vert1x, Segments[i].vert1y )
			v[1].m_Position.Init( flHalfWide + flWide * ( Segments2[i].vert1x - 0.5 ), flHalfTall + flTall * ( Segments2[i].vert1y - 0.5 ) );
			v[1].m_TexCoord.Init( Segments2[i].vert1x, Segments2[i].vert1y );
