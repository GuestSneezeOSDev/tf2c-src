...
void Splitter::EvenlyRespaceSplitters( )
{
	...
	float flUnlockedSize = static_cast<float>( GetPosRange() - nLockedSize );
	float flDPos = flUnlockedSize / static_cast<float>(nSplitterCount);
	if ( flDPos < static_cast<float>(SPLITTER_HANDLE_WIDTH) )
	{
...
{ 
	...
	for ( int i = 0; i < c; i++ )
	{
		...
		pFractions[i] = userConfig->GetFloat( name, flTotalSize + static_cast<float>(SPLITTER_HANDLE_WIDTH) + 1.0f );
